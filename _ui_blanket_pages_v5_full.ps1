...
Backup $PathRoot  "ui_blanket_v5_full"
Backup $PathLogin "ui_blanket_v5_full"
Backup $PathLobby "ui_blanket_v5_full"
Backup $PathRoom  "ui_blanket_v5_full"
Backup $PathRsub  "ui_blanket_v5_full"
...
function Avatar(props: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      background: "rgba(255,255,255,.07)", border: "1px solid rgba(148,163,184,.16)",
      color: "rgba(243,244,246,.95)", fontWeight: 1000, flex: "0 0 auto"
    }}>
...
export default function LobbyPage() {
  ...
  const grid = { display: "grid", gridTemplateColumns: "320px minmax(520px, 1fr)", gap: 14, alignItems: "start" } as React.CSSProperties;
    ...
    <div style={page}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <Link href={`/r/${encodeURIComponent(sub)}`} style={{ padding: "8px 10px", borderRadius: 12, border: "1px solid rgba(148,163,184,.20)", background: "rgba(255,255,255,.05)", fontWeight: 950, textDecoration: "none", color: "rgba(243,244,246,.98)" }}>
            Open subreddit
          </Link>

          <button onClick={loadRooms} style={{ padding: "8px 10px", borderRadius: 12, border: "1px solid rgba(148,163,184,.20)", background: "rgba(255,255,255,.05)", color: "rgba(243,244,246,.98)", fontWeight: 950 }}>
            Refresh
          </button>
        </div>
      </div>
      ...
      <div style={grid}>
        <section style={panel}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="Create a room…"
              style={{ flex: 1, padding: "10px 10px", borderRadius: 12, border: "1px solid rgba(148,163,184,.18)", background: "rgba(255,255,255,.05)", color: "rgba(243,244,246,.95)", outline: "none", fontWeight: 900 }} />
            <button onClick={createRoom}
              style={{ padding: "10px 10px", borderRadius: 12, border: "1px solid var(--weered-accent-ring, rgba(14,165,233,.34))", background: "var(--weered-accent-bg, rgba(14,165,233,.16))", color: "rgba(243,244,246,.95)", fontWeight: 1000 }}>
              Create
            </button>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 520, overflow: "auto", paddingRight: 4 }}>
            {rooms.map((r: any) => {
              ...
              const go = rid.startsWith("lobby:")
                ? `/lobby?sub=${encodeURIComponent(rid.slice("lobby:".length))}`
                : `/room/${encodeURIComponent(rid)}`;

              return (
                <Link key={rid || Math.random()} href={go} style={{ ...card, textDecoration: "none", color: "rgba(243,244,246,.96)" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0 }}>
                    <Avatar name={title} size={28} />
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 1000, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{title}</div>
                      <div style={{ opacity: 0.70, fontSize: 12, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        id: {rid} · users: {users}
                      </div>
                    </div>
                  </div>
                  <div style={{ padding: "6px 10px", borderRadius: 999, border: "1px solid rgba(148,163,184,.18)", background: "rgba(255,255,255,.04)", fontWeight: 950, fontSize: 12 }}>
                    Open
                  </div>
                </Link>
              );
...
Set-Content -LiteralPath $PathRsub  -Value $ContentRsub  -Force
Write-Host "OK Wrote:" $PathRsub
Set-Content -LiteralPath $PathLobby -Value $ContentLobby -Force
Write-Host "OK Wrote:" $PathLobby
Set-Content -LiteralPath $PathRoom  -Value $ContentRoom  -Force
Write-Host "OK Wrote:" $PathRoom
