...
function CrewFlair({ userId, size = 13 }: { userId: string; size?: number }) {
  ...
  useEffect(() => {
    ...
    if (existing) { existing.then(() => setFlair(crewFlairCache.get(userId) ?? null)); return; }
    const token = (typeof window !== "undefined" ? localStorage.getItem("weered_token") : "") || "";
    const p = fetch(`${API}/profile/${encodeURIComponent(userId)}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    }).then(r => r.json()).then(j => {
      const pc = j?.primaryCrew;
      crewFlairCache.set(userId, pc ? { tag: String(pc.tag || ""), logoUrl: pc.logoUrl || null, accentColor: pc.accentColor || null } : null);
    }).catch(() => { crewFlairCache.set(userId, null); }).finally(() => {
  ...
  const accent = flair.accentColor && /^#[0-9a-f]{6}$/i.test(flair.accentColor) ? flair.accentColor : "rgba(201,160,102,0.7)";
  if (flair.logoUrl) {
    return (
      <span
        title={`Crew: [${flair.tag || ""}]`}
        style={{
          width: size, height: size, borderRadius: 3, display: "inline-block",
          backgroundImage: `url(${flair.logoUrl})`, backgroundSize: "cover", backgroundPosition: "center",
  ...
  if (flair.tag) {
    return (
      <span
        title={`Crew: [${flair.tag}]`}
        style={{
          fontSize: Math.max(8, size - 4), fontWeight: 800, letterSpacing: "1px",
          padding: "0 4px", borderRadius: 2, color: accent,
          border: `1px solid ${accent}`, fontFamily: "ui-monospace, monospace",
          lineHeight: `${size}px`,
        }}
      >{flair.tag}</span>
    );
...
export default function LobbyChatPanel(
  ...
  return (
    <>
    <div className="weered-chat-layout" style={{ display: "flex", flexDirection: "row", height: "100%", minHeight: 0, ...props.style }}>
    ...
    <aside className="weered-chat-members" aria-label="Room members">
      {(() => {
        ...
        return (
          <>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "2px", textTransform: "uppercase", color: "var(--weered-muted, rgba(148,163,184,.7))", marginBottom: 10 }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: "transparent", cursor: isMe ? "default" : "pointer",
                      textAlign: "left", fontFamily: "inherit",
                      transition: "background .12s, border-color .12s",
                    }}
                    onMouseOver={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,.04)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,.08)"; }}
                  ...
                  >
                    <div style={{
                      ...
                      background: u?.avatar ? "rgba(255,255,255,.08)" : avatarBg(String(u?.name || "?"), false, u?.avatarColor),
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 11, fontWeight: 800, color: "#fff", overflow: "hidden",
                    }}>
                      {u?.avatar ? <img src={u.avatar} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : String(u?.name || "?").slice(0, 1).toUpperCase()}
                    </div>
                    <span style={{ ...ns, fontSize: 12, fontWeight: 700, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
