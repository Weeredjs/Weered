...
export default function GtaLfgBoard({
  lobbyId,
  accent = ACCENT,
  currentUserId,
}: {
  ...
  const load = useCallback(() => {
    apiFetch(`/lfg/${encodeURIComponent(lobbyId)}`)
      .then((j) => {
        setPosts((j.posts || []).filter((p: Post) => (p.roleSlots || []).length > 0));
        setLoading(false);
      })
  ...
  function setRole(i: number, v: string) {
    setRoles((r) => r.map((x, idx) => (idx === i ? v : x)));
  }
  function addRole() {
    setRoles((r) => (r.length < 12 ? [...r, "Player"] : r));
  }
  function removeRole(i: number) {
    setRoles((r) => (r.length > 1 ? r.filter((_, idx) => idx !== i) : r));
  }

  async function create() {
    if (!authHeaders().Authorization) {
      setMsg("Log in to post a session.");
      return;
    }
    setBusy("create");
  ...
  async function claim(postId: string, index: number) {
    if (!authHeaders().Authorization) {
      setMsg("Log in to claim a slot.");
      return;
    }
    setBusy(`${postId}:${index}`);
    const j = await apiFetch(`/lfg/${postId}/roles/${index}/claim`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    setBusy("");
  ...
  async function release(postId: string) {
    setBusy(`${postId}:rel`);
    const j = await apiFetch(`/lfg/${postId}/roles/release`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    setBusy("");
  ...
  async function complete(postId: string) {
    setBusy(`${postId}:done`);
    const j = await apiFetch(`/lfg/${postId}/complete`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    setBusy("");
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 11, color: "rgba(148,163,184,.7)" }}>
            Post a session, claim a role, run it. Completing a session builds your reputation.
          </div>
        </div>
        <button
          type="button"
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, margin: "6px 0 12px" }}>
            {roles.map((r, i) => (
              <div key={i} style={{ display: "flex", gap: 6, alignItems: "center" }}>
                ...
                />
                {i === 0 ? (
                  <span style={{ fontSize: 9, color: "rgba(148,163,184,.6)", width: 50 }}>you</span>
                ) : (
                  <button
                    type="button"
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
            ...
            {scheduledFor ? (
              ...
              <span style={{ fontSize: 11, color: "rgba(148,163,184,.6)" }}>
                leave empty for now / ASAP
              </span>
            )}
          </div>
          <input
          ...
          >
            {busy === "create" ? "Posting…" : "Post session"}
          </button>
        </div>
      )}
          ...
          {posts.map((p) => {
            ...
            return (
              <div
                key={p.id}
                style={{
                  border: `1px solid ${p.status === "FULL" ? `${accent}55` : "rgba(255,255,255,.08)"}`,
                  background: "rgba(255,255,255,.02)",
                  padding: 12,
                }}
              >
                ...
                >
                  <span>
                    {p.platform && p.platform !== "crossplay" ? p.platform : "Any platform"}
                    {p.region ? ` · ${p.region}` : ""} · host {p.userName} · {filled}/
                    {p.roleSlots.length}
                  </span>
                  {(p.hostCompleted ?? 0) > 0 && (
                    <span
                      title={`${p.hostCompleted} sessions completed`}
                      style={{
                        fontFamily: "var(--font-barlow), sans-serif",
                        fontSize: 9,
                        fontWeight: 800,
                        letterSpacing: "1px",
                        textTransform: "uppercase",
                        color: tierColor(p.hostTier),
                        border: `1px solid ${tierColor(p.hostTier)}55`,
                        padding: "1px 5px",
                      }}
                    >
                ...
                {p.description && (
                  <div style={{ fontSize: 12, color: "rgba(203,213,225,.8)", marginBottom: 8 }}>
                    {p.description}
                  </div>
                )}
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          gap: 8,
                          padding: "5px 8px",
                          background: open ? "transparent" : "rgba(124,58,237,0.08)",
                          border: `1px solid ${open ? "rgba(255,255,255,.06)" : `${accent}33`}`,
                        }}
                      >
                        <span
                          style={{
                            ...
                            fontSize: 12,
                            fontWeight: 700,
                            color: open ? "rgba(148,163,184,.85)" : "rgba(243,244,246,.95)",
                          }}
                        >
                          {role}
                        </span>
                        {open ? (
                          p.status === "OPEN" && !inIt ? (
                            <button
                              type="button"
                              onClick={() => claim(p.id, i)}
                              disabled={busy === `${p.id}:${i}`}
                              style={{
                                padding: "3px 10px",
                                fontSize: 10,
                                fontWeight: 800,
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                  {inIt && !isHost && (
                    <button
                      ...
                      style={miniBtn("rgba(148,163,184,.85)")}
                    >
                      Leave
                    </button>
                  )}
                  {isHost && p.status !== "COMPLETED" && (
                    <button
                      type="button"
                      onClick={() => complete(p.id)}
                      style={miniBtn("#86efac", "rgba(34,197,94,0.4)")}
                    >
                      Mark complete
                    </button>
                  )}
                  {isHost && (
                    <button
                      type="button"
                      onClick={() => del(p.id)}
                      style={miniBtn("rgba(252,165,165,.85)")}
                    >
                      Delete
                    </button>
                  )}
