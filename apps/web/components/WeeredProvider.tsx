...
type Ctx = {
  ...
  activeRoomId: string;      // desired / viewed room
  joinedRoomId: string;      // actually joined room
  setActiveRoomId: (id: string) => void;

  users: RoomUser[];         // for active room
  msgs: ChatMsg[];           // for active room
  meta: RoomMeta | null;     // for active room
  admin: AdminState | null;  // for active room (mods only)
  role: Role;                // my role in active room
  joinStatus: JoinStatus;    // status for active room
...
export function WeeredProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === "presence:state") {
        ...
        // join-confirmation: if this is the active room, consider it joined and leave old joined room
  ...
  // Join active room (do NOT leave current joined room until join confirmed)
  useEffect(() => {
    if (!wsReady) return;
