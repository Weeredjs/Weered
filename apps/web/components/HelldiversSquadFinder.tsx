...
const S = {
  ...
  btnPri: { padding: "7px 14px", borderRadius: 6, border: `1px solid ${ACCENT}50`, background: `${ACCENT}18`, fontSize: 12, cursor: "pointer", color: ACCENT, fontWeight: 700, fontFamily: "inherit", letterSpacing: ".5px" } as React.CSSProperties,
  btnDanger: { padding: "5px 12px", borderRadius: 6, border: "1px solid rgba(239,68,68,.35)", background: "rgba(239,68,68,.10)", fontSize: 11, cursor: "pointer", color: "#FCA5A5", fontFamily: "inherit" } as React.CSSProperties,
...
function tagColor(tag: string): string {
  ...
  if (k === "faction") return FACTION_COLORS[tag] || FACTION_COLORS[FACTION_FROM_SHORT[tag] || "Any"] || "#94A3B8";
...
export default function HelldiversSquadFinder({
  lobbyId, accentColor, currentUserId,
}: {
  lobbyId: string; accentColor?: string; currentUserId?: string;
}) {
  ...
  async function create() {
    ...
    const activityText = (tags.join(" ") + " " + (opName || "Looking for squad")).trim().slice(0, 100);
    ...
    if (j.ok) {
      setShowForm(false);
      setRole(""); setDifficulty(""); setFaction(""); setOpName(""); setDesc("");
      setMaxPlayers(4);
      load();
    } else {
  ...
  return (
    <div style={{ padding: "14px 16px", color: "rgba(243,244,246,.92)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 11, opacity: 0.55, marginTop: 2, letterSpacing: ".5px" }}>
            {loading ? "Loading active operations..." : `${posts.length} active operation${posts.length === 1 ? "" : "s"}`}
          </div>
        </div>
      ...
      {msg && <div style={{ fontSize: 11, marginBottom: 8, opacity: 0.75, color: "#FCA5A5" }}>{msg}</div>}
      ...
      {showForm && (
        <div style={{ ...S.card, marginBottom: 14, border: `1px solid ${accent}45`, background: `${accent}08` }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
            {ROLES.map(r => (
              <button key={r} onClick={() => setRole(role === r ? "" : r)} style={role === r ? S.btnActive : S.btn}>{r}</button>
            ))}
          </div>
          ...
          <input style={{ ...S.input, marginBottom: 12 }} value={desc} onChange={e => setDesc(e.target.value)} placeholder="Mic preferred, casual chill run, samples priority..." />
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <input
                type="number"
                value={maxPlayers}
                onChange={e => setMaxPlayers(Math.max(2, Math.min(4, Number(e.target.value) || 4)))}
                min={2} max={4}
                style={S.input}
              />
            </div>
            <div style={{ flex: 1 }}>
              ...
              <select value={platform} onChange={e => setPlatform(e.target.value)} style={{ ...S.input, cursor: "pointer" }}>
                {["crossplay","pc","psn"].map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
          </div>

          <button style={{ ...S.btnPri, width: "100%", padding: "10px 0" }} onClick={create}>Deploy Squad Beacon</button>
        </div>
      )}
            ...
            <div key={p.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 3, display: "flex", alignItems: "center", gap: 8 }}>
                    {rest || p.activity}
                    {isFull && <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 999, background: "rgba(245,158,11,.15)", border: "1px solid rgba(245,158,11,.35)", color: "rgb(253,230,138)" }}>FULL</span>}
                  </div>
                  {p.description && <div style={{ fontSize: 11, opacity: 0.55, marginBottom: 4 }}>{p.description}</div>}
                  <div style={{ display: "flex", gap: 8, fontSize: 11, opacity: 0.55 }}>
                    <span>{p.platform}</span>
                    <span>·</span>
                    <span>by {p.userName}</span>
                    <span>·</span>
                    <span style={{ color: isFull ? "rgb(253,230,138)" : accent, fontWeight: 700 }}>{slots}</span>
                  </div>
                </div>
                <div>
                  {inSquad ? (
                    <button style={S.btnDanger} onClick={() => leave(p.id)}>{isMine ? "Disband" : "Leave"}</button>
                  ) : isFull ? null : (
