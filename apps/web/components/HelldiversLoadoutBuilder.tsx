...
const ARMORS_LIGHT = [
  ...
  "CM-09 Bonesnapper (Light)",
  "RS-37 Legionnaire (Light)",
  "EX-00 Prototype 16 (Light)",
  "SA-25 Steel Trooper (Light)",
];
const ARMORS_MEDIUM = [
  "B-08 Light Gunner (Medium)",
  "FS-23 Battle Master (Medium)",
  "DP-11 Champion of the People (Medium)",
  "PH-9 Predator (Medium)",
...
const HELMETS = [
  ...
  "SC-30 Trailblazer Helmet",
  "FS-23 Battle Master Helmet",
  "PH-9 Predator Helmet",
  "CE-27 Ground Breaker Helmet",
  "TR-117 Alpha Commander Helmet",
  "CE-67 Titan Helmet",
  "CW-22 Kodiak Helmet",
  "B-27 Fortified Helmet",
  "EX-00 Prototype Helmet",
  "Hero of the Federation Helmet",
];
...
export default function HelldiversLoadoutBuilder({ onSaved, onCancel }: Props) {
  ...
  const factionMeta = useMemo(
    () => FACTIONS.find((f) => f.id === faction) || FACTIONS[0],
    [faction],
  );
  ...
  async function handleSave() {
    setError(null);
    if (!name.trim()) {
      setError("Loadout needs a name, Helldiver.");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={stampHeader(factionMeta.color)}>
          SUPER EARTH // FIELD MANUAL // LOADOUT REGISTRY
        </div>
        ...
        <Field label="Operation Codename">
          <input
            ...
            maxLength={80}
            placeholder="e.g. Hot Rotor Tank"
            style={inputStyle}
          />
        </Field>
        ...
        <Field label="Briefing (optional)">
          <textarea
            ...
            maxLength={1000}
            rows={3}
            placeholder="One paragraph the squad should read before extraction."
        ...
        <Field label="Throwable">
          <select
            value={throwable}
            onChange={(e) => setThrowable(e.target.value)}
            style={inputStyle}
          >
            {THROWABLES.map((w) => (
              <option key={w} value={w}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          ...
          >
            {saving ? "Filing..." : "Submit to Archive"}
          </button>
          {onCancel && (
            <button
              onClick={onCancel}
    ...
    <Field label={label}>
      <select value={value} onChange={(e) => onChange(e.target.value)} style={inputStyle}>
        <optgroup label="Supply">
          {STRATAGEMS_SUPPLY.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
        <optgroup label="Offensive">
          {STRATAGEMS_OFFENSIVE.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
        <optgroup label="Defensive">
          {STRATAGEMS_DEFENSIVE.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
        <optgroup label="Support Weapons">
          {STRATAGEMS_WEAPONS.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
        <optgroup label="Backpack">
          {STRATAGEMS_BACKPACK.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
        <optgroup label="Vehicles">
          {STRATAGEMS_VEHICLES.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </optgroup>
      </select>
...
function DossierPreview(props: {
  name: string;
  description: string;
  faction: { id: string; label: string; color: string };
  role: string;
  difficulty: string;
  primary: string;
  secondary: string;
