...
export default function InviteModal({ type, targetId, targetName, onClose }: Props) {
  ...
  async function sendToUser() {
    ...
    setLoading(false);
    if (j.ok) {
      setMsg(`Invite sent to ${j.sentTo} via DM ✓`);
      setUsername("");
      ...
      <div style={S.modal}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            {targetName && (
              <div style={{ fontSize: 12, opacity: 0.45, marginTop: 2 }}>{targetName}</div>
            )}
          </div>
          <button
            onClick={onClose}
          ...
          {(["link", "send"] as const).map((t) => (
            <button
              ...
              style={{
                ...
                color: tab === t ? "rgb(216,180,254)" : "rgba(148,163,184,.65)",
        ...
        {tab === "link" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {link && (
              <button
                style={{ ...S.btn, fontSize: 12, opacity: 0.6 }}
                onClick={() => {
                  setLink("");
                  setMsg("");
                }}
        ...
        {tab === "send" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ fontSize: 13, opacity: 0.55 }}>
              Search by username and send a direct invite via DM. They'll get a link valid for 7
              days.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <input
        ...
        {msg && (
          <div
            style={{
              ...
              padding: "8px 12px",
              borderRadius: 8,
              background: msg.includes("✓") ? "rgba(16,185,129,.08)" : "rgba(239,68,68,.07)",
              border: `1px solid ${msg.includes("✓") ? "rgba(16,185,129,.20)" : "rgba(239,68,68,.20)"}`,
              color: msg.includes("✓") ? "rgb(110,231,183)" : "rgba(252,165,165,.90)",
            }}
          >
            {msg}
          </div>
