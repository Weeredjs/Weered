...
export default function LobbyPlayingNowPanel({
  ...
  const [invited, setInvited] = React.useState<
    Record<string, "sending" | "sent" | "rate-limited" | "error">
  >({});
  ...
  async function sendInvite(targetUserId: string) {
    ...
    try {
      ...
      if (r.status === 429) {
        setInvited((prev) => ({ ...prev, [targetUserId]: "rate-limited" }));
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      try {
        const r = await fetch(`${API}/steam/playing/${encodeURIComponent(appId)}`, {
          cache: "no-store",
        });
              ...
              >
                <div
                  style={{
                    ...
                    background: p.avatar
                      ? "rgba(255,255,255,.08)"
                      : p.avatarColor || "rgba(255,215,0,.3)",
                    display: "flex",
                    alignItems: "center",
