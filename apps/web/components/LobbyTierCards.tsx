...
export default function LobbyTierCards({ lobbyId }: { lobbyId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (tiersRes.ok) {
      ...
      setRoleNames(tiersRes.roleNames || {});
    }
    if (myRes.ok) {
      setMyTier(myRes.tier);
      setMySub(myRes.sub);
    }
    setLoading(false);
  }, [lobbyId]);
  ...
  async function checkout(tierId: string) {
    setChecking(tierId);
    const j = await apiFetch(`/lobbies/${encodeURIComponent(lobbyId)}/tiers/${tierId}/checkout`, {
      method: "POST",
    });
    if (j.ok && j.url) window.location.href = j.url;
    else {
      weeredToast.error(j.error || "Checkout failed.");
      setChecking(null);
    }
  }

  async function manage() {
    const j = await apiFetch(`/lobbies/${encodeURIComponent(lobbyId)}/tiers/portal`, {
      method: "POST",
    });
    if (j.ok && j.url) window.location.href = j.url;
        ...
        {tiers.map((t) => {
          ...
          return (
            ...
            >
              ...
              {t.description && (
                <div style={{ fontSize: 12, opacity: 0.6, lineHeight: 1.4 }}>{t.description}</div>
              )}
              ...
              {isCurrent ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>
                  <div
                    style={{ fontSize: 11, color: accent, fontWeight: 700, letterSpacing: ".3px" }}
                  >
                    CURRENT TIER
                  </div>
                  {mySub?.cancelAtPeriodEnd && mySub?.currentPeriodEnd && (
                    <div style={{ fontSize: 10, opacity: 0.5 }}>
                      Expires {new Date(mySub.currentPeriodEnd).toLocaleDateString()}
                    </div>
                  )}
                  <button
                    onClick={manage}
