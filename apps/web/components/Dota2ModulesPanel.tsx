...
const S = {
  ...
  btnPri: { padding: "6px 12px", borderRadius: 8, border: "1px solid rgba(194,60,42,.35)", background: "rgba(194,60,42,.12)", fontSize: 12, cursor: "pointer", color: "rgb(194,60,42)", fontWeight: 600 } as React.CSSProperties,
...
function PlayerLookup({ accent, heroMap }: { accent: string; heroMap: Record<number, string> }) {
  ...
  async function selectPlayer(player: any) {
    ...
    } catch { /* silently fail, partial data is fine */ }
              ...
              <div style={{
                ...
                {profile.profile?.avatarfull && (
                  <img
                    ...
                    alt=""
                    style={{ width: 56, height: 56, borderRadius: 12, border: `2px solid ${rankColor(profile.rank_tier)}`, flexShrink: 0 }}
                  />
                )}
                <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {!selected && results.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          {results.map(p => (
            ...
            >
              {p.avatarfull && (
                <img src={p.avatarfull} alt="" style={{
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Sort by average MMR descending, take top 10
        const sorted = data
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {matches.map((m, idx) => {
          ...
          // Fallback: split evenly if team field is missing
...
function HeroesBrowser({ accent }: { accent: string }) {
  ...
  const totalPicksForRate = useMemo(() => {
    // Sum across all ranks for a rough pick count
        ...
        <div style={{ display: "flex", gap: 2, flexShrink: 0 }}>
          {([["name", "A-Z"], ["pick", "Pick%"], ["win", "Win%"]] as [SortKey, string][]).map(([key, label]) => (
            <button key={key} onClick={() => setSortBy(key)} style={{
              padding: "6px 10px", borderRadius: 6, border: "none", fontSize: 10, cursor: "pointer", fontWeight: sortBy === key ? 700 : 400,
