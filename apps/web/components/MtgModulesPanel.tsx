...
async function fetchRandomCard(): Promise<ScryfallLite | null> {
  try {
    const r = await fetch("https://api.scryfall.com/cards/random", {
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, textAlign: "center" }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: "rgba(255,235,200,0.95)" }}>
          {card.name}
        </div>
...
export default function MtgModulesPanel({
  style,
}: {
  lobbyId: string;
  ...
  const search = useCallback(async () => {
    ...
    if (!card) {
      setSearchErr(`No card found for "${q}"`);
      setSearched(null);
    } else {
      setSearched(card);
      setSearchErr(null);
  ...
  return (
    ...
    >
      <Section title="Card of the Day" stripe>
        {featured ? (
          <CardDisplay card={featured} />
        ) : (
          <div style={{ opacity: 0.5, fontSize: 12 }}>Drawing a card…</div>
        )}
        <button
      ...
      <Section title="Quick Card Lookup">
        ...
        <div style={{ marginTop: 8, fontSize: 11, opacity: 0.55 }}>
          ...
          in any chat for inline previews.
        </div>
      </Section>
      ...
      <Section title="Banlist + Reference">
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[
            {
              label: "Wizards B&R Announcements",
