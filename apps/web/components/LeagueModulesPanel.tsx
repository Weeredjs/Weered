...
function TwitchStreams({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {activeStream && (
        <div
          style={{
            borderRadius: 2,
            overflow: "hidden",
            border: `1px solid ${ACCENT_LEAGUE}40`,
            background: "#000",
            marginBottom: 4,
          }}
        >
          <iframe
            title="League of Legends stream"
          ...
          <div
            style={{
              ...
              padding: "8px 12px",
              background: `${ACCENT_LEAGUE}10`,
            }}
          >
            <span style={{ fontSize: 12, fontWeight: 700, color: ACCENT_LEAGUE }}>
              {activeStream}
            </span>
            <button
            ...
            style={{
              ...
              border:
                activeStream === s.userLogin
                  ? `1px solid ${ACCENT_LEAGUE}50`
                  : "1px solid rgba(255,255,255,.08)",
              background:
                activeStream === s.userLogin ? `${ACCENT_LEAGUE}10` : "rgba(255,255,255,.03)",
            }}
          >
            {s.thumbnailUrl && (
              <img
      ...
      {streams.length === 0 && (
        <EmptyState compact title={`Nobody streaming ${gameName} right now.`} />
      )}
      ...
      {showForm && (
        ...
        >
          ...
          <input
            ...
            placeholder="Looking for duo, chill game, tryhard..."
          />
...
function SummonerLookup({ champions, accent }: { champions: any[]; accent: string }) {
  ...
  async function search() {
    if (!query.includes("#")) {
      setError("Enter Riot ID (Name#TAG)");
      return;
    }
    ...
    try {
      const j = await apiFetch(
        `/league/summoner/${encodeURIComponent(gameName)}/${encodeURIComponent(tagLine)}`,
      );
      if (j.ok) setData(j);
      else
          ...
          >
            <div style={{ position: "relative" }}>
              <img
                ...
                alt={data.summoner.name + " summoner icon"}
                style={{ width: 56, height: 56, borderRadius: 2, border: `2px solid ${accent}50` }}
              />
              <div
                style={{
                  position: "absolute",
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 16, color: "rgba(243,244,246,.95)" }}>
                ...
                <span style={{ opacity: 0.4, fontWeight: 400 }}>#{data.summoner.tagLine}</span>
              </div>
...
function RankedCard({
  ...
  return (
    ...
    >
      ...
      <img
        src={tierBadgeUrl(data.tier)}
        alt={data.tier + " rank badge"}
        style={{
          width: 48,
          height: 48,
          margin: "0 auto 4px",
...
function MatchRow({
  ...
  return (
    ...
    >
      <img
        src={champImg}
        alt={m.championName + " champion icon"}
        style={{
          width: 32,
          height: 32,
          borderRadius: 2,
          border: m.win ? "1px solid rgba(34,197,94,.3)" : "1px solid rgba(239,68,68,.3)",
        }}
      />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span
            style={{
              fontWeight: 700,
              fontSize: 12,
              color: m.win ? "rgba(134,239,172,.9)" : "rgba(252,165,165,.9)",
            }}
          >
            {m.win ? "WIN" : "LOSS"}
          </span>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        <div style={{ fontWeight: 800, fontSize: 14, color: TIER_COLORS.CHALLENGER }}>
          Challenger Ladder
        </div>
        ...
        {data.entries?.map((e: any) => (
          <div
            ...
            style={{
              ...
              border:
                e.rank <= 3 ? `1px solid ${TIER_COLORS.CHALLENGER}15` : "1px solid transparent",
            }}
          >
            <span
              style={{
                ...
                fontWeight: 800,
                fontSize: 12,
                color: e.rank <= 3 ? TIER_COLORS.CHALLENGER : "rgba(255,255,255,.5)",
              }}
            >
              {e.rank}
            </span>
            ...
            >
              {e.gameName}
              {e.tagLine ? (
                <span style={{ opacity: 0.35, fontWeight: 400 }}>#{e.tagLine}</span>
              ) : (
                ""
              )}
            </span>
            ...
            <span
              style={{ width: 45, textAlign: "right", fontSize: 11, color: "rgba(134,239,172,.7)" }}
            >
              {e.wins}
            </span>
            <span
              style={{ width: 45, textAlign: "right", fontSize: 11, color: "rgba(252,165,165,.7)" }}
            >
              {e.losses}
            </span>
            <span style={{ width: 45, textAlign: "right", fontSize: 11, opacity: 0.5 }}>
              {e.winRate}%
            </span>
          </div>
        ))}
...
export default function LeagueModulesPanel({
  ...
  useEffect(() => {
    apiFetch("/league/champions")
      .then((j) => {
        if (j.ok) setChampions(j.champions || []);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Weered isn't endorsed by Riot Games and doesn't reflect the views or opinions of Riot
