...
const NPC_PRESETS = [
  ...
  { name: "Merchant", portrait: "💰", personality: "Shrewd but fair. Loves to haggle and gets excited about rare items.", knowledge: "Trade routes, item values, exotic goods from distant lands, regional supply and demand.", secrets: "Smuggles rare magical components. Has a contact in the thieves' guild.", greeting: "Fine wares for fine adventurers! Come, come — let me show you what I've got.", appearance: "A well-dressed halfling with rings on every finger and a cart full of goods." },
  { name: "Guard Captain", portrait: "🛡️", personality: "Duty-bound and tired. Pragmatic with a dry wit. Respects those who help the town.", knowledge: "Local laws, recent crimes, patrol routes, threats, active bounties.", secrets: "Guard force is undermanned. Suspects the mayor of corruption but lacks proof.", greeting: "State your business. We've had enough trouble around here.", appearance: "A weathered half-elf in dented plate armor with a scar across her jaw." },
  { name: "Sage", portrait: "📚", personality: "Brilliant but absent-minded. Goes on tangents about obscure topics.", knowledge: "History, arcane theory, monster lore, ancient civilizations, magical artifacts.", secrets: "Has been researching a dangerous summoning ritual. Knows the true nature of the local curse.", greeting: "Oh! Visitors! I was just reading about the most fascinating— ahem. How can I help?", appearance: "An elderly gnome with ink-stained fingers, wild white hair, and tiny spectacles." },
];

const PORTRAIT_OPTIONS = ["🍺", "🗡️", "💰", "🛡️", "📚", "🧙", "👸", "🐉", "⚔️", "🧝", "🧛", "🎭", "👹", "🦹", "🧌", "🐺", "👁️", "🏴‍☠️", "🦴", "🧞"];
...
const S = {
  ...
  btnPri: { padding: "8px 20px", borderRadius: 10, border: `1px solid ${ACCENT}55`, background: `${ACCENT}22`, color: "rgba(243,244,246,.95)", fontSize: 12, fontWeight: 700 as const, cursor: "pointer" } as React.CSSProperties,
  input: { width: "100%", padding: "8px 12px", borderRadius: 8, border: `1px solid ${ACCENT}22`, background: "rgba(0,0,0,.3)", color: "rgba(243,244,246,.9)", fontSize: 12, outline: "none", fontFamily: "inherit", boxSizing: "border-box" as const } as React.CSSProperties,
...
function speak(text: string, pitch = 1, rate = 0.95) {
  ...
  const preferred = voices.find(v => v.lang.startsWith("en") && (v.name.includes("Google") || v.name.includes("Microsoft") || v.name.includes("Daniel") || v.name.includes("Samantha"))) || voices.find(v => v.lang.startsWith("en")) || voices[0];
...
export default function DndNpcPanel({ roomId }: { roomId: string }) {
  ...
  function openChat(npc: Npc) {
    ...
    setView("chat");
    setMessages([]);
    fetch(`${API}/rooms/${encodeURIComponent(roomId)}/npcs/${npc.id}/messages`, { headers: authHeaders() })
      .then(r => r.json())
      .then(j => {
  ...
  async function sendMessage() {
    ...
    try {
      const res = await fetch(`${API}/rooms/${encodeURIComponent(roomId)}/npcs/${activeNpc.id}/messages`, {
        method: "POST",
  ...
  async function handleSave() {
    ...
    const config: NpcConfig = {
      personality: cPersonality, knowledge: cKnowledge, secrets: cSecrets,
      appearance: cAppearance, greeting: cGreeting,
    ...
    try {
      ...
      const j = await res.json();
      if (j.ok) { loadNpcs(); resetForm(); setView("gallery"); }
    } catch {}
    setCreating(false);
  }

  async function handleDelete(npcId: string) {
    await fetch(`${API}/rooms/${encodeURIComponent(roomId)}/npcs/${npcId}`, { method: "DELETE", headers: authHeaders() }).catch(() => {});
    loadNpcs();
    if (activeNpc?.id === npcId) { setView("gallery"); setActiveNpc(null); }
  }

  function applyPreset(p: typeof NPC_PRESETS[0]) {
    setCName(p.name); setCPortrait(p.portrait); setCPersonality(p.personality);
    setCKnowledge(p.knowledge); setCSecrets(p.secrets); setCAppearance(p.appearance); setCGreeting(p.greeting);
  }

  function startEdit(npc: Npc) {
    setEditId(npc.id); setCName(npc.name); setCPortrait(npc.portrait);
    setCPersonality(npc.config.personality || ""); setCKnowledge(npc.config.knowledge || "");
    setCSecrets(npc.config.secrets || ""); setCAppearance(npc.config.appearance || "");
    setCGreeting(npc.config.greeting || "");
    setCPitch(npc.config.voice?.pitch ?? 1); setCRate(npc.config.voice?.rate ?? 0.95);
    setView("create");
  }

  function resetForm() {
    setEditId(null); setCName(""); setCPortrait("🧙"); setCPersonality("");
    setCKnowledge(""); setCSecrets(""); setCAppearance(""); setCGreeting("");
      ...
      <div style={{ padding: "14px 12px", display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(243,244,246,.85)" }}>
            NPCs {npcs.length > 0 && <span style={{ fontWeight: 400, color: "rgba(148,163,184,.4)", fontSize: 11 }}>({npcs.length})</span>}
          </div>
          <button onClick={() => { resetForm(); setView("create"); }} style={S.btn}>+ Summon NPC</button>
        </div>

        {loadingNpcs ? (
          <div style={{ textAlign: "center", padding: 40, color: "rgba(148,163,184,.4)", fontSize: 12 }}>Loading...</div>
        ) : npcs.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            <div style={{ fontSize: 36, marginBottom: 10, opacity: 0.2 }}>🧙</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,.25)", marginBottom: 6 }}>No NPCs yet</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.15)", marginBottom: 16, lineHeight: 1.5 }}>
              Summon an AI-driven NPC for your party to interact with.<br/>
              They remember conversations and stay in character.
            </div>
            <button onClick={() => { resetForm(); setView("create"); }} style={S.btnPri}>+ Summon NPC</button>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
            {npcs.map(npc => (
              <div
                ...
                style={{ ...S.card, textAlign: "center", cursor: "pointer", transition: "all .18s" }}
                onClick={() => openChat(npc)}
              >
                <div style={{ fontSize: 36, marginBottom: 6 }}>{npc.portrait || "🧙"}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(243,244,246,.9)", marginBottom: 4 }}>{npc.name}</div>
                <div style={{ fontSize: 10, color: "rgba(148,163,184,.4)", marginBottom: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderBottom: `1px solid ${ACCENT}15`, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: "rgba(243,244,246,.9)" }}>{activeNpc.name}</div>
            {activeNpc.config.appearance && (
              <div style={{ fontSize: 10, color: "rgba(148,163,184,.35)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {activeNpc.config.appearance}
              </div>
            )}
          </div>
          <button
            ...
            style={{
              ...S.btn, fontSize: 10, padding: "4px 10px",
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {messages.map(m => {
            ...
            return (
              <div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: isNpc ? "flex-start" : "flex-end", alignSelf: isNpc ? "flex-start" : "flex-end", maxWidth: "85%" }}>
                <div style={{
                  padding: "8px 12px", borderRadius: 12,
                  background: isNpc ? `${ACCENT}12` : "rgba(255,255,255,.06)",
                  border: `1px solid ${isNpc ? `${ACCENT}22` : "rgba(255,255,255,.08)"}`,
                  fontSize: 13, color: "rgba(243,244,246,.88)", lineHeight: 1.55,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2, padding: "0 4px" }}>
                  ...
                  {isNpc && (
                    <button
                      onClick={() => { const v = activeNpc.config?.voice || {}; speak(m.content, v.pitch ?? 1, v.rate ?? 0.95); }}
                      style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, opacity: 0.35, padding: 0 }}
                      title="Speak aloud"
                    >
                      🔊
                    </button>
          ...
          {sending && (
            <div style={{ alignSelf: "flex-start", maxWidth: "85%" }}>
              <div style={{ padding: "10px 18px", borderRadius: 12, background: `${ACCENT}0c`, border: `1px solid ${ACCENT}18`, fontSize: 14, color: `${ACCENT}66` }}>
        ...
        <div style={{ padding: "10px 12px", borderTop: `1px solid ${ACCENT}15`, display: "flex", gap: 8, flexShrink: 0 }}>
          ...
          <button onClick={sendMessage} disabled={sending || !input.trim()} style={{
            ...S.btnPri, opacity: sending || !input.trim() ? 0.4 : 1, flexShrink: 0,
          }}>
            {sending ? "..." : "Send"}
          </button>
        </div>
    ...
    <div style={{ padding: "14px 12px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {!editId && (
        <div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
            {NPC_PRESETS.map(p => (
              <button key={p.name} onClick={() => applyPreset(p)} style={{ ...S.btn, fontSize: 10, padding: "4px 10px" }}>
                {p.portrait} {p.name}
              </button>
            ))}
          </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 4 }}>
          {PORTRAIT_OPTIONS.map(p => (
            <button key={p} onClick={() => setCPortrait(p)} style={{
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-end" }}>
        <div style={{ flex: 1 }}>
          ...
          <input type="range" min="0.5" max="1.8" step="0.1" value={cPitch} onChange={e => setCPitch(Number(e.target.value))} style={{ width: "100%", marginTop: 4, accentColor: ACCENT }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "rgba(148,163,184,.3)" }}>
            <span>Deep</span><span>High</span>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={S.label}>Voice Speed <span style={{ fontWeight: 400, opacity: 0.5 }}>{cRate.toFixed(1)}</span></div>
          <input type="range" min="0.6" max="1.4" step="0.1" value={cRate} onChange={e => setCRate(Number(e.target.value))} style={{ width: "100%", marginTop: 4, accentColor: ACCENT }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "rgba(148,163,184,.3)" }}>
            <span>Slow</span><span>Fast</span>
          </div>
        </div>
        <button
          onClick={() => speak(cGreeting || "Greetings, adventurer. I am but a humble voice.", cPitch, cRate)}
