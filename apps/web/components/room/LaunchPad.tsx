...
export default function LaunchPad({ roomId, moduleType }: { roomId: string; moduleType?: string }) {
  ...
  const isOwner = !!meId && (meId === ownerId || ["GOD","STAFF","SUPPORT"].includes(meGlobalRole));
      ...
      <>
        <div style={barStyle}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={subStyle}>Set a Steam server target and rally the crew. MPlayer-style fire when ready.</div>
            </div>
          </div>
        ...
        {showSetup && (
          <SetupDialog
            ...
            onSubmit={(t) => { send({ type: "launch:set", ...t }); setShowSetup(false); }}
          />
        )}
      </>
    );
        ...
        {isOwner && (
          <>
            ...
            <button
              ...
              style={{
                ...primaryBtnStyle,
                background: playerCount === 0 ? "rgba(255,255,255,.05)" : "linear-gradient(135deg, rgba(239,68,68,.3), rgba(234,88,12,.3))",
              ...
              title={playerCount === 0 ? "At least one player must slot in" : "Launch the countdown"}
            >
            ...
            return (
              <span key={u.id} style={{
                ...
                background: slot === "player"
                  ? (isReady ? "rgba(34,197,94,.15)" : "rgba(124,58,237,.15)")
                  : "rgba(255,255,255,.04)",
                border: `1px solid ${slot === "player"
                  ? (isReady ? "rgba(34,197,94,.4)" : "rgba(124,58,237,.35)")
                  : "rgba(255,255,255,.08)"}`,
                color: slot === "player"
                  ? (isReady ? "#86efac" : "#c4b5fd")
                  : "rgba(203,213,225,.55)",
              }}>
                <span style={{ fontSize: 10 }}>{slot === "player" ? (isReady ? "✓" : "●") : "◎"}</span>
                {u.name || u.id}
              </span>
            );
          })}
...
function CountdownOverlay({
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(252,165,165,.75)", position: "relative" }}>
        {done ? "Go · Go · Go" : "Launching"}
      </div>
      {!done ? (
        <div style={{
          fontSize: 84, fontWeight: 900, lineHeight: 1,
          color: "#fecaca", textShadow: "0 0 40px rgba(239,68,68,.5)",
        ...
        <button
          ...
          style={{
            padding: "18px 44px",
            fontSize: 28, fontWeight: 900, letterSpacing: "4px", textTransform: "uppercase",
            color: "#fff",
        ...
        <div style={{ fontSize: 14, color: "rgba(229,231,235,.85)", fontStyle: "italic", position: "relative" }}>
          Players are boarding the ship. Grab a seat and watch.
        </div>
      )}
      <div style={{ fontSize: 11, color: "rgba(203,213,225,.7)", fontFamily: "ui-monospace, monospace", position: "relative" }}>
      ...
      {isOwner && (
        <button
          ...
          style={{
            marginTop: 4,
            padding: "5px 12px", fontSize: 10, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase",
            background: "rgba(255,255,255,.05)", color: "rgba(252,165,165,.85)",
            border: "1px solid rgba(239,68,68,.3)", borderRadius: 6, cursor: "pointer",
            position: "relative",
          }}
        >Abort</button>
      )}
...
function SetupDialog({
  ...
  useEffect(() => {
    ...
    fetch(`${API}/windrose/public-servers`).then(r => r.json()).then(j => {
  ...
  return (
    <div style={dialogBackdropStyle} onClick={onClose}>
      <div style={dialogStyle} onClick={e => e.stopPropagation()}>
        <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(196,181,253,.85)", marginBottom: 4 }}>
          Launch Target
        </div>
        <div style={{ fontSize: 18, fontWeight: 800, color: "rgba(243,244,246,.95)", marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 10, color: "rgba(148,163,184,.6)", marginTop: 4 }}>
          Paste a <code style={{ fontFamily: "ui-monospace, monospace" }}>steam://connect/...</code> URL and we'll strip it.
        </div>
              ...
              {publicServers.map((s: any) => (
                <button
                  ...
                  style={{
                    ...
                    color: "rgba(229,231,235,.85)", cursor: "pointer", textAlign: "left",
                ...
                >
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{s.name || s.addr}</span>
                  <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 10, color: "rgba(148,163,184,.7)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 18 }}>
          ...
          <button type="button" onClick={submit} disabled={!connect.trim()} style={{
            ...primaryBtnStyle,
...
function PadIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
      ...
      <path d="M12 3v4M12 17v4M3 12h4M17 12h4" stroke="rgba(196,181,253,.55)" strokeWidth="1.5" strokeLinecap="round" />
...
const labelStyle: React.CSSProperties = {
  fontSize: 12, fontWeight: 800, color: "rgba(243,244,246,.92)", letterSpacing: "0.02em",
  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
};
const subStyle: React.CSSProperties = {
  fontSize: 11, color: "rgba(148,163,184,.72)", marginTop: 1,
  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
};
const primaryBtnStyle: React.CSSProperties = {
  padding: "6px 14px", borderRadius: 8,
  background: "rgba(124,58,237,.18)", border: "1px solid rgba(124,58,237,.45)",
  color: "#c4b5fd", fontFamily: "inherit", fontSize: 12, fontWeight: 700,
  cursor: "pointer", letterSpacing: "0.02em", flexShrink: 0, whiteSpace: "nowrap",
};
const secondaryBtnStyle: React.CSSProperties = {
  padding: "6px 10px", borderRadius: 8,
  background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.1)",
  color: "rgba(203,213,225,.75)", fontFamily: "inherit", fontSize: 11, fontWeight: 600,
  cursor: "pointer", letterSpacing: "0.02em", flexShrink: 0, whiteSpace: "nowrap",
};
const dialogBackdropStyle: React.CSSProperties = {
  position: "fixed", inset: 0, zIndex: 9999,
  background: "rgba(5,5,10,.72)", backdropFilter: "blur(6px)",
  display: "flex", alignItems: "center", justifyContent: "center",
};
const dialogStyle: React.CSSProperties = {
  width: "min(480px, calc(100% - 32px))",
  background: "rgba(18,18,26,.98)", border: "1px solid rgba(124,58,237,.35)",
...
const dialogFieldLabel: React.CSSProperties = {
  ...
  color: "rgba(148,163,184,.72)", marginBottom: 5,
};
const dialogInputStyle: React.CSSProperties = {
  width: "100%", padding: "9px 12px", borderRadius: 8,
  background: "rgba(0,0,0,.3)", border: "1px solid rgba(255,255,255,.1)",
  color: "rgba(243,244,246,.95)", fontFamily: "inherit", fontSize: 13,
