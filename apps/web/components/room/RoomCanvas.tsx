...
function YouTubeIcon({
  ...
  return (
    ...
    >
      <path
        d="M154 17.5c-1.82-6.73-7.07-12-13.8-13.8C128.04 0 79.5 0 79.5 0S30.96 0 18.8 3.7C12.07 5.5 6.82 10.77 5 17.5 1.3 29.66 1.3 55 1.3 55s0 25.34 3.7 37.5c1.82 6.73 7.07 12 13.8 13.8C30.96 110 79.5 110 79.5 110s48.54 0 60.7-3.7c6.73-1.82 12-7.07 13.8-13.8 3.7-12.16 3.7-37.5 3.7-37.5s0-25.34-3.7-37.5z"
        fill={color}
      />
...
export default function RoomCanvas({ roomId }: { roomId: string }) {
  ...
  const joinStatus =
    w?.statusByRoom?.[activeRid] ||
    w?.statusByRoom?.["room:" + activeRid] ||
    w?.statusByRoom?.[roomId] ||
    "idle";
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(SOLO_KEY, soloViewing ? "1" : "0");
    } catch {}
  }, [soloViewing, SOLO_KEY]);
  const soloRef = useRef(soloViewing);
  useEffect(() => {
    soloRef.current = soloViewing;
  }, [soloViewing]);
  ...
  const MODULES = useMemo(() => {
    ...
    if (!lobbyContext?.moduleType)
      return ALL_MODULES.filter((m) => DEFAULT_ROOM_MODULES.includes(m.id));
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") toggleChatFullscreen();
    };
  ...
  const roomLabel = useMemo(() => {
    const name = String(
      w?.meta?.name || w?.meta?.title || w?.meta?.label || w?.admin?.name || "",
    ).trim();
    if (name) return name;
    try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const entry =
        typeof cache[roomId] === "object" ? cache[roomId] : { name: cache[roomId] || roomLabel };
      ...
      try {
        window.dispatchEvent(new CustomEvent("weered:roomnames:update"));
      } catch {}
    } catch {}
  ...
  const handleModuleClick = (id: NonNullable<StageMode>) => {
    ...
    if (newMode) {
      ...
      const isStaff = ["GOD", "STAFF", "SUPPORT", "ADMIN"].includes(
        String(w?.globalRole || w?.me?.globalRole || "USER").toUpperCase(),
      );
  ...
  const isFullStageMode =
    stageActive && !["youtube", "browser", "twitch", "article", "chess"].includes(stageMode!);
  ...
  const isRoomMod =
    !!(w?.meta?.mods && Array.isArray(w.meta.mods) && w.meta.mods.includes(meId)) || isStaffGlobal;

  return (
    <div
      className="flex flex-col min-w-0"
      style={{ position: "relative", overflow: "hidden", height: "calc(100vh - 32px)" }}
    >
      {joinStatus === "knocking" && (
        <div
          style={{
        ...
        >
          <div
            style={{
              ...
              justifyContent: "center",
              fontSize: 24,
              animation: "weered-pulse 2s ease-in-out infinite",
            }}
          >
            🚪
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "rgba(148,163,184,0.55)", marginTop: 1 }}>
                Join to hear and speak with others
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 6, marginLeft: 4 }}>
            {voice.tiles.slice(0, 5).map((t) => (
              <span
                ...
                style={{
                  fontSize: 11,
                  color: t.isMuted ? "rgba(148,163,184,0.5)" : "rgba(134,239,172,0.9)",
                }}
              >
                {t.name}
                {t.isMuted ? " 🔇" : ""}
          ...
          stageMode !== "article" && (
            <div style={{ height: "100%" }}>
              <RoomStage
                roomId={roomId}
                mode={stageMode}
                moduleType={lobbyContext?.moduleType}
                roomUsers={(() => {
            ...
            {(() => {
              ...
              if (isBlocked)
                return (
                  ...
                  >
                    <div style={{ fontSize: 40 }}>🚫</div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(226,232,240,0.7)" }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(100,116,139,0.4)", marginTop: 4 }}>
                      The discussion room is still active -- others can join and chat about this
                      article
                    </div>
                  </div>
                );
              const embedUrl = toEmbedUrl(browserUrl);
              if (embedUrl === "__youtube_blocked__")
                return (
                  <div
                    style={{
                      flex: 1,
                  ...
                  >
                    ...
                    >
                      Use the YouTube tab above to sync playback for everyone
                    </div>
                    <button
                      onClick={() => {
                        handleModuleClick("youtube" as any);
            ...
            {twitchChannel ? (
              ...
              >
                ...
                <div style={{ fontSize: 13, opacity: 0.4 }}>
                  Enter a Twitch channel name above to start watching
                </div>
                <div style={{ fontSize: 11, opacity: 0.25 }}>
                  You can paste a full twitch.tv URL or just the channel name
                </div>
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  background: chessSource.includes("/tv/")
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: "1px solid rgba(124,58,237,0.25)",
                  background: chessSource.includes("/training/")
                    ? "rgba(124,58,237,0.18)"
                    : "rgba(124,58,237,0.06)",
                  color: "rgba(216,180,254,0.9)",
                  fontSize: 11,
              ...
              {chessSource && (
                <a
                  href={chessSource.replace("/frame", "").replace("/embed/", "/").split("?")[0]}
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{
                    padding: "4px 8px",
            ...
            {chessSource ? (
              ...
              >
                ...
                <div style={{ fontSize: 13, opacity: 0.55, maxWidth: 360 }}>
                  Pick a feed above — Lichess TV (live featured game), Daily Puzzle, or paste any
                  Lichess URL (game, study, broadcast).
                </div>
                <div style={{ fontSize: 11, opacity: 0.3 }}>
                  Chess.com doesn&apos;t allow embedding. Lichess only.
                </div>
              </div>
            )}
          </div>
      ...
      <div
        className={isFullStageMode ? "flex" : stageActive ? "flex" : "flex flex-1 min-h-0"}
        style={{
          position: "relative",
          overflow: "visible",
          ...(isFullStageMode
            ...
            : stageActive
              ? { flexShrink: 0, height: 0 }
              : {}),
        }}
      >
        <div
          className={isFullStageMode ? "" : "flex flex-col flex-1 min-w-0 min-h-0"}
          style={{ position: "relative" }}
        >
          {!isFullStageMode && <div className="flex-1 min-h-0" />}
        ...
        <div
          ...
          style={{
            ...
            transition:
              "right 0.34s cubic-bezier(0.22,1,0.36,1), background 0.2s ease, color 0.2s ease",
            zIndex: 100,
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
        ...
        >
          ...
          <span style={{ writingMode: "horizontal-tb", fontSize: 10, lineHeight: 1 }}>
            {chatOpen ? "›" : "‹"}
          </span>
          <span>CHAT</span>
        </div>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <button
                    ...
                    title={
                      chatFullscreen ? "Collapse chat (Esc)" : "Go fullscreen — room to breathe"
                    }
              ...
              >
                {isTwitch ? (
                  ...
                  <YouTubeIcon
                    size={15}
                    color={
                      isActive ? "#FF0000" : isLive ? "rgba(255,0,0,0.6)" : "rgba(255,255,255,0.18)"
                    }
                  />
                ) : (
                  <span style={{ fontSize: 12, lineHeight: 1 }}>{m.icon}</span>
...
function RoomMemberToolbar({
  ...
  return (
    <div
      className="flex items-center gap-2 px-4 py-1.5 border-b border-white/[0.05]"
      style={{ background: "rgba(8,8,14,0.4)" }}
    >
      <button
        type="button"
