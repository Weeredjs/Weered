...
const ALL_MODULES: { id: NonNullable<StageMode>; label: string; icon: string; live: boolean }[] = [
  ...
  { id: "hllv",    icon: "🚁", label: "Vietnam",   live: true  },
...
export default function RoomCanvas({ roomId }: { roomId: string }) {
  ...
  return (
    <div className="flex flex-col min-w-0" style={{ position: "relative", overflow: "hidden", height: "calc(100vh - 32px)" }}>
      ...
      <div style={{ flexShrink: 0, borderTop: "1px solid rgba(255,255,255,0.07)", padding: "10px 16px 8px", background: "transparent", zIndex: 20 }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
          {MODULES.map((m) => {
            const isActive = m.id === "office" ? officeStage : stageMode === m.id;
            const isLive = m.live;
      ...
      {voice.connState === "connected" && voice.activeRoomId && voice.activeRoomId !== roomId && (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(226,232,240,0.8)", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{voice.activeRoomId === roomId ? roomLabel : voice.activeRoomId}</div>
          </div>
