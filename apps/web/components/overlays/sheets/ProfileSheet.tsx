...
const AVATAR_STYLES = [
  {
    ...
    label: "Thumbs",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/thumbs/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "bottts",
    label: "Bots",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/bottts/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "pixel-art",
    label: "Pixel",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/pixel-art/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "identicon",
    label: "Identity",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/identicon/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "shapes",
    label: "Shapes",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/shapes/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "rings",
    label: "Rings",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/rings/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "glass",
    label: "Glass",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/glass/svg?seed=${s}&backgroundColor=transparent`,
  },
  {
    id: "fun-emoji",
    label: "Emoji",
    url: (s: string) =>
      `https://api.dicebear.com/9.x/fun-emoji/svg?seed=${s}&backgroundColor=transparent`,
  },
];
        ...
        {AVATAR_STYLES.map((s) => (
          <button
            ...
            style={{
              ...
              background:
                s.id === selectedStyle ? "rgba(167,139,250,.15)" : "rgba(255,255,255,.04)",
...
function FriendButton({ profile, apiBase }: { profile: any; apiBase: string }) {
  ...
  React.useEffect(() => {
    setStatus(String(profile?.friendStatus || "none"));
    ...
    return (
      <button
        ...
        onClick={async () => {
          ...
          setBusy(true);
          try {
            await fetch(`${apiBase}/friends/accept/${profile.friendRequestId}`, {
              method: "POST",
              headers: hdr(),
            });
            setStatus("friends");
  ...
  return (
    <button
      ...
      onClick={async () => {
        setBusy(true);
        try {
          const r = await fetch(`${apiBase}/friends/request/${profile.id}`, {
            method: "POST",
            headers: hdr(),
          }).then((x) => x.json());
...
export default function ProfileSheet({ userId }: { userId: string }) {
  ...
  const changeTheme = (name: WeeredThemeName) => {
    setTheme(name);
    try {
      localStorage.setItem(WEERED_THEME_KEY, name);
    } catch {}
    try {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/profile/${resolvedId}`, {
      ...
      .then((j) => {
        ...
        if (j.avatarColor) {
          try {
            localStorage.setItem("weered:avatarColor", j.avatarColor);
          } catch {}
        }
      })
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/users/${resolvedId}/reliability`)
      .then((r) => r.json())
      .then((d) => {
        if (d?.ok && (d.completed ?? 0) > 0)
          setReliability({ completed: d.completed, tier: d.tier });
      })
      .catch(() => {});
  }, [resolvedId, apiBase]);
  ...
  const saveColor = useCallback(
    async (color: string) => {
      ...
      try {
        localStorage.setItem("weered:avatarColor", color);
      } catch {}
      try {
        window.dispatchEvent(new CustomEvent("weered:avatarColor", { detail: color }));
      } catch {}
      if (!token) return;
      setSavingColor(true);
      try {
  ...
  const saveFrame = useCallback(
    async (key: string) => {
      setProfile((prev) =>
        prev ? ({ ...prev, avatarFrame: key === "none" ? null : key } as any) : prev,
      );
      if (!token) return;
      try {
        await fetch(`${apiBase}/profile/me`, {
  ...
  const saveNameEffect = useCallback(
    async (key: string) => {
      setProfile((prev) =>
        prev ? ({ ...prev, nameEffect: key === "none" ? null : key } as any) : prev,
      );
      if (!token) return;
      try {
        await fetch(`${apiBase}/profile/me`, {
  ...
  async function toggleBlock() {
    ...
    if (!target) {
      ...
      const ok = await weeredConfirm({
        title: `Block ${profile?.name || userId}?`,
        body: "They won't be able to DM you. You won't see their messages. You can unblock in Settings.",
        confirmLabel: "Block",
        destructive: true,
      });
      if (!ok) return;
    ...
    try {
      ...
      weeredToast.error("Action failed.");
    } finally {
      setBlockBusy(false);
    }
  }
    ...
    <div style={wrap}>
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(180deg, transparent 42%, rgba(0,0,0,0.18) 62%, rgba(0,0,0,0.5) 100%)",
          }}
        />
        <div
          style={{
      ...
      >
        <div
          ...
          className={
            (profile as any).avatarFrame
              ? "weered-frame-" + (profile as any).avatarFrame
              : undefined
          }
          style={{
            width: 96,
        ...
        <div style={{ flex: 1, minWidth: 0, paddingBottom: 2 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            <span
              className={
                (profile as any).nameEffect
                  ? "weered-name-" + (profile as any).nameEffect
                  : undefined
              }
              style={{ fontSize: 25, fontWeight: 900, letterSpacing: "-0.4px" }}
            >
              {profile.name}
            </span>
            <span
      ...
      <div
        style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 2, marginBottom: 16 }}
      >
        {((profile as any).statusText || (profile as any).statusEmoji) && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13 }}>
            {(profile as any).statusEmoji && (
              <span style={{ fontSize: 15 }}>{(profile as any).statusEmoji}</span>
            )}
            {(profile as any).statusText && (
              <span
                style={{ fontStyle: "italic", color: "rgba(243,244,246,.85)", paddingRight: 3 }}
              >
                {(profile as any).statusText}
              </span>
            )}
        ...
        {(profile as any).primaryCrew?.name && (
          <a
            href={`/crew/${encodeURIComponent((profile as any).primaryCrew.id)}`}
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 7,
          ...
          >
            <span
              style={{
                ...
                fontWeight: 900,
                letterSpacing: "1px",
                color: (profile as any).primaryCrew.accentColor || "#a78bfa",
                border: `1px solid ${(profile as any).primaryCrew.accentColor || "#a78bfa"}55`,
                padding: "1px 7px",
              }}
            >
              [{(profile as any).primaryCrew.tag}]
            </span>
            <span
              style={{
                fontSize: 13,
        ...
        <div style={{ fontSize: 11, opacity: 0.4, fontFamily: "monospace" }}>
          ...
          {lastSeen && (
            <span
              style={{
                marginLeft: 10,
                color: lastSeen === "Active now" ? "#22c55e" : "inherit",
                opacity: lastSeen === "Active now" ? 1 : 0.7,
              }}
            >
              {lastSeen}
            </span>
        ...
        <div style={section}>
          ...
          >
            <div style={sectionLabel}>
              Customize Avatar{" "}
              {(savingColor || savingAvatar) && (
                <span style={{ opacity: 0.4, fontWeight: 400 }}>saving...</span>
              )}
            </div>
            {hasAvatar && (
          ...
          {activeTab === "gta" ? (
            <GtaGallery currentAvatar={avatarUrl} onSelect={saveAvatar} />
          ) : activeTab === "gallery" ? (
            <AvatarGallery
              ...
              >
                {uploading ? (
                  <>
                    ...
                    <div style={{ fontSize: 12, color: "rgba(212,160,23,.7)", fontWeight: 700 }}>
                      Uploading...
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div style={{ fontSize: 10, color: "rgba(148,163,184,.4)" }}>
                      PNG, JPEG, WebP, or GIF — max 2MB
                    </div>
                    <div style={{ fontSize: 10, color: "rgba(148,163,184,.3)" }}>
                      Auto-resized to 256×256
                    </div>
                  </>
                )}
              </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              ...
              <div
                style={{ fontSize: 11, fontWeight: 700, color: "#D4A017", fontFamily: "monospace" }}
              >
            ...
            {rank.next > rank.prev && (
              ...
              >
                {(rank.next - profile.notoriety).toLocaleString()} XP to{" "}
                {notorietyRank(rank.next).name}
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div
              style={{
                ...
                justifyContent: "center",
                fontSize: 20,
                color: relColor(reliability.tier),
              }}
            >
              ★
            </div>
      ...
      <div
        style={{ ...section, background: "transparent", border: "none", padding: 0, marginTop: 10 }}
      >
        ...
        <div style={{ ...section, marginTop: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 8 }}>
            {["none", "gold", "flames", "crystal", "neon", "circuit"].map((key) => (
              ...
              >
                <div
                  className={key === "none" ? undefined : "weered-frame-" + key}
                  style={{
                    width: 30,
                    height: 30,
                    borderRadius: "50%",
                    background: aColor,
                    border: key === "none" ? "1px dashed rgba(255,255,255,.3)" : "none",
                    outline:
                      ((profile as any).avatarFrame || "none") === key ? "2px solid #fff" : "none",
                    outlineOffset: 3,
                  }}
                />
                <span
            ...
            {["none", "gold", "fire", "ice", "toxic", "royal", "rainbow"].map((key) => (
              <button
                ...
                style={{
                  ...
                  gap: 3,
                  outline:
                    ((profile as any).nameEffect || "none") === key
              ...
              >
                <span
                  className={key === "none" ? undefined : "weered-name-" + key}
                  style={{
                    fontSize: 14,
                    fontWeight: 900,
                    color: key === "none" ? "rgba(255,255,255,.5)" : undefined,
                  }}
                >
                  Name
                </span>
      ...
      {profile?.id && (
        <div style={{ marginTop: 14 }}>
          <SteamAchievementsPanel
            appId="553850"
            userId={profile.id}
            gameDisplayName="Helldivers 2"
            accentColor="#FFD700"
          />
          <SteamAchievementsPanel
            appId="3041230"
            userId={profile.id}
            gameDisplayName="Windrose"
            accentColor="#b8935a"
          />
          <SteamAchievementsPanel
            appId="1085660"
        ...
        <div style={{ ...section, marginTop: 10 }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button
              onClick={openDM}
              disabled={isBlocked}
              title={isBlocked ? "Unblock to message" : undefined}
              style={{
                ...actionButton,
                background: "var(--weered-accent-bg, rgba(167,139,250,.1))",
                borderColor: "var(--weered-accent-ring, rgba(167,139,250,.25))",
            ...
            <button
              ...
              style={{
                ...
                borderColor: isBlocked
                  ? "rgba(239,68,68,.45)"
                  : "var(--weered-border, rgba(148,163,184,.22))",
                color: isBlocked
                  ? "rgba(252,165,165,.95)"
                  : "var(--weered-muted, rgba(148,163,184,.75))",
              }}
            >
              <span style={{ fontSize: 14 }}>{isBlocked ? "✓" : "🚫"}</span>{" "}
