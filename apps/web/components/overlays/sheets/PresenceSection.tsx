...
export default function PresenceSection() {
  ...
  const [saving, setSaving] = React.useState<"" | "steam" | "twitch" | "xbox" | "psn" | "lichess" | "chesscom" | "overlay">("");
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${apiBase}/me/overlay`, { headers: { Authorization: `Bearer ${token()}` } });
        const j = await r.json();
  ...
  async function generateOverlayToken() {
    ...
    try {
      ...
      setMsg({ ok: true, text: wasActive ? "Overlay URL rotated — old link revoked." : "Overlay URL generated." });
    } catch (e: any) {
  ...
  return (
    <Section title="Rich Presence">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "8px 0", marginTop: 8, borderTop: "1px solid rgba(148,163,184,.12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: "var(--weered-text, rgba(243,244,246,.95))" }}>Streamer overlay (OBS / Twitch)</span>
          {overlayToken && (
            <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "rgba(217,169,66,.16)", border: "1px solid rgba(217,169,66,.40)", color: "rgba(252,211,77,.95)", letterSpacing: ".04em", fontWeight: 700 }}>ACTIVE</span>
          )}
        </div>
        <div style={{ fontSize: 11, opacity: 0.6, color: "var(--weered-muted, rgba(148,163,184,.75))", lineHeight: 1.4 }}>
          Drop the URL into OBS as a Browser Source (suggested size ~440×180). It renders your live Weered lobby + room + people-here count + join link, updating every 5 seconds. Anyone with the URL can render it — rotate to revoke if someone shares it around.
        </div>
        {overlayToken ? (
          <>
            <div style={{ display: "flex", gap: 6, marginTop: 4, alignItems: "center" }}>
              <input
                ...
                style={{ ...stackedInputStyle, fontFamily: "ui-monospace, monospace", fontSize: 11 }}
              />
              <button type="button" style={{ ...btnStyle, padding: "8px 14px", fontSize: 12 }} onClick={copyOverlayUrl}>
                {overlayCopied ? "Copied" : "Copy"}
              </button>
            </div>
            <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
              <a href={`/overlay/${overlayToken}`} target="_blank" rel="noopener noreferrer" style={{ ...btnStyle, padding: "8px 14px", fontSize: 12, textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
                Preview
              </a>
              <button type="button" style={{ ...btnStyle, padding: "8px 14px", fontSize: 12 }} onClick={generateOverlayToken} disabled={saving === "overlay"}>
                {saving === "overlay" ? "…" : "Rotate"}
              </button>
              <button type="button" style={{ ...btnStyle, padding: "8px 12px", fontSize: 12, opacity: 0.7 }} onClick={disableOverlay} disabled={saving === "overlay"}>
                Disable
              </button>
            </div>
          </>
