    ...
    <div style={{ border: "1px solid #e5e5e5", borderRadius: 12, padding: 12, marginTop: 10 }}>
        ...
        {(admin?.knocks?.length || 0) === 0 ? (
          ...
          <div style={{ display: "grid", gap: 6 }}>
            {admin!.knocks!.slice().reverse().map((k) => (
              <div
                key={k.userId}
                style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: 8, border: "1px solid #eee", borderRadius: 10 }}
              >
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => w.admit(k.userId)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Admit</button>
                  <button onClick={() => w.deny(k.userId)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Deny</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {(users.length || 0) === 0 ? (
          ...
          <div style={{ display: "grid", gap: 6 }}>
            {users.map((u) => {
              ...
              return (
                <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: 8, border: "1px solid #eee", borderRadius: 10 }}>
                  <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
                    {canPromote && (
                      <button onClick={() => w.promote(u.id)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Promote</button>
                    )}
                    {canDemote && (
                      <button onClick={() => w.demote(u.id)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Demote</button>
                    )}
                    {canKick && (
                      <button onClick={() => w.kick(u.id)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Kick</button>
                    )}
                    {canBan && (
                      <button onClick={() => w.ban(u.id)} style={{ padding: "6px 10px", borderRadius: 10, border: "1px solid #ddd" }}>Ban</button>
                    )}
                  </div>
                </div>
              );
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input
            ...
            style={{ padding: "8px 10px", borderRadius: 10, border: "1px solid #ddd", minWidth: 240 }}
          />
          <button
            onClick={() => {
      ...
      <div style={{ marginTop: 12 }}>
        ...
        {(admin?.audit?.length || 0) === 0 ? (
          ...
          <div style={{ maxHeight: 220, overflow: "auto", border: "1px solid #eee", borderRadius: 10, padding: 8 }}>
