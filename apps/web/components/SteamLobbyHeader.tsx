...
export default function SteamLobbyHeader({
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      try {
        const r = await fetch(`${API}/steam/players/${encodeURIComponent(appId)}`, { cache: "no-store" });
        if (!r.ok) throw new Error(String(r.status));
        const j = await r.json();
        if (!alive) return;
        if (j?.ok && typeof j.count === "number") { setCount(j.count); setErrored(false); }
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const j = await r.json();
        if (!alive) return;
        if (j?.ok && j.linked && j.owned) setOwned({ owned: true, hoursPlayed: Number(j.hoursPlayed || 0), lastPlayed: j.lastPlayed || null });
      } catch {}
    })();
    return () => { alive = false; };
  }, [appId]);
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
      <a
        ...
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = `linear-gradient(135deg, ${accent}40, ${accent}1a)`; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = `linear-gradient(135deg, ${accent}28, ${accent}12)`; }}
      >
...
function SteamGlyph({ color }: { color: string }) {
  return (
    <svg width="11" height="11" viewBox="0 0 16 16" fill="none" aria-hidden>
      ...
      <path d="M3.5 9.2 L6.4 10.4 M6.4 10.4 L8.6 7.5" stroke={color} strokeWidth="1.2" strokeLinecap="round" />
    </svg>
  );
}
