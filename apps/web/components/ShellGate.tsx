...
function OverlayPanel({
  ...
  useEffect(() => {
    ...
    const onMove = (e: TouchEvent) => {
      ...
      if (Math.abs(moveY) > Math.abs(moveX) && dx === 0) {
        start = null;
        return;
      }
    ...
    const tick = async () => {
      try {
        ...
        const total = (j?.threads || []).reduce(
          (s: number, t: any) => s + (Number(t?.unread) || 0),
          0,
        );
          ...
          <div className="weered-rail-stack">
              ...
              onClick={() => {
                try {
                  window.dispatchEvent(
                    new CustomEvent("weered:dock:open", { detail: { tab: "crew" } }),
                  );
                } catch {}
              }}
            />
            <RailQuickButton
              ...
              onClick={() => {
                try {
                  window.dispatchEvent(
                    new CustomEvent("weered:dock:open", { detail: { tab: "friends" } }),
                  );
                } catch {}
              }}
            />
            <RailQuickButton
              ...
              onClick={() => {
                try {
                  window.dispatchEvent(
                    new CustomEvent("weered:dock:open", { detail: { tab: "dms" } }),
                  );
                } catch {}
              }}
            />
            ...
            >
              ...
              >
                ...
                <path d="M15 3v18" />
                <polyline points="10 8 7 12 10 16" />
              </svg>
...
const QUICK_PALETTE: Record<QuickKind, { bg: string; border: string; glow: string; fg: string }> = {
  crew: {
    bg: "linear-gradient(180deg, rgba(217,169,66,0.25) 0%, rgba(138,107,62,0.32) 100%)",
    border: "rgba(217,169,66,0.55)",
    glow: "rgba(217,169,66,0.18)",
    fg: "#f3d48a",
  },
  friends: {
    bg: "linear-gradient(180deg, rgba(52,211,153,0.22) 0%, rgba(16,122,88,0.30) 100%)",
    border: "rgba(52,211,153,0.50)",
    glow: "rgba(52,211,153,0.16)",
    fg: "#a7f3d0",
  },
  dms: {
    bg: "linear-gradient(180deg, rgba(167,139,250,0.26) 0%, rgba(124,58,237,0.32) 100%)",
