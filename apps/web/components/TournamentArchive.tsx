...
export default function TournamentArchive({ lobbyId }: { lobbyId: string }) {
  ...
  React.useEffect(() => {
    ...
    fetch(`${API}/tournaments/archive?lobbyId=${encodeURIComponent(lobbyId)}&limit=10`)
      .then(r => r.json())
      .then(j => { if (alive && j?.ok) setArchive(j.archive || []); })
      .catch(() => {})
  ...
  return (
    <div style={{ marginTop: 18 }}>
      <div style={{
        ...
        color: "rgba(245,130,32,.7)", textTransform: "uppercase",
          ...
          return (
            <div key={t.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", background: "rgba(255,255,255,.05)", borderRadius: 999, color: "rgba(255,255,255,.55)", letterSpacing: 0.4 }}>
                  {t.format.replace("_", " ")}
                </span>
...
function PodiumChip({ rank, name, color, emoji }: { rank: number; name: string; color: string; emoji: string }) {
  return (
    <span style={{
