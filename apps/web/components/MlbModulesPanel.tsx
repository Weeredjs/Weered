...
function Scoreboard({ accentColor }: { accentColor: string }) {
  ...
  function toggleExpand(gameId: string) {
    ...
    apiFetch(`/mlb/game/${gameId}/boxscore`)
      .then((j) => {
        if (j.ok !== false) setBoxscore(j.game || j);
        setBoxLoading(false);
      })
  ...
  function statusBadge(g: any) {
    ...
    return (
      <span style={{ fontSize: 10, opacity: 0.5 }}>
        {g.startTime ? fmtTime(g.startTime) : "TBD"}
      </span>
    );
  }
  ...
  const isUpcoming = (g: any) =>
    !isLive(g) && !isFinal(g) && !(g.status || "").toLowerCase().includes("postpone");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14 }}>
        <button
          onClick={() => shiftDate(-1)}
          style={{ ...S.btn, padding: "4px 10px", fontSize: 16, lineHeight: 1 }}
          title="Previous day"
        >
          &larr;
        </button>
      ...
      {error && (
        <div style={{ fontSize: 12, color: "rgba(252,165,165,.8)", textAlign: "center" }}>
          {error}
        </div>
      )}
          ...
          return (
            <div key={gid}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      {g.awayRecord && (
                        <span style={{ fontSize: 10, opacity: 0.35 }}>({g.awayRecord})</span>
                      )}
                    </div>
                    <span
                      style={{
                        ...
                        color: !isUpcoming(g) ? "#fff" : "rgba(255,255,255,.2)",
                      }}
                    >
                  ...
                  >
                    ...
                    >
                      ...
                      {g.homeRecord && (
                        <span style={{ fontSize: 10, opacity: 0.35 }}>({g.homeRecord})</span>
                      )}
                    </div>
                    <span
                      style={{
              ...
              {isExp && (
                ...
                >
                  {boxLoading && (
                    <div style={{ textAlign: "center", padding: 12, opacity: 0.4, fontSize: 12 }}>
                      Loading boxscore...
                    </div>
                  )}
                  {!boxLoading && boxscore && (
                    <div style={{ overflowX: "auto" }}>
                          ...
                          <tbody>
                            {["away", "home"].map((side) => (
                              ...
                              >
                                ...
                                >
                                  {boxscore.linescore[side]?.runs ??
                                    (side === "away" ? g.awayScore : g.homeScore) ??
                                    "-"}
                                </td>
                                <td
                                  style={{
                      ...
                      {(boxscore.winningPitcher ||
                        boxscore.losingPitcher ||
                        boxscore.savePitcher) && (
                        <div
                          style={{
                            display: "flex",
                            gap: 12,
                        ...
                        >
                          {boxscore.winningPitcher && (
                            <span>
                              <strong style={{ color: "#4ade80" }}>W:</strong>{" "}
                              {boxscore.winningPitcher}
                            </span>
                          )}
                          {boxscore.losingPitcher && (
                            <span>
                              <strong style={{ color: "#f87171" }}>L:</strong>{" "}
                              {boxscore.losingPitcher}
                            </span>
                          )}
                          {boxscore.savePitcher && (
                            <span>
                              <strong style={{ color: "#60a5fa" }}>SV:</strong>{" "}
                              {boxscore.savePitcher}
                            </span>
                          )}
                        </div>
                      ...
                      {!boxscore.linescore && (
                        <div
                          style={{ textAlign: "center", padding: 8, opacity: 0.35, fontSize: 12 }}
                        >
                          Boxscore data not yet available
                        </div>
                      )}
                    </div>
                  )}
                  {!boxLoading && !boxscore && <EmptyState compact title="No boxscore data." />}
                </div>
              )}
            </div>
          );
...
function Standings({ accentColor }: { accentColor: string }) {
  ...
  if (error)
    return (
      <div
        style={{ padding: 20, textAlign: "center", fontSize: 12, color: "rgba(252,165,165,.8)" }}
      >
        {error}
      </div>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              ...
              <tbody>
                {(div.teams || []).map((t: any, i: number) => {
                  ...
                  const diffColor =
                    diff > 0 ? "#4ade80" : diff < 0 ? "#f87171" : "rgba(255,255,255,.5)";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ textAlign: "center", padding: "6px 4px", opacity: 0.6 }}>
                        {t.gamesBack ?? t.gb ?? "-"}
                      </td>
                      <td style={{ textAlign: "center", padding: "6px 4px", opacity: 0.6 }}>
                        {t.streak ?? t.strk ?? "-"}
      ...
      {divisions.length === 0 && <EmptyState title="No standings data." />}
    </div>
  );
}
...
function Leaders({
  ...
  accentColor: string;
  onPlayerClick: (id: string | number, name: string) => void;
}) {
  const accent = accentColor || MLB_RED;
  ...
  const getCat = (name: string) =>
    categories.find((c: any) => (c.name || c.stat || "").toUpperCase() === name.toUpperCase());
  ...
  function LeaderCard({ statName }: { statName: string }) {
    ...
    return (
      <div style={{ ...S.card, padding: "8px 10px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {leaders.slice(0, 10).map((p: any, i: number) => (
            <div
              ...
              onClick={() =>
                (p.id || p.playerId) &&
                onPlayerClick(p.id || p.playerId, p.name || p.playerName || "")
              }
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              <span style={{ fontSize: 9, opacity: 0.35, flexShrink: 0 }}>
                {p.team || p.teamAbbrev || ""}
              </span>
              <span
                style={{
                  ...
                  minWidth: 36,
                  textAlign: "right",
                  color: i === 0 ? accent : "rgba(243,244,246,.88)",
                }}
              >
                {p.value ?? p.stat ?? "-"}
              </span>
        ...
        {leaders.length === 0 && (
          <div style={{ fontSize: 11, opacity: 0.35, textAlign: "center", padding: 8 }}>
            No data
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          {BATTING_CATS.map((cat) => (
            <LeaderCard key={cat} statName={cat} />
          ))}
        </div>
      </div>
      ...
      {categories.length === 0 && <EmptyState title="No leaderboard data." />}
    </div>
  );
}
          ...
          <>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div
                  style={{ display: "flex", flexWrap: "wrap", gap: 6, fontSize: 11, opacity: 0.6 }}
                >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            ...
                            {[
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  padding: "5px 5px",
                                  fontVariantNumeric: "tabular-nums",
                                  fontWeight: ["avg", "obp", "slg", "ops"].includes(key)
                                    ? 700
                                    : 400,
                                }}
                              >
                            ...
                            {[
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  padding: "5px 5px",
                                  fontVariantNumeric: "tabular-nums",
                                  fontWeight: ["era", "whip"].includes(key) ? 700 : 400,
                                }}
                              >
                  ...
                  <div>
                    ...
                    <div style={{ ...S.card, overflowX: "auto", padding: "6px 8px" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                        <thead>
                          <tr style={{ borderBottom: "1px solid rgba(255,255,255,.08)" }}>
                            {["POS", "G", "GS", "INN", "TC", "PO", "A", "E", "DP", "FPCT"].map(
                              (col) => (
                                <th
                                  key={col}
                                  style={{
                ...
                {!playerStats.batting && !playerStats.pitching && !playerStats.fielding && (
                  <div style={{ ...S.card, padding: 0 }}>
                    <EmptyState compact title="No stats for this player." />
                  </div>
                )}
              </div>
            )}
...
function MlbTwitchStreams({ lobbyId, accentColor }: { lobbyId?: string; accentColor?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {activeStream && (
        <div
          style={{
            borderRadius: 2,
            overflow: "hidden",
            border: `1px solid ${accentColor || MLB_RED}40`,
            background: "#000",
            marginBottom: 4,
          }}
        >
          ...
          <div
            style={{
              ...
              justifyContent: "space-between",
              padding: "8px 12px",
              background: `${accentColor || MLB_RED}10`,
            }}
          >
        ...
        {streams.map((s) => (
          <div
            ...
            style={{
              ...
              transition: "border-color .15s, background .15s",
              border:
                activeStream === (s.userLogin || s.user_login)
                  ? `1px solid ${accentColor || MLB_RED}50`
                  : "1px solid rgba(255,255,255,.08)",
              background:
                activeStream === (s.userLogin || s.user_login)
                  ? `${accentColor || MLB_RED}10`
                  : "rgba(255,255,255,.03)",
            }}
          >
            {(s.thumbnailUrl || s.thumbnail_url) && (
              <img
                ...
                alt={(s.userName || s.user_name) + " stream thumbnail"}
                style={{
                  width: "100%",
                  borderRadius: 2,
                  marginBottom: 6,
      ...
      {streams.length === 0 && <EmptyState compact title="Nobody streaming MLB right now." />}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {playing && (
        <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 4 }}>
          ...
          />
          <button onClick={() => setPlaying(null)} style={{ ...S.btn, margin: 8, fontSize: 11 }}>
            Close Player
          </button>
        </div>
      )}
      ...
      >
        {highlights.map((h: any) => (
          ...
          >
            <div
              style={{ position: "relative", paddingTop: "56.25%", background: "rgba(0,0,0,.4)" }}
            >
              {h.thumbnailUrl && (
                <img
                  src={h.thumbnailUrl}
...
function Matchups({ accentColor }: { accentColor: string }) {
  ...
  const statRow = (label: string, value: string | number | undefined, highlight?: boolean) => (
    ...
    >
      ...
      <span style={{ fontWeight: 600, color: highlight ? accentColor : "rgba(243,244,246,.92)" }}>
        {value ?? "-"}
      </span>
    </div>
  );
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 13 }}>
                  {m.away?.abbr} <span style={{ opacity: 0.4, fontWeight: 400 }}>@</span>{" "}
                  {m.home?.abbr}
                </span>
                {(isLive || isFinal) && (
            ...
            {isExpanded && (
              ...
              >
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  {m.away?.probablePitcher && (
                    <div>
                      <div style={{ ...S.label, fontSize: 9, marginBottom: 4 }}>
                        {m.away.abbr} — {m.away.probablePitcher.name}
                      </div>
                  ...
                  {m.home?.probablePitcher && (
                    <div>
                      <div style={{ ...S.label, fontSize: 9, marginBottom: 4 }}>
                        {m.home.abbr} — {m.home.probablePitcher.name}
                      </div>
                ...
                <div style={{ marginTop: 10, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <div style={{ fontSize: 11, opacity: 0.45 }}>
                    <span style={{ fontWeight: 600 }}>Records:</span> {m.away?.abbr} {m.away?.wins}-
                    {m.away?.losses} · {m.home?.abbr} {m.home?.wins}-{m.home?.losses}
                  </div>
                </div>
              </div>
            )}
...
export default function MlbModulesPanel({
  ...
  const [jumpToPlayer, setJumpToPlayer] = useState<{ id: string | number; name: string } | null>(
    null,
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {tab === "leaders" && (
          <Leaders accentColor={accentColor} onPlayerClick={handleLeaderPlayerClick} />
        )}
        {tab === "player" && (
          <PlayerSearch
            accentColor={accentColor}
