  ...
  const sections = React.useMemo(() => {
    ...
    function buildRounds(ms: Match[]) {
      ...
      return Array.from(byRound.entries())
        ...
        .map(([r, ms2]) => ({
          round: r,
          matches: ms2.sort((a, b) => a.bracketPosition - b.bracketPosition),
        }));
    }
    if (!isDouble) {
      return [
        { side: null as "WINNERS" | "LOSERS" | "GRAND" | null, rounds: buildRounds(matches) },
      ];
    }
    ...
    const out: Array<{
      side: "WINNERS" | "LOSERS" | "GRAND" | null;
  ...
  return (
    ...
    >
      ...
      {match.status === "CONFIRMED" && match.pgcrInstanceId && (
        <div
          title="Auto-verified from Bungie PGCR"
          style={{
            padding: "3px 8px",
            fontSize: 8,
            fontWeight: 800,
