    ...
    if (typeof window !== "undefined" && !_sessionExpiredNotified) {
      ...
      setTimeout(() => {
        if (typeof document !== "undefined" && document.title.indexOf("Session expired") === -1) {
          try {
            console.warn("[Weered] Session expired — sign out + back in to refresh.");
          } catch {}
        }
      }, 500);
    }
...
function useWsRefresh(types: string[], cb: () => void) {
  ...
  React.useEffect(() => {
    ...
    return () => {
      if (t) clearTimeout(t);
      names.forEach((n) => window.removeEventListener(n, fire));
    };
...
function useLobbyPresence(lobbyId: string) {
  ...
  useWsRefresh(["presence:join", "presence:leave", "presence:state", "lobby:activity"], () =>
    loadRef.current(),
  );
  return users;
}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {visible.map((u) => (
          <PresenceAvatar
            ...
            user={u}
            size={28}
            onClick={() => openSheet("profile", { userId: String(u.id) })}
          />
        ))}
        {overflow > 0 && (
          <div
...
function RoomsEmptyDiscovery({ lobbyId, canPin }: { lobbyId: string; canPin: boolean }) {
  ...
  if (!others.length) {
    return (
      <EmptyState
        compact
        title={lobbyId ? "No rooms open here." : "No rooms."}
        hint={canPin ? "Hit + Room to start one." : "Check back in a minute."}
      />
    );
  }
  return (
...
function RoomsPanel({ currentRoomId, lobbyId }: { currentRoomId: string; lobbyId: string }) {
  ...
  React.useEffect(() => {
    ...
    fetch(`${API_BASE}/lobbies/${encodeURIComponent(lobbyId)}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => setLobbyOwnerId(String(j?.lobby?.ownerId || "")))
      .catch(() => {});
  }, [lobbyId]);
  ...
  async function createRoom() {
    ...
    if (!name || !lobbyId) {
      setErr(!lobbyId ? "No lobby context." : "");
      return;
    }
    setCreating(true);
    setErr("");
  ...
  const filtered = rows
    .filter(
      (r) => !q.trim() || (r.name + " " + r.id).toLowerCase().includes(q.trim().toLowerCase()),
    )
    .sort((a, b) => {
  ...
  return (
    <div className="weered-rr-section" style={{ marginBottom: 16 }}>
      ...
      >
        <div
          className="weered-rr-section-title"
          style={{
            fontSize: 11,
            fontWeight: 700,
            opacity: 0.5,
      ...
      {showCreate && (
        <div
          className="weered-rr-create-panel"
          style={{
            marginBottom: 10,
            padding: "12px",
            borderRadius: 12,
            border: "1px solid rgba(88,0,229,.25)",
            background: "rgba(88,0,229,.05)",
          }}
        >
          <div
            className="weered-rr-create-title"
            style={{
              fontSize: 10,
              fontWeight: 800,
              letterSpacing: ".7px",
          ...
          <input
            style={{
              ...s.input,
              marginBottom: 10,
              borderColor: roomPassword ? "rgba(88,0,229,.30)" : "rgba(255,255,255,.10)",
            }}
          ...
          <button
            ...
            style={{
              ...
              padding: "9px",
              borderRadius: 9,
              border: "1px solid rgba(88,0,229,.40)",
              background: "rgba(88,0,229,.15)",
          ...
          >
            {creating
              ? "Creating…"
              : `Create "${newRoom.trim() || "…"}" with ${ROOM_MODULES.find((m) => m.id === selectedModule)?.label || "module"}`}
          </button>

          {err && (
            <div style={{ fontSize: 11, color: "rgba(252,165,165,.80)", marginTop: 6 }}>{err}</div>
          )}
        </div>
      )}

      <input
        className="weered-rr-search"
      ...
      />
      {err && (
        <div style={{ fontSize: 11, color: "rgba(252,165,165,.80)", marginBottom: 6 }}>{err}</div>
      )}
      ...
      >
        ...
        {!loading && !filtered.length && <RoomsEmptyDiscovery lobbyId={lobbyId} canPin={canPin} />}
        {filtered.slice(0, 40).map((rm) => {
          ...
          const icon: React.ReactNode = customIcon ? (
            ...
            "⚓"
          ) : isWindrose && (n.includes("crew") || n.includes("mate")) ? (
            "🏴\u200d☠️"
            ...
            "🎙"
          ) : n.includes("lfg") || n.includes("squad") ? (
            "🔥"
          ) : n.includes("trad") ? (
            "💱"
          ) : n.includes("general") ? (
            ...
            "🏆"
          ) : n.includes("watch") || n.includes("stream") ? (
            "📺"
          ) : n.includes("cryo") || n.includes("archive") ? (
            "🧊"
          ...
          const cardAccent =
            customAccent || (isLive ? "#22c55e" : active ? "#5800E5" : "rgba(255,255,255,.08)");
          const borderColor = customAccent
            ? `${customAccent}55`
            : active
              ? "rgba(88,0,229,.45)"
              : isLive
            ...
            <Link
              ...
              onMouseEnter={(e) => {
                ...
                el.style.boxShadow = isLive
                  ? "0 0 12px rgba(34,197,94,.08)"
                  : active
                    ? "0 0 12px rgba(88,0,229,.08)"
                    : "none";
              }}
              onMouseLeave={(e) => {
                const el = e.currentTarget as HTMLElement;
              ...
              {customBanner && (
                <div
                  ...
                  style={{
                    ...
                    inset: 0,
                    background:
                      "linear-gradient(90deg, rgba(10,12,20,.85) 0%, rgba(10,12,20,.55) 50%, rgba(10,12,20,.25) 100%)",
                    pointerEvents: "none",
                  }}
                />
              )}
              ...
              {active && (
                <div
                  style={{
                    ...
                    borderRadius: 2,
                    background: "#5800E5",
                    boxShadow: "0 0 6px rgba(88,0,229,.4)",
                  }}
                />
              )}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    {rm.pinned && (
                      <span
                        title="Pinned room — survives cleanup"
                        style={{ marginRight: 4, fontSize: 10, color: "#f59e0b" }}
                      >
                        📌
                      </span>
                    )}
                    {rm.name || rm.id}
                    {rm.locked && (
                      <span style={{ marginLeft: 4, fontSize: 9, opacity: 0.45 }}>🔒</span>
                    )}
                    {rm.hasPassword && !rm.locked && (
                      <span style={{ marginLeft: 4, fontSize: 9, opacity: 0.45 }}>🔑</span>
                    )}
                  </div>
                  {liveWsUsers.length > 0 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 3 }}>
                      ...
                      {liveWsUsers.length > 3 && (
                        <span style={{ fontSize: 9, opacity: 0.35, fontFamily: "monospace" }}>
                          +{liveWsUsers.length - 3}
                        </span>
                      )}
                    </div>
                    ...
                    <>
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          void toggleEvent(rm.id, Boolean(rm.isEvent));
                        }}
                        style={{
                          marginLeft: 4,
                          width: 22,
                        ...
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLElement).style.background = rm.isEvent
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          void togglePin(rm.id, Boolean(rm.pinned));
                        }}
                        style={{
                          marginLeft: 4,
                          width: 22,
                        ...
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLElement).style.background = rm.pinned
...
function LobbyModPanel({ globalRole, lobbyId }: { globalRole: string; lobbyId: string }) {
  ...
  React.useEffect(() => {
    if (ctxLocked !== null) {
      try {
        localStorage.setItem(LS_KEY, String(ctxLocked));
      } catch {}
    }
  }, [ctxLocked]);
  ...
  async function action(type: "lock" | "unlock") {
    ...
    try {
      ...
      if (j?.ok) {
        ...
        try {
          localStorage.setItem(LS_KEY, String(willLock));
        } catch {}
      } else {
        setOptimistic(!willLock);
        ...
        <a
          ...
          onClick={async () => {
            ...
            const ok = await weeredConfirm({
              title: "Clear all lobby chat?",
              body: "Every message in this lobby's chat gets wiped. Can't be undone.",
...
function CrewEmptyDiscovery({ lobbyId }: { lobbyId: string }) {
  ...
  const top = lobbyCrews
    ...
    .sort(
      (a, b) =>
        Number(b?.recruiting) - Number(a?.recruiting) ||
            ...
            >
              ...
              >
                {c.logoUrl ? (
                  ...
                  (c.tag || c.name || "?").slice(0, 2).toUpperCase()
                )}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div
                ...
                >
                  ...
                  {c.tag && (
                    <span
                      style={{ marginLeft: 5, fontSize: 9, opacity: 0.5, fontFamily: "monospace" }}
                    >
                      [{c.tag}]
                    </span>
                  )}
                ...
                <div style={{ fontSize: 10, opacity: 0.55, marginTop: 1 }}>
                  ...
                  {c.recruiting && (
                    <span style={{ marginLeft: 6, color: "rgb(110,231,183)" }}>· recruiting</span>
                  )}
                </div>
              </div>
              <span
                ...
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <div
                    style={{
                      ...
                      height: 26,
                      borderRadius: 999,
                      background: m.avatar ? "rgba(255,255,255,.08)" : "rgba(245,158,11,.25)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: 11,
                  ...
                  <span
                    style={{
                      ...
                      height: 8,
                      borderRadius: 999,
                      background: m.online ? "#22c55e" : "rgba(255,255,255,.15)",
                      border: "2px solid rgba(10,10,15,1)",
                    }}
                  />
                </div>
    ...
    <div className="weered-rr-section" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {lobbies.map((l) => {
          ...
          const initial = String(l.name || l.id || "?")
            .slice(0, 1)
            .toUpperCase();
          return (
            <button
