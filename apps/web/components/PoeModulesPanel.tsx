...
const S = {
  ...
  btnPri: {
    padding: "6px 12px",
    borderRadius: 2,
    border: "1px solid rgba(175,96,37,.35)",
    background: "rgba(175,96,37,.12)",
    fontSize: 12,
    cursor: "pointer",
    color: "rgb(175,96,37)",
    fontWeight: 600,
...
const LEAGUES = ["Mirage", "Hardcore Mirage", "Standard", "Hardcore"];
...
function EconomyTab({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({
        asOf: j?.asOf,
        divineChaos: j?.divineChaos,
        chaosIcon: j?.chaosIcon,
        divineIcon: j?.divineIcon,
    ...
    <div>
          ...
          {data.divineChaos ? (
            <>
              ...
              <div style={{ lineHeight: 1.15 }}>
                ...
                >
                  {data.divineChaos.toLocaleString(undefined, { maximumFractionDigits: 0 })}{" "}
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow = `0 0 0 1px ${accent}22, inset 0 0 22px ${accent}0c`;
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = hot ? `${accent}33` : "rgba(255,255,255,.06)";
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 5 }}>
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${Math.max(4, Math.round((item.volume / maxVol) * 100))}%`,
                    ...
                    >
                      {Number(item.volume).toLocaleString(undefined, {
                        notation: "compact",
              ...
              <div style={{ textAlign: "right", flexShrink: 0, lineHeight: 1.25 }}>
                <div
                  style={{
                    fontSize: 13.5,
                    fontWeight: 800,
                    color: hot ? accent : "rgba(243,244,246,.9)",
                    fontFamily: "monospace",
                    display: "flex",
                    alignItems: "center",
                    gap: 4,
                ...
                >
                  {(item.chaos || 0).toLocaleString(undefined, {
                    maximumFractionDigits: item.chaos >= 100 ? 0 : 1,
                  })}
      ...
      {filtered.length === 0 && (
        <EmptyState
          ...
          hint="The exchange digest may not be published yet for this league."
        />
      )}
    </div>
  );
...
function ItemsTab({ league, accent }: { league: string; accent: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const j = await ninjaFetch(
        `${POE_NINJA}/itemoverview?league=${encodeURIComponent(league)}&type=${category}`,
      );
      setItems(j.lines || []);
    } catch (e) {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          <div style={{ fontSize: 10, color: "rgba(148,163,184,.4)", marginBottom: 8 }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
                      ...
                      >
                        {(item.chaosValue || 0).toLocaleString(undefined, {
                          maximumFractionDigits: 1,
                        })}
                        <span
                          style={{
...
function DivCardsTab({ league, accent }: { league: string; accent: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const j = await ninjaFetch(
        `${POE_NINJA}/itemoverview?league=${encodeURIComponent(league)}&type=DivinationCard`,
      );
      setCards(j.lines || []);
    } catch (e) {
  ...
  const filtered = (
    search ? cards.filter((c) => c.name?.toLowerCase().includes(search.toLowerCase())) : cards
  ).sort((a, b) => {
    if (sortBy === "chaos") return (b.chaosValue || 0) - (a.chaosValue || 0);
    return (a.name || "").localeCompare(b.name || "");
  });
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <input
          style={{ ...S.input, flex: "1 1 200px" }}
          placeholder="Search divination cards..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
        <select style={S.select} value={sortBy} onChange={(e) => setSortBy(e.target.value as any)}>
                ...
                <div
                  ...
                  onMouseLeave={(e) =>
                    (e.currentTarget.style.borderColor =
                      (card.chaosValue || 0) >= 100 ? `${accent}30` : "rgba(255,255,255,.06)")
                  }
                >
                  <div
                    style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
                      ...
                      >
                        {(card.chaosValue || 0).toLocaleString(undefined, {
                          maximumFractionDigits: 1,
                        })}
                        <span
                          style={{
...
function GemsTab({ league, accent }: { league: string; accent: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const j = await ninjaFetch(
        `${POE_NINJA}/itemoverview?league=${encodeURIComponent(league)}&type=SkillGem`,
      );
      setGems(j.lines || []);
    } catch (e) {
  ...
  const filtered = (
    search ? gems.filter((g) => g.name?.toLowerCase().includes(search.toLowerCase())) : gems
  )
    .filter((g) => {
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {filtered.slice(0, 100).map((gem, idx) => {
              ...
              return (
                <div
                  key={gem.id || `${gem.name}-${gem.gemLevel}-${gem.gemQuality}-${idx}`}
                  style={{
                    ...S.card,
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
                      ...
                      >
                        {(gem.chaosValue || 0).toLocaleString(undefined, {
                          maximumFractionDigits: 1,
                        })}
                        <span
                          style={{
  ...
  return (
    <div>
      ...
      {posts.length === 0 ? (
        <EmptyState
          icon="👥"
          title="No parties posted yet."
          hint="Drop one up top — someone's looking."
        />
      ) : (
  ...
  const load = useCallback(async () => {
    try {
      const j = await apiFetch(
        `/twitch/streams?game=${encodeURIComponent("Path of Exile")}&first=20`,
      );
    ...
    return (
      <div style={{ textAlign: "center", padding: 30 }}>
        ...
        <div style={{ fontSize: 13, color: "var(--weered-muted, rgba(148,163,184,.55))" }}>
          Nobody streaming PoE right now.
        </div>
      </div>
    );
...
const POE_FRAME_COLOR: Record<number, string> = {
  0: "#c8c8c8",
  1: "#8888ff",
  2: "#ffff77",
  3: "#af6025",
...
function GearGrid({ items, accent }: { items: any[]; accent: string }) {
  const equip = items
    .filter((i) => POE_SLOT_ORDER.includes(i.slot))
    .sort((a, b) => POE_SLOT_ORDER.indexOf(a.slot) - POE_SLOT_ORDER.indexOf(b.slot));
  const flasks = items.filter((i) => i.slot === "Flask");
  if (equip.length === 0 && flasks.length === 0)
    return <div style={{ fontSize: 11, opacity: 0.5, padding: 8 }}>No equipped items.</div>;
  return (
    <div>
            ...
            <div style={{ minWidth: 0 }}>
              <div
                style={{
                  fontSize: 11,
                  fontWeight: 700,
                  color: POE_FRAME_COLOR[it.frame] || "#c8c8c8",
                  whiteSpace: "nowrap",
                  overflow: "hidden",
                  textOverflow: "ellipsis",
                }}
              >
                {it.name || it.typeLine}
                {it.corrupted && <span style={{ color: "#d20000", marginLeft: 4 }}>{"⊘"}</span>}
              </div>
              <div
                style={{
                  fontSize: 9,
              ...
              it.icon ? (
                <img
                  ...
                  style={{
                    ...
                    height: 28,
                    objectFit: "contain",
                    border: `1px solid ${POE_FRAME_COLOR[it.frame] || "#555"}55`,
                    borderRadius: 3,
                    background: "rgba(0,0,0,.28)",
                  }}
                />
...
function PoeAccountTab({ accent }: { accent: string }) {
  ...
  async function openChar(c: any) {
    ...
    setGearLoading(true);
    const j = await apiFetch(
      `/poe/character?name=${encodeURIComponent(c.name)}&realm=${encodeURIComponent(c.realm || "pc")}`,
    );
    setGear(j);
    ...
    return (
      <div style={{ maxWidth: 460, margin: "0 auto", textAlign: "center", paddingTop: 28 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 800, color: "#fff", marginBottom: 8 }}>
          Link your Path of Exile account
        </div>
        <div
          style={{
            fontSize: 12,
        ...
        >
          Connect through Grinding Gear Games to surface your characters and league in your Weered
          profile. Read-only — Weered only requests profile and character access, and you can unlink
          anytime.
        </div>
        <a
          href={linkHref}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)" }}>
            {String(a.realm || "pc").toUpperCase()} · linked
          </div>
        </div>
        <button
          onClick={unlink}
      ...
      <div>
        ...
        {chars.length === 0 ? (
          <div style={{ fontSize: 12, color: "rgba(255,255,255,.4)", padding: 12 }}>
            No characters returned (private profile, or the token expired — re-link to refresh).
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {chars.map((c: any) => {
                ...
                <div key={c.id || c.name}>
                  <div
                    ...
                    style={{
                      ...
                      gap: 10,
                      padding: "8px 12px",
                      background: open ? `${accent}14` : "rgba(255,255,255,.03)",
                      border: `1px solid ${open ? accent + "44" : "rgba(255,255,255,.07)"}`,
                      borderRadius: 4,
                      cursor: "pointer",
                    }}
                  >
                    <span style={{ width: 10, color: "rgba(255,255,255,.4)", fontSize: 10 }}>
                      {open ? "▾" : "▸"}
                    </span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)" }}>
                        {c.class}
                        {c.league ? ` · ${c.league}` : ""}
                        {c.realm ? ` · ${String(c.realm).toUpperCase()}` : ""}
                      </div>
                    </div>
                    {typeof c.level !== "undefined" && (
                  ...
                  {open && (
                    <div style={{ padding: "10px 4px 6px" }}>
                      {gearLoading ? (
                        <div style={{ fontSize: 11, opacity: 0.5, padding: 8 }}>Loading gear…</div>
                      ) : gear && gear.items && gear.items.length ? (
                        <GearGrid items={gear.items} accent={accent} />
                      ) : (
                        <div style={{ fontSize: 11, opacity: 0.5, padding: 8 }}>
                          {gear && gear.error === "private"
                            ? "Items hidden — enable the Stash/Items privacy on pathofexile.com."
                            : "No gear data."}
                        </div>
                      )}
                    </div>
...
function LadderTab({ league, accent }: { league: string; accent: string }) {
  ...
  if (loading)
    return <div style={{ padding: 20, opacity: 0.5, fontSize: 12 }}>Loading ladder…</div>;
  const entries = data?.entries || [];
  if (entries.length === 0)
    return (
      <div style={{ padding: 24, textAlign: "center", opacity: 0.45, fontSize: 13 }}>
        No ladder available for {league}.
      </div>
    );
        ...
        {entries.map((e: any) => (
          <div
            ...
            style={{
              ...
              gap: 10,
              padding: "7px 12px",
              background: e.rank % 2 ? "rgba(255,255,255,.02)" : "transparent",
              borderRadius: 3,
            }}
          >
            <div
              style={{
                ...
                fontWeight: 800,
                fontSize: 13,
                color: e.rank === 1 ? "#fbbf24" : e.rank <= 3 ? "#cbd5e1" : "rgba(255,255,255,.45)",
              }}
            >
              {e.rank}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              >
                {e.name}
                {e.dead && (
                  <span style={{ color: "#f87171", fontSize: 10, marginLeft: 6, fontWeight: 700 }}>
                    DEAD
                  </span>
                )}
              </div>
...
export default function PoeModulesPanel({
  ...
  useEffect(() => {
    apiFetch("/poe/leagues").then((j: any) => {
      const ls = j?.leagues || [];
      if (ls.length) setLeagues(ls);
    });
  }, []);
    ...
    >
      ...
      >
        ...
        {(tab === "economy" || tab === "divcards" || tab === "ladder") && (
          ...
          >
            {(leagues.length ? leagues.map((l) => l.id) : LEAGUES).map((l) => (
      ...
      >
        ...
        >
          Weered is not affiliated with, endorsed by, or sponsored by Grinding Gear Games. Path of
          Exile is a registered trademark of Grinding Gear Games. Economy and passive tree data via
          Grinding Gear Games' official API.
        </p>
      </div>
    </div>
  );
