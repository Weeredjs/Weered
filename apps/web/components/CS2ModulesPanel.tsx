...
const S = {
  ...
  btnPri: {
    padding: "6px 12px",
    borderRadius: 2,
    border: "1px solid rgba(222,155,53,.35)",
    background: "rgba(222,155,53,.12)",
    fontSize: 12,
    cursor: "pointer",
    color: "rgb(222,155,53)",
    fontWeight: 600,
  } as React.CSSProperties,
  input: {
    width: "100%",
...
function SquadFinder({ lobbyId, accent }: { lobbyId: string; accent: string }) {
  ...
  const rankOptions =
    mode === "Premier" || mode === "Faceit" ? CS2_PREMIER_RATINGS : CS2_COMP_RANKS;
  ...
  return (
    <div>
      ...
      {showForm && (
        ...
        >
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={S.label}>
                {mode === "Premier" || mode === "Faceit" ? "CS Rating" : "Rank"}
              </div>
              <select
                style={{ ...S.select, width: "100%" }}
                value={rank}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              ...
              <select
                style={{ ...S.select, width: "100%" }}
                value={mapPref}
                onChange={(e) => setMapPref(e.target.value)}
              >
                {CS2_MAPS.map((m) => (
                  <option key={m} value={m}>
                    {m}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: micRequired ? 700 : 400,
                transition: "all .15s",
              }}
            >
      ...
      {posts.length === 0 ? (
        <EmptyState
          icon="🎮"
          title="No squads posted yet."
          hint="Drop the first one — someone's queueing."
        />
      ) : (
                    ...
                    {slots.map((filled, i) => (
                      <div
                        ...
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 700,
                          color: filled ? accent : "rgba(148,163,184,.25)",
                          transition: "all .15s",
                        }}
                      >
                        {filled ? "\u2713" : ""}
  ...
  const load = useCallback(async () => {
    try {
      const j = await apiFetch(
        `/twitch/streams?game=${encodeURIComponent("Counter-Strike")}&first=20`,
      );
...
function RanksReference({ accent }: { accent: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <div
          style={{ ...S.label, marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}
        >
      ...
      >
        ...
        Premier uses a visible CS Rating (Elo). Competitive uses the classic 18-rank ladder. Both
        reset with major updates. Your rank reflects individual performance plus win/loss record.
      </div>
    </div>
  );
}
...
const MAP_DATA: { name: string; color: string; bgColor: string; desc: string }[] = [
  {
    name: "Dust2",
    color: "#D4A017",
    bgColor: "rgba(212,160,23,.08)",
    desc: "Classic desert battleground",
  },
  {
    name: "Mirage",
    color: "#E8943A",
    bgColor: "rgba(232,148,58,.08)",
    desc: "Moroccan market showdown",
  },
  {
    name: "Inferno",
    color: "#E06030",
    bgColor: "rgba(224,96,48,.08)",
    desc: "Italian street warfare",
  },
  {
    name: "Nuke",
    color: "#4A90D9",
    bgColor: "rgba(74,144,217,.08)",
    desc: "Nuclear power plant, vertical play",
  },
  {
    name: "Anubis",
    color: "#9B59B6",
    bgColor: "rgba(155,89,182,.08)",
    desc: "Egyptian temple, newest addition",
  },
  {
    name: "Ancient",
    color: "#27AE60",
    bgColor: "rgba(39,174,96,.08)",
    desc: "Aztec ruins, jungle setting",
  },
  {
    name: "Vertigo",
    color: "#3498DB",
    bgColor: "rgba(52,152,219,.08)",
    desc: "Skyscraper rooftop combat",
  },
  {
    name: "Overpass",
    color: "#2ECC71",
    bgColor: "rgba(46,204,113,.08)",
    desc: "European canal, complex rotations",
  },
];
...
function MapPool({ accent }: { accent: string }) {
  return (
    <div>
      ...
      >
        Active Duty maps rotate with CS2 major updates. Competitive and Premier queues use this
        pool.
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Weered is not affiliated with, endorsed by, or sponsored by Valve Corporation.
          Counter-Strike 2 and all related logos and trademarks are the property of Valve
          Corporation.
        </p>
      </div>
    </div>
