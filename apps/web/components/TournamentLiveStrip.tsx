...
export default function TournamentLiveStrip({
  ...
  const headerLabel = (() => {
    ...
    if (championCount)
      parts.push(`${championCount} recent champion${championCount > 1 ? "s" : ""}`);
    if (activeCount) parts.push(`${activeCount} active`);
    if (upcomingCount) parts.push(`${upcomingCount} starting soon`);
    return parts.join(" · ");
  })();

  const openMatchItem = items.find((i) => i.kind === "live_match" && i.id === openMatchId) as
    | LiveMatchItem
    | undefined;
        ...
        {!collapsed &&
          items.map((it) => {
            ...
            if (it.kind === "recent_champion") {
              return (
                <ChampionRow
                  key={`c-${it.tournament.id}`}
                  item={it}
                  onOpen={() => {
                    try {
                      window.dispatchEvent(
                        new CustomEvent("weered:openTournamentsTab", {
                          detail: { tournamentId: it.tournament.id },
                        }),
                      );
                    } catch {}
                  }}
...
function TournamentStateRow({
  item,
  onOpen,
}: {
  item: TournamentRowItem;
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = `0 0 12px ${accent}33`;
    ...
    >
      ...
      <span style={{ fontSize: 11, color: "rgba(255,255,255,.55)", marginLeft: "auto" }}>
        {t.format === "CHALLENGE_RACE" && t.leader ? (
          <>
            Leader:{" "}
            <span style={{ color: "rgba(255,255,255,.85)", fontWeight: 700 }}>
              {t.leader.displayName}
            </span>{" "}
            <span style={{ color: ACCENT, fontFamily: "monospace", fontWeight: 800 }}>
              @ {t.leader.score}
              {t.pointsToWin ? `/${t.pointsToWin}` : ""}pts
            </span>
          </>
        ) : (
          <>
...
function ChampionRow({ item, onOpen }: { item: ChampionItem; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = "0 0 16px rgba(251,191,36,.35)";
        (e.currentTarget as HTMLElement).style.borderColor = "#fbbf24";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.boxShadow = "0 0 8px rgba(251,191,36,.18)";
        (e.currentTarget as HTMLElement).style.borderColor = "rgba(251,191,36,.5)";
      }}
    >
      <span
        style={{
      ...
      <span style={{ fontSize: 11, color: "rgba(255,255,255,.55)" }}>won</span>
