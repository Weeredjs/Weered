...
export default function PassiveTree({ accent = "#AF6025" }: { accent?: string }) {
  ...
  useEffect(() => {
    ...
    sheets.current = {
      active: mk(sp.active.url),
      inactive: mk(sp.inactive.url),
      mA: mk(sp.masteryActive.url),
      mI: mk(sp.masteryInactive.url),
    };
  }, [tree]);
    ...
    (async () => {
      try {
        ...
        if (alive && j?.ok) {
          ...
          if (tree && j.hashes?.length)
            centerOnAllocated(new Set<number>(j.hashes.map((x: any) => Number(x))));
        }
      } catch {}
    })();
    return () => {
  ...
  const centerOnAllocated = useCallback((set: Set<number>) => {
    ...
    const w = wrap.clientWidth,
      h = wrap.clientHeight;
    const bw = Math.max(800, maxX - minX),
      bh = Math.max(800, maxY - minY);
  ...
  const draw = useCallback(() => {
    ...
    for (const n of tree.nodes) {
      ...
      const sheet = isMast
        ? on
          ...
          : sheets.current.mI
        : on
          ? sheets.current.active
          : sheets.current.inactive;
      const cmap = sp
        ? (isMast ? (on ? sp.masteryActive : sp.masteryInactive) : on ? sp.active : sp.inactive)
            .coords
        : null;
      ...
      if (sheet && sheet.complete && sheet.naturalWidth > 0 && co) {
        ...
        if (on) {
          ...
          ctx.shadowBlur = n.k === KIND.KEYSTONE ? 42 : n.k === KIND.NOTABLE ? 26 : 14;
        }
        ...
        if (on && (n.k === KIND.NOTABLE || n.k === KIND.KEYSTONE)) {
          ctx.beginPath();
          ctx.arc(n.x, n.y, Math.max(rw, rh) / 2 + 5, 0, Math.PI * 2);
          ctx.lineWidth = 4;
        ...
        if (on) {
          ctx.shadowColor = accent;
          ctx.shadowBlur = n.k === KIND.KEYSTONE ? 34 : 18;
        }
        ctx.beginPath();
        ctx.arc(n.x, n.y, rad, 0, Math.PI * 2);
        ctx.fillStyle = on ? accent : hasBuild ? "#2a2a31" : "#3c3c45";
        ctx.fill();
        ctx.shadowBlur = 0;
      }
    }
    ...
    if (hover) {
      const n = hover.node;
      const rad =
        n.k === KIND.KEYSTONE ? 64 : n.k === KIND.NOTABLE ? 44 : n.k === KIND.MASTERY ? 38 : 26;
      ctx.beginPath();
  ...
  const hitTest = (sx: number, sy: number): TNode | null => {
    ...
    for (const n of tree.nodes) {
      ...
      const dx = n.x - wpt.x,
        ...
        d = dx * dx + dy * dy;
      const r =
        n.k === KIND.KEYSTONE ? 64 : n.k === KIND.NOTABLE ? 44 : n.k === KIND.MASTERY ? 38 : 28;
      if (d < r * r && d < bestD) {
        bestD = d;
        best = n;
      }
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        {linked && chars.length > 0 ? (
          ...
          <span style={{ fontSize: 11, color: "rgba(148,163,184,.55)" }}>
            {linked
              ? "No characters found."
              : "Link your PoE account (My Account) to overlay your build."}
          </span>
        )}
        {allocated.size > 0 && (
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
          <button
            onClick={() => {
              view.current.scale = Math.min(1.2, view.current.scale * 1.3);
              requestDraw();
            }}
            style={btn}
          >
          ...
          <button
            onClick={() => {
              view.current.scale = Math.max(0.012, view.current.scale / 1.3);
              requestDraw();
            }}
            style={btn}
          >
          ...
          <button
            onClick={() => (allocated.size ? centerOnAllocated(allocated) : fit())}
            style={btn}
          >
            Fit
          </button>
      ...
      <div
        ...
        style={{
          ...
          background: "radial-gradient(circle at 50% 50%, rgba(255,255,255,.02), #0b0b0e 70%)",
      ...
      >
        ...
        {hover && (
          <div
            style={{
              position: "absolute",
              left: Math.min(hover.sx + 14, (wrapRef.current?.clientWidth || 400) - 240),
          ...
          >
            <div
              style={{
                ...
                fontWeight: 800,
                color:
                  hover.node.k === KIND.KEYSTONE
                    ? "#ff9d5c"
                    : hover.node.k === KIND.NOTABLE
                      ? accent
                      : "rgba(243,244,246,.95)",
                marginBottom: hover.node.st?.length ? 5 : 0,
              }}
            >
            ...
            {(hover.node.st || []).map((line, i) => (
              <div
                key={i}
                style={{ fontSize: 11, color: "rgba(148,180,220,.9)", lineHeight: 1.35 }}
              >
                {line}
              </div>
            ))}
            {allocated.size > 0 && (
              <div
                style={{
                  fontSize: 9,
                  marginTop: 5,
                  color: allocated.has(hover.node.h) ? accent : "rgba(148,163,184,.45)",
                  fontWeight: 700,
                  letterSpacing: ".4px",
                }}
              >
        ...
        >
          <span>
            <span style={{ color: accent }}>●</span> Keystone/Notable
          </span>
