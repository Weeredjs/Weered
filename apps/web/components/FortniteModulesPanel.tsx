...
function useWishlist() {
  ...
  async function toggle(
    cosmeticId: string,
    meta?: { name?: string; type?: string; rarity?: string; image?: string },
  ) {
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: size,
        lineHeight: 1,
        color: wishlisted ? "#EF4444" : "rgba(148,163,184,.3)",
...
function TwitchStreams({
  ...
  if (streams.length === 0 && !activeStream)
    return (
      <EmptyState
        compact
        title="Nobody streaming right now."
        hint="Check back, someone usually goes live soon."
      />
    );
    ...
    <div>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              ...
              >
                {["crossplay", "pc", "xbox", "psn", "switch", "mobile"].map((p) => (
      ...
      {posts.length === 0 ? (
        <EmptyState
          ...
          hint="Drop the first one — someone's looking."
        />
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {posts.map((p) => (
...
function StatsLookup({ accent }: { accent: string }) {
  ...
  function ModeRow({ label, data }: { label: string; data: any }) {
    ...
    return (
      ...
      >
        <span style={{ fontSize: 11, fontWeight: 700, color: accent, width: 50, flexShrink: 0 }}>
          {label}
        </span>
        <span style={{ fontSize: 11, color: "rgba(243,244,246,.8)", flex: 1 }}>
          {data.wins ?? 0} W · {data.kills ?? 0} K · {(data.kd ?? 0).toFixed(2)} K/D ·{" "}
          {data.matches ?? 0} games
        </span>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            {stats.image && (
              <img
                src={stats.image}
                alt={(stats.account?.name || "Player") + " avatar"}
                style={{
                  width: 64,
                  height: 64,
                  borderRadius: 2,
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 18, fontWeight: 800, color: "rgba(243,244,246,.95)" }}>
                  {stats.account?.name}
                </span>
                {preloaded && (
                  <span
              ...
              {stats.battlePass && (
                <div style={{ fontSize: 11, color: "rgba(148,163,184,.5)", marginTop: 2 }}>
                  BP Level {stats.battlePass.level}
                </div>
              )}
            </div>
          </div>
          {stats.stats?.all && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              ...
              <StatBox
                label="Win Rate"
                value={
                  stats.stats.all.winRate ? `${(stats.stats.all.winRate * 100).toFixed(1)}%` : "—"
                }
              />
...
function RankedTab({ accent }: { accent: string }) {
  ...
  async function lookup(name?: string) {
    ...
    if (j.ok) setData(j);
    else if (!name) setError(j.error || "Lookup failed");
    setLoading(false);
  }
  ...
  function CompareRow({ label, lifetime, season }: { label: string; lifetime: any; season: any }) {
    ...
    const lk = lifetime?.kd ?? 0,
      sk = season?.kd ?? 0;
    const lw = lifetime?.winRate ?? 0,
      sw = season?.winRate ?? 0;
    const kdDelta = sk - lk;
    const wrDelta = sw - lw;
    return (
      <div style={{ padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, fontSize: 11 }}>
          <div>
            <span style={{ opacity: 0.4 }}>Stat</span>
          </div>
          <div style={{ textAlign: "center" }}>
            <span style={{ opacity: 0.4 }}>Lifetime</span>
          </div>
          <div style={{ textAlign: "center" }}>
            <span style={{ opacity: 0.4 }}>This Season</span>
          </div>
          ...
          <div
            style={{
              textAlign: "center",
              fontWeight: 700,
              color: kdDelta > 0 ? "#22C55E" : kdDelta < 0 ? "#EF4444" : "rgba(243,244,246,.7)",
            }}
          >
            {sk.toFixed(2)}{" "}
            {kdDelta !== 0 && (
          ...
          <div
            style={{
              textAlign: "center",
              fontWeight: 700,
              color: wrDelta > 0 ? "#22C55E" : wrDelta < 0 ? "#EF4444" : "rgba(243,244,246,.7)",
            }}
          >
            {(sw * 100).toFixed(1)}%{" "}
            {wrDelta !== 0 && (
          ...
          <div style={{ textAlign: "center", fontWeight: 600 }}>
            {(lifetime?.wins ?? 0).toLocaleString()}
          </div>
          <div style={{ textAlign: "center", fontWeight: 600 }}>
            {(season?.wins ?? 0).toLocaleString()}
          </div>

          <div style={{ color: "rgba(243,244,246,.7)" }}>Matches</div>
          <div style={{ textAlign: "center", fontWeight: 600 }}>
            {(lifetime?.matches ?? 0).toLocaleString()}
          </div>
          <div style={{ textAlign: "center", fontWeight: 600 }}>
            {(season?.matches ?? 0).toLocaleString()}
          </div>
        </div>
      </div>
    );
    ...
    <div>
      ...
      {data && (
        <div>
          ...
          {data.battlePass && (
            <div style={{ fontSize: 11, color: "rgba(148,163,184,.5)", marginBottom: 16 }}>
          ...
          <div style={S.card}>
            ...
            <CompareRow
              label="Overall"
              lifetime={data.lifetime?.overall}
              season={data.season?.overall}
            />
        ...
        >
          {(n.image || n.tileImage) && (
            <div
              style={{
                height: 140,
                background: `url(${n.image || n.tileImage}) center/cover no-repeat`,
...
function CosmeticSearch({
  ...
  useEffect(() => {
    setLoading(true);
    apiFetch("/fortnite/cosmetics/new")
      .then((j) => {
  ...
  async function search() {
    ...
    const j = await apiFetch(
      `/fortnite/cosmetics/search?query=${encodeURIComponent(query.trim())}`,
    );
  ...
  return (
    <div>
      ...
      {!loading && searched && !showingNew && items.length === 0 && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Weered is not affiliated with, endorsed by, or sponsored by Epic Games, Inc. Fortnite and
          all related logos and trademarks are the property of Epic Games, Inc.
        </p>
      </div>
    </div>
  );
