...
function VisualModes() {
  const Pill = ({ label, sub, active }: { label: string; sub: string; active?: boolean }) => (
    <div style={{
      ...
      <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "1px", textTransform: "uppercase", color: active ? "#fff" : "rgba(243,244,246,.55)" }}>{label}</div>
      <div style={{ fontSize: 10, color: "rgba(243,244,246,.4)", marginTop: 3 }}>{sub}</div>
    </div>
  );
  return (
    <div style={{ display: "flex", gap: 6, padding: 6, borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)" }}>
...
function VisualChart() {
  ...
  return (
    <div style={{
      height: 140, padding: 12, borderRadius: 12,
      border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)",
      display: "flex", flexDirection: "column", gap: 8,
    }}>
      <div style={{ display: "flex", gap: 4, fontSize: 9, letterSpacing: "0.5px" }}>
        {["1m","5m","15m","1h","4h","1D","1W"].map((t,i) => (
          <span key={t} style={{ padding: "2px 7px", borderRadius: 4, background: i===0 ? `${ACCENT}20` : "rgba(255,255,255,0.04)", color: i===0 ? ACCENT : "rgba(243,244,246,0.4)", fontWeight: 700 }}>{t}</span>
        ))}
      </div>
      ...
      <div style={{ fontSize: 10, color: "rgba(243,244,246,0.45)", fontFamily: "monospace" }}>BTCUSDT · $67,234.12 <span style={{ color: GREEN }}>+2.4%</span></div>
    </div>
  );
}
...
function VisualOrderEntry() {
  return (
    <div style={{ display: "flex", gap: 8, padding: 10, borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: 10, color: "rgba(243,244,246,0.4)", letterSpacing: "0.4px", textTransform: "uppercase" }}>USD amount</div>
        <div style={{ fontFamily: "monospace", fontSize: 14, color: "rgba(243,244,246,0.85)", padding: "6px 8px", borderRadius: 6, background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.08)" }}>$1,000</div>
        <div style={{ display: "flex", gap: 4 }}>
          <div style={{ flex: 1, padding: "5px 8px", borderRadius: 6, background: `${GREEN}1a`, border: `1px solid ${GREEN}55`, color: GREEN, fontSize: 11, fontWeight: 800, textAlign: "center", letterSpacing: "0.5px" }}>BUY · LONG</div>
          <div style={{ flex: 1, padding: "5px 8px", borderRadius: 6, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(243,244,246,0.45)", fontSize: 11, fontWeight: 600, textAlign: "center", letterSpacing: "0.5px" }}>SELL · SHORT</div>
        </div>
      </div>
      <div style={{ width: 1, background: "rgba(255,255,255,0.05)" }} />
      <div style={{ flex: 1.2, display: "flex", flexDirection: "column", justifyContent: "center", gap: 4 }}>
        <div style={{ fontSize: 9, color: "rgba(243,244,246,0.35)", letterSpacing: "0.6px", textTransform: "uppercase" }}>Then in chat:</div>
        <div style={{
          display: "flex", alignItems: "center", gap: 6, padding: "5px 8px", borderRadius: 4,
          borderLeft: `2px solid ${GREEN}99`, background: `${GREEN}08`, fontFamily: "monospace", fontSize: 10,
        }}>
          ...
          <span style={{ background: `${GREEN}26`, color: GREEN, padding: "0 4px", borderRadius: 2, fontSize: 8, fontWeight: 800 }}>LONG</span>
...
function VisualTicker() {
  const Row = ({ rank, name, pnl, pct, up }: { rank: string; name: string; pnl: string; pct: string; up: boolean }) => (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 11, fontFamily: "monospace" }}>
  ...
  return (
    <div style={{ display: "flex", gap: 10, padding: 12, borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
        <div style={{ fontSize: 10, color: "rgb(212,160,23)", fontWeight: 800, letterSpacing: "0.4px", textTransform: "uppercase" }}>The Operator</div>
        <div style={{
...
function VisualReset() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 14, borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)", alignItems: "center" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
        <div style={{ fontSize: 20, fontWeight: 900, color: RED, fontFamily: "monospace" }}>$0.42</div>
        <div style={{ fontSize: 18, color: "rgba(243,244,246,0.3)" }}>→</div>
        <button type="button" disabled style={{ padding: "5px 14px", borderRadius: 7, border: `1px solid ${ACCENT}40`, background: `${ACCENT}15`, color: ACCENT, fontSize: 11, fontWeight: 800, letterSpacing: "0.5px", textTransform: "uppercase" }}>Reset</button>
        <div style={{ fontSize: 18, color: "rgba(243,244,246,0.3)" }}>→</div>
        <div style={{ fontSize: 20, fontWeight: 900, color: GREEN, fontFamily: "monospace" }}>$1K</div>
      </div>
      <div style={{ fontSize: 10, color: "rgba(243,244,246,0.4)", letterSpacing: "0.4px", textTransform: "uppercase" }}>blank slate. no record kept.</div>
    </div>
  );
}
...
function VisualLobbyVsRoom() {
  return (
    <div style={{ padding: 14, borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,.25)" }}>
      <div style={{
        padding: "8px 12px", borderRadius: 10,
        border: `1px dashed ${ACCENT}55`, background: `${ACCENT}06`,
        fontSize: 11, fontWeight: 800, color: ACCENT, letterSpacing: "1px", textTransform: "uppercase",
      }}>FakeOut Lobby — everyone trading</div>
      <div style={{ paddingLeft: 16, marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
        {["Stirling's room · live signals", "Wall Street Crew · ranked", "Forex Desk · EUR/USD focus"].map(name => (
          <div key={name} style={{
...
const PANELS: PanelDef[] = [
  ...
  },
  {
    ...
    body: (
      <>
        <strong style={{ color: ACCENT }}>Casual — $100K.</strong> For fun. Tank the account, who cares.
        <br /><br />
        <strong style={{ color: ACCENT }}>Ranked — $1K.</strong> Counts. Pays 10× the Paper, climbs Notoriety XP, sits on a separate leaderboard people actually respect.
        <br /><br />
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <br /><br />
        Pick a timeframe — 1m for scalping, 1D if you like to look at it like a chart. The chart updates live off the Binance feed.
        <br /><br />
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <br /><br />
        Type a dollar amount. Hit BUY or SELL. The room sees your trade instantly — name, side, size, price — as a chip in chat. No bots. No copy-paste.
        <br /><br />
  ...
  },
  {
    ...
    body: (
      <>
        That scrolling bar at the top is the live leaderboard — top 8 traders by total PnL, refreshed every 5 seconds.
        <br /><br />
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <br /><br />
        <span style={{ opacity: 0.55, fontSize: 12 }}>Nothing personal. Or maybe a little personal.</span>
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        One button. Your account starts fresh at the mode&apos;s starting balance — $100K casual, $1K ranked.
        <br /><br />
        No log of the casualty. No badge of shame. Just a clean slate.
        <br /><br />
        <span style={{ opacity: 0.55, fontSize: 12 }}>The leaderboard remembers. The leaderboard always remembers.</span>
      </>
    ),
    visual: <VisualReset />,
  },
  {
    ...
    body: (
      <>
        ...
        <br /><br />
        <strong style={{ color: ACCENT }}>Rooms inside it</strong> are your crew, your stream, your strategy. Voice, screenshare, charts, paper-trade state — all native.
        <br /><br />
        Members trade alongside you. The chat sees the book. No 50-viewer cap.
        <br /><br />
        <span style={{ opacity: 0.55, fontSize: 12 }}>Discord&apos;s at the door. You&apos;re inside.</span>
      </>
    ),
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "22px 24px 18px", display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            <div style={{
              fontSize: 22, fontWeight: 800, letterSpacing: "-0.4px", marginBottom: 10,
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                border: `1px solid ${ACCENT}50`, background: `${ACCENT}18`,
                color: ACCENT, fontSize: 12, fontWeight: 800, letterSpacing: "0.5px", cursor: "pointer",
                textTransform: "uppercase",
              }}
            >{isLast ? "Step onto the floor" : "Next"}</button>
          </div>
        </div>
      </div>
    </div>
