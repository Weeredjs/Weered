...
export default function ModuleTabBar({
  ...
  const corona = `conic-gradient(from 12deg at 50% 55%, transparent 0 6deg, ${a(.95)} 9deg, transparent 13deg 41deg, ${a(.72)} 45deg, transparent 49deg 96deg, ${a(.9)} 100deg, transparent 105deg 151deg, ${a(.64)} 155deg, transparent 160deg 211deg, ${a(.85)} 215deg, transparent 220deg 271deg, ${a(.7)} 275deg, transparent 280deg 331deg, ${a(.8)} 335deg, transparent 340deg 360deg)`;
  const crackMask = "conic-gradient(from 28deg at 50% 50%, #000 0deg, rgba(0,0,0,.28) 40deg, #000 92deg, rgba(0,0,0,.45) 150deg, #000 206deg, rgba(0,0,0,.3) 266deg, #000 320deg, rgba(0,0,0,.5) 360deg)";
            ...
            {on && (
              <>
                ...
                <span aria-hidden style={{
                  ...
                  clipPath: CLIP, background: a(.95),
                  WebkitMaskImage: crackMask, maskImage: crackMask,
                  animation: "wmtCrack 90s ease-in-out infinite",
                  pointerEvents: "none", zIndex: 1,
                }} />
                <span aria-hidden style={{
                  position: "absolute", top: -1, bottom: -1, left: -1, right: -1,
                  clipPath: CLIP, background: jamb,
