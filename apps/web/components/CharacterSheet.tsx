...
type Character = {
  ...
  derived: {
    ...
    skills: Array<{ skill: string; ability: string; proficient: boolean; expertise: boolean; bonus: number }>;
...
export default function CharacterSheet({ roomId, lobbyId }: { roomId: string; lobbyId: string }) {
  ...
  if (!token) {
    return <div style={{ padding: 24, textAlign: "center", opacity: 0.5 }}>Sign in to manage characters.</div>;
  }
  if (loading) {
    ...
    <div className="weered-dnd-modules" style={{ height: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {!asDM && (
            <button className="dnd-stone-tile" style={{ flex: "0 0 auto", fontSize: 12, padding: "6px 12px" }} onClick={openCreate}>
              + New
            </button>
          )}
        </div>
      ...
      {asDM && (
        <div style={{ flex: selected ? "0 0 auto" : 1, overflow: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
          {characters.length === 0 && (
            <div style={{ padding: 24, textAlign: "center", opacity: 0.5, fontFamily: "var(--font-cormorant), serif", fontStyle: "italic" }}>
    ...
    <div className="dnd-card" style={{
      ...
      <div onClick={onSelect} style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="dnd-heading" style={{ fontSize: 16, lineHeight: 1.1 }}>{c.name}</div>
      ...
      <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 8 }}>
        <button className="dnd-stone-tile dnd-stone-tile--dis" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 14 }}
          onClick={() => onHp(-1)}>−</button>
        <button className="dnd-stone-tile dnd-stone-tile--dis" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 11 }}
          onClick={() => onHp(-5)}>−5</button>
        <div style={{ flex: 1, height: 10, borderRadius: 4, background: "rgba(0,0,0,.4)", overflow: "hidden", border: "1px solid rgba(196,165,90,.20)" }}>
          <div style={{ height: "100%", width: `${pct}%`, background: hpColor, transition: "width .2s, background .2s" }} />
        </div>
        <button className="dnd-stone-tile dnd-stone-tile--adv" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 11 }}
          onClick={() => onHp(5)}>+5</button>
        <button className="dnd-stone-tile dnd-stone-tile--adv" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 14 }}
          onClick={() => onHp(1)}>+</button>
      </div>
    </div>
  );
    ...
    <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", gap: 14, paddingRight: 4 }}>
      ...
      <div className="dnd-card" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="dnd-heading" style={{ fontSize: 24, lineHeight: 1.0 }}>{c.name}</div>
            <div className="dnd-serif" style={{ fontSize: 13, opacity: 0.75 }}>
              Level {c.level} {c.race}{c.race && c.className ? " " : ""}{c.className}
              {c.alignment ? ` · ${c.alignment}` : ""}
            </div>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            {isOwner && <button className="dnd-stone-tile" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 11 }} onClick={onEdit}>Edit</button>}
            {isOwner && <button className="dnd-stone-tile dnd-stone-tile--dis" style={{ flex: "0 0 auto", padding: "4px 10px", fontSize: 11 }} onClick={onDelete}>Delete</button>}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 8 }}>
        <VitalCard label="Hit Points">
          <div style={{ fontSize: 22, fontWeight: 800, color: hpColor, fontFamily: "var(--font-pirata), serif", letterSpacing: 1 }}>
            {c.hpCurrent}<span style={{ opacity: 0.4 }}> / {c.hpMax}</span>
            {c.hpTemp > 0 && <span style={{ fontSize: 13, color: "#60a5fa", marginLeft: 6 }}>+{c.hpTemp}</span>}
          </div>
          <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
            <button className="dnd-stone-tile dnd-stone-tile--dis" style={{ flex: 1, padding: "4px 6px", fontSize: 12 }} onClick={() => onHp(-1)}>−1</button>
            <button className="dnd-stone-tile dnd-stone-tile--dis" style={{ flex: 1, padding: "4px 6px", fontSize: 11 }} onClick={() => onHp(-5)}>−5</button>
            <button className="dnd-stone-tile dnd-stone-tile--adv" style={{ flex: 1, padding: "4px 6px", fontSize: 11 }} onClick={() => onHp(5)}>+5</button>
            <button className="dnd-stone-tile dnd-stone-tile--adv" style={{ flex: 1, padding: "4px 6px", fontSize: 12 }} onClick={() => onHp(1)}>+1</button>
          </div>
        </VitalCard>
        <VitalCard label="Armor Class"><Big>{c.ac}</Big></VitalCard>
        <VitalCard label="Speed"><Big>{c.speed}<span style={{ fontSize: 13, opacity: 0.5 }}> ft</span></Big></VitalCard>
        <VitalCard label="Initiative">
          <button className="dnd-stone-tile" style={{ width: "100%", padding: "10px 0" }}
        ...
        <VitalCard label="Proficiency"><Big>+{c.derived.proficiencyBonus}</Big></VitalCard>
        <VitalCard label="Passive Perception"><Big>{c.derived.passivePerception}</Big></VitalCard>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 6 }}>
          {(["STR","DEX","CON","INT","WIS","CHA"] as const).map(k => {
            ...
            return (
              <button key={k} className="dnd-stone-tile" style={{ flex: "0 0 auto", padding: "10px 4px", textAlign: "center" }}
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            {c.derived.saves.map(s => (
              <button key={s.ability} className="dnd-stone-tile" style={{
                flex: "0 0 auto", padding: "5px 10px", display: "flex", justifyContent: "space-between",
                alignItems: "center", textAlign: "left", fontSize: 12,
              }}
                onClick={() => onRoll(`${ABILITY_LABEL[s.ability]} save`, `1d20${modSign(s.bonus)}`)}>
                <span>
                  <span style={{
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {c.derived.skills.map(s => (
              <button key={s.skill} className="dnd-stone-tile" style={{
                flex: "0 0 auto", padding: "4px 10px", display: "flex", justifyContent: "space-between",
                alignItems: "center", textAlign: "left", fontSize: 12,
              }}
                onClick={() => onRoll(`${SKILL_LABEL[s.skill]} (${s.ability})`, `1d20${modSign(s.bonus)}`)}>
                <span>
                  <span style={{
                    display: "inline-block", width: 9, height: 9, borderRadius: "50%",
                    border: `1px solid ${ACCENT}77`,
            ...
            {c.attacks.map((a, i) => (
              <div key={i} className="dnd-card" style={{ padding: "8px 10px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="dnd-heading" style={{ fontSize: 14 }}>{a.name}</div>
      ...
      {Object.keys(c.spellSlots || {}).length > 0 && (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            {Object.keys(c.spellSlots).sort().map(level => {
              const slot = c.spellSlots[level];
              if (!slot || slot.max <= 0) return null;
              return (
                <div key={level} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontFamily: "var(--font-pirata), serif", fontSize: 14, color: "#F5D58A", minWidth: 36 }}>
                    L{level}
                  </span>
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {Array.from({ length: slot.max }).map((_, i) => {
                      ...
                      return (
                        <button key={i} onClick={() => onToggleSlot(level, i)} disabled={!isOwner}
                          style={{
      ...
      {c.spells.length > 0 && (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {[...c.spells].sort((a, b) => a.level - b.level || a.name.localeCompare(b.name)).map((s, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 8px", fontSize: 12 }}>
                <span style={{ fontFamily: "var(--font-cormorant), serif", color: "#F5D58A" }}>{s.name}</span>
                <span style={{ opacity: 0.55, fontSize: 11 }}>{s.level === 0 ? "cantrip" : `L${s.level}`}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {(["pp","gp","ep","sp","cp"] as const).map(k => (
            <div key={k} className="dnd-card" style={{ flex: 1, padding: "6px 8px", textAlign: "center" }}>
              <div style={{ fontSize: 10, opacity: 0.55, letterSpacing: 1.5 }}>{k.toUpperCase()}</div>
              <div style={{ fontFamily: "var(--font-pirata), serif", fontSize: 16, color: "#F5D58A" }}>{(c as any)[k]}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            {c.features.map((f, i) => (
              <div key={i} className="dnd-card" style={{ padding: "8px 10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <span className="dnd-heading" style={{ fontSize: 14 }}>{f.title}</span>
                  <span style={{ fontSize: 9, opacity: 0.5, letterSpacing: 1, textTransform: "uppercase" }}>{f.source}</span>
                </div>
                {f.body && <div style={{ fontSize: 12, opacity: 0.8, fontFamily: "var(--font-cormorant), serif", whiteSpace: "pre-wrap", marginTop: 4 }}>{f.body}</div>}
              </div>
            ))}
          </div>
        </div>
...
function VitalCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="dnd-card" style={{ padding: "8px 10px", textAlign: "center" }}>
      <div style={{ fontSize: 9, opacity: 0.55, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
      {children}
    </div>
  );
}
function Big({ children }: { children: React.ReactNode }) {
  return <div style={{ fontFamily: "var(--font-pirata), serif", fontSize: 24, color: "#F5D58A" }}>{children}</div>;
}
...
function NotesBlock({ c, isOwner, isDM }: { c: Character; isOwner: boolean; isDM: boolean }) {
  ...
  if (isOwner && c.notesPlayer) blocks.push({ label: "Player Notes (private)", body: c.notesPlayer });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {blocks.map((b, i) => (
          <div key={i} className="dnd-card" style={{ padding: "8px 10px" }}>
            <div style={{ fontSize: 10, opacity: 0.55, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>{b.label}</div>
            <div style={{ fontSize: 12, opacity: 0.85, fontFamily: "var(--font-cormorant), serif", whiteSpace: "pre-wrap" }}>{b.body}</div>
          </div>
        ))}
      </div>
    </div>
...
function RollPrompt({ label, expression, onCast, onCancel }: {
  ...
  return (
    <div onClick={onCancel} style={{
      ...
      <div onClick={e => e.stopPropagation()} className="dnd-card weered-dnd-modules" style={{
        padding: 24, minWidth: 280, maxWidth: 360, textAlign: "center",
      }}>
        <div className="dnd-section-label" style={{ marginBottom: 8 }}>Roll</div>
        <div className="dnd-heading" style={{ fontSize: 18, marginBottom: 6 }}>{label}</div>
        <div style={{ fontFamily: "var(--font-pirata), serif", fontSize: 32, color: "#F5D58A", letterSpacing: 2, marginBottom: 16 }}>
          {expression}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="dnd-stone-tile" style={{ flex: 1, padding: "10px 0" }} onClick={onCancel}>Cancel</button>
          <button className="dnd-stone-tile dnd-stone-tile--adv" style={{ flex: 2, padding: "10px 0", fontSize: 16 }} onClick={onCast}>
            Cast
          </button>
        </div>
      </div>
