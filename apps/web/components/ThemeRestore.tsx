...
export default function ThemeRestore() {
  ...
  useLayoutEffect(() => {
    try {
      ...
      let s: any = null;
      try { const raw = localStorage.getItem("weered:settings:v0"); s = raw ? JSON.parse(raw) : null; } catch {}
      const v2 = localStorage.getItem("weered_theme_v2");
      const theme = v2 && VALID_THEMES.includes(v2) ? v2
        : (s && VALID_THEMES.includes(s.theme) ? s.theme : "press");
