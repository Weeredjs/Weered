...
const LEVEL_STYLES: Record<string, { bg: string; border: string; color: string; icon: string; tag: string; tagColor: string }> = {
  info:    { bg: "rgba(30,14,58,.96)",  border: "rgba(124,58,237,.55)", color: "rgba(224,200,255,.96)", icon: "📢", tag: "ANNOUNCEMENT", tagColor: "#a78bfa" },
  warning: { bg: "rgba(54,38,8,.96)",   border: "rgba(245,158,11,.55)", color: "rgb(253,230,138)",     icon: "⚠️", tag: "MAINTENANCE",  tagColor: "#f5b700" },
  urgent:  { bg: "rgba(56,12,12,.96)",  border: "rgba(239,68,68,.6)",   color: "rgba(254,202,202,.98)", icon: "🚨", tag: "URGENT",       tagColor: "#fca5a5" },
};
...
export default function SystemBroadcast() {
  ...
  useEffect(() => {
    ...
    try { dismissedIds = JSON.parse(localStorage.getItem("weered:banner:dismissedIds") || "[]"); } catch {}
    fetch(`${API}/banner`).then(r => r.json()).then(j => {
      ...
      const shown = banners
        ...
        .map((b: any) => ({ id: b.id, message: b.message, level: b.level || "info", from: b.from || "Weered", ts: b.ts || 1, sticky: !!b.sticky }));
      if (shown.length) setMsgs(prev => {
  ...
  function dismiss(msg: BroadcastMsg) {
    if (msg.id) {
      try {
        const ids: string[] = JSON.parse(localStorage.getItem("weered:banner:dismissedIds") || "[]");
        if (!ids.includes(msg.id)) localStorage.setItem("weered:banner:dismissedIds", JSON.stringify([...ids, msg.id].slice(-50)));
      } catch {}
    }
      ...
      {msgs.map(msg => {
        ...
        return (
          ...
          >
            ...
            <span style={{
              flexShrink: 0, fontSize: 9, fontWeight: 900, letterSpacing: "1px",
              textTransform: "uppercase", color: s.tagColor,
              border: `1px solid ${s.border}`, borderRadius: 4,
              padding: "2px 7px", background: "rgba(0,0,0,.25)",
            }}>{s.tag}</span>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: `1px solid ${s.border}`, background: "rgba(0,0,0,.28)",
                color: s.color, fontSize: msg.sticky ? 15 : 11, fontWeight: 700,
                cursor: "pointer", opacity: 0.75, transition: "opacity .15s",
