...
export default function TournamentRoundRobin({
  ...
  React.useEffect(() => {
    ...
    fetch(`${API}/tournaments/${encodeURIComponent(tournamentId)}/standings`)
      .then((r) => r.json())
      .then((j) => {
        if (alive && j?.ok) setStandings(j.standings || []);
      })
      .catch(() => {});
    return () => {
      alive = false;
          ...
          <tbody>
            {standings.length === 0 ? (
              <tr>
                <td
                  colSpan={6}
                  style={{ padding: 20, textAlign: "center", color: "rgba(255,255,255,.4)" }}
                >
                  Standings will compute once results come in.
                </td>
              </tr>
            ) : (
              standings.map((s, idx) => (
                <tr
                  ...
                  style={{
                    ...
                    background: idx === 0 ? `${ACCENT}10` : idx < 3 ? `${ACCENT}05` : "transparent",
                  }}
                >
                  <Td
                    align="center"
                    style={{ fontWeight: 800, color: idx === 0 ? ACCENT : "rgba(255,255,255,.65)" }}
                  >
                    {s.rank}
                  </Td>
                  <Td>{s.displayName}</Td>
                  <Td align="center" style={{ color: "#4ade80", fontWeight: 700 }}>
                    {s.wins}
                  </Td>
                  <Td align="center" style={{ color: "#f87171" }}>
                    {s.losses}
                  </Td>
                  <Td
                    align="center"
                    style={{
                      color:
                        s.diff > 0 ? "#4ade80" : s.diff < 0 ? "#f87171" : "rgba(255,255,255,.6)",
                  ...
                  <Td
                    align="center"
                    style={{ fontFamily: "ui-monospace, monospace", color: "rgba(255,255,255,.7)" }}
                  >
                    {s.pointsFor}
    ...
    <div>
      <div
        style={{
          ...
          letterSpacing: 1.4,
          textTransform: "uppercase",
          color: muted ? "rgba(255,255,255,.4)" : ACCENT,
          marginBottom: 8,
        }}
      >
        {title}
