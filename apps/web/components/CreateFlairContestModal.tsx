    ...
    <div onClick={onClose} role="dialog" aria-modal="true" style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(8,5,16,.85)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: ACCENT, letterSpacing: 1, textTransform: "uppercase" }}>New Design Contest</div>
        ...
        <Field label="Title">
          <input value={title} onChange={e => setTitle(e.target.value.slice(0, 120))} placeholder="Crew Banner — Spring Open" style={inputS} />
        </Field>
        <Field label="Theme">
          <input value={theme} onChange={e => setTheme(e.target.value.slice(0, 200))} placeholder="What should artists design around?" style={inputS} />
        </Field>
        <Field label="Description">
          <textarea value={description} onChange={e => setDescription(e.target.value.slice(0, 2000))} placeholder="Rules, sizing notes, anything else." rows={3} style={{ ...inputS, resize: "vertical" }} />
        </Field>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <Field label="Submissions Open">
            <input type="datetime-local" value={submissionOpensAt} onChange={e => setSubOpens(e.target.value)} style={inputS} />
          </Field>
          <Field label="Submissions Close">
            <input type="datetime-local" value={submissionClosesAt} onChange={e => setSubCloses(e.target.value)} style={inputS} />
          </Field>
          <Field label="Voting Opens">
            <input type="datetime-local" value={voteOpensAt} onChange={e => setVoteOpens(e.target.value)} style={inputS} />
          </Field>
          <Field label="Voting Closes">
            <input type="datetime-local" value={voteClosesAt} onChange={e => setVoteCloses(e.target.value)} style={inputS} />
          </Field>
        </div>
        ...
        <button onClick={submit} disabled={busy || !valid} style={{
          ...
          background: valid && !busy ? `linear-gradient(135deg, ${ACCENT} 0%, #c4b5fd 100%)` : "rgba(255,255,255,.06)",
          ...
          cursor: valid && !busy ? "pointer" : "default", fontFamily: "inherit",
        }}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: 1.4, color: "rgba(196,181,253,.7)", textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
      {children}
    </div>
  );
}
