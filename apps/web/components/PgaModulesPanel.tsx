...
function formatScore(score: string | number | null | undefined): { text: string; color: string } {
  if (score === null || score === undefined || score === "" || score === "—")
    return { text: "—", color: "rgba(255,255,255,.5)" };
...
function HoleByHole({ holes, accentColor }: { holes: any[]; accentColor: string }) {
  if (!holes?.length)
    return (
      <div style={{ fontSize: 11, opacity: 0.35, padding: "4px 0" }}>No hole-by-hole data yet</div>
    );
  return (
          ...
          >
            ...
            <div
              style={{
                ...
                fontSize: 10,
                fontWeight: 700,
                color: bg === "rgba(255,255,255,.08)" ? "rgba(255,255,255,.6)" : bg,
              }}
            >
              {h.score}
            </div>
            <div style={{ fontSize: 7, opacity: 0.4, fontWeight: 600 }}>
              {tp === "E" || tp === "0" ? "" : tp}
            </div>
          </div>
        );
                    ...
                    >
                      ...
                      <td
                        style={{
                          ...
                          fontWeight: 600,
                          color:
                            thru === "F" || (thru || "").toLowerCase() === "finished"
                              ? "rgba(255,255,255,.4)"
                              : isCut
                                ? "rgba(239,68,68,.6)"
                                : "rgba(255,255,255,.6)",
...
function FieldIntel({ accentColor }: { accentColor: string }) {
  ...
  const fieldAvg =
    todayScores.length > 0
      ? (todayScores.reduce((s, v) => s + v, 0) / todayScores.length).toFixed(1)
      : "—";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
        <div style={{ ...S.card, textAlign: "center", padding: "10px 8px" }}>
          ...
          <div style={{ fontWeight: 900, fontSize: 20, fontVariantNumeric: "tabular-nums" }}>
            {fieldAvg}
          </div>
        </div>
        <div style={{ ...S.card, textAlign: "center", padding: "10px 8px" }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        {(["position", "today", "name"] as FieldSort[]).map((s) => (
          <button key={s} onClick={() => setSort(s)} style={sort === s ? S.btnPri(accent) : S.btn}>
      ...
      {field.length === 0 && <EmptyState title="No field data." />}
    </div>
  );
}
...
function PgaNews({ accentColor }: { accentColor: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {articles.length === 0 && <EmptyState title="No PGA news right now." />}
    </div>
  );
}
...
function GolfTwitchStreams({ lobbyId, accentColor }: { lobbyId?: string; accentColor?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {streams.length === 0 && <EmptyState compact title="Nobody streaming golf right now." />}
...
function PgaYouTube({ accentColor }: { accentColor?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            >
              ...
              <div style={{ padding: "8px 10px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 10, color: "rgba(148,163,184,.55)" }}>
                    {v.channelName}
                  </span>
      ...
      {videos.length === 0 && <EmptyState compact title="No videos found." />}
    </div>
  );
}
    ...
    >
      <ModuleTabBar
        tabs={TABS.map((t) => ({
          ...
          icon:
            t.icon === "yt" ? (
              <YouTubeIcon size={13} color={tab === t.id ? "#FF0000" : "rgba(148,163,184,.5)"} />
            ) : (
