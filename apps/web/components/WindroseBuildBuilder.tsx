...
export default function WindroseBuildBuilder({
  ...
  function addTag() {
    const t = tagInput.trim().toLowerCase().replace(/[^a-z0-9 -]/g, "").slice(0, 24);
  ...
  async function submit() {
    ...
    try {
      ...
      if (!tok) { setError("Sign in to file a log."); setSubmitting(false); return; }
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div>
            <div style={{
              ...
              fontSize: 26, color: "#e8c48a", letterSpacing: 1, lineHeight: 1,
            }}>File a Log</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(280px, 360px)", gap: 16 }}>
          <div>
            ...
            {images.length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginTop: 8 }}>
                {images.map((img, idx) => (
                  <div key={idx} style={{ position: "relative", paddingBottom: "66%", overflow: "hidden", borderRadius: 4, border: `1px solid ${ACCENT}55` }}>
                    <img src={img.previewUrl} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 60%, rgba(0,0,0,.7) 100%)", pointerEvents: "none" }} />
                    <div style={{ position: "absolute", left: 4, bottom: 4, fontSize: 9, fontWeight: 800, color: "#e8c48a", letterSpacing: 1 }}>
                      #{idx + 1}{idx === 0 ? " · COVER" : ""}
                    </div>
                    <div style={{ position: "absolute", right: 4, top: 4, display: "flex", gap: 3 }}>
                      {idx > 0 && <Mini onClick={() => reorder(idx, idx - 1)} title="Move up">↑</Mini>}
                      {idx < images.length - 1 && <Mini onClick={() => reorder(idx, idx + 1)} title="Move down">↓</Mini>}
                      <Mini onClick={() => removeAt(idx)} title="Remove" danger>✕</Mini>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <input
                ...
                style={inputStyle}
              />
              <div style={{ fontSize: 10, opacity: 0.5, marginTop: 2, textAlign: "right" }}>{title.length}/80</div>
            </div>
            ...
            <div style={{ marginTop: 10 }}>
              ...
              />
              <div style={{ fontSize: 10, opacity: 0.5, marginTop: 2, textAlign: "right" }}>{description.length}/4000</div>
            </div>

            <div style={{ marginTop: 10 }}>
              <Label>Tags <span style={{ opacity: 0.55, fontWeight: 400 }}>· up to 8</span></Label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 6 }}>
                {tags.map((t, i) => (
                  <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 3, background: `${ACCENT}26`, border: `1px solid ${ACCENT}55`, fontSize: 11, color: "#e8c48a" }}>
                    {t}
                    <button onClick={() => setTags(prev => prev.filter((_, j) => j !== i))} style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", padding: 0, fontSize: 12, lineHeight: 1 }}>×</button>
                  </span>
                ))}
              </div>
              <input
          ...
          <div>
            ...
            <label style={{
              ...
              <input type="checkbox" checked={watermarked} onChange={e => setWatermarked(e.target.checked)} />
              <span>Watermark with my handle (deters reposts)</span>
            </label>

            {error && (
              <div style={{ marginTop: 12, padding: "8px 10px", background: "rgba(185,28,28,.18)", border: "1px solid rgba(239,68,68,.4)", borderRadius: 3, color: "#fca5a5", fontSize: 11 }}>
                {error}
              </div>
            )}
            ...
            <button
              ...
              style={{
                ...
                fontFamily: "inherit",
                boxShadow: valid && !submitting ? "0 0 12px rgba(232,196,138,.25), inset 0 1px 0 rgba(255,255,255,.25)" : "none",
              }}
            >
...
function DropZone({ count, onPick, onClick, accent }: { count: number; onPick: (f: FileList) => void; onClick: () => void; accent: string }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, color: "rgba(228,212,176,.75)" }}>
        {remaining > 0 ? `Drop or click to add images (${remaining} slot${remaining === 1 ? "" : "s"} left)` : "Max 3 images per build"}
      </div>
    </div>
  );
}

function Selector({ value, setValue, options }: { value: string; setValue: (v: string) => void; options: string[] }) {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
      {options.map(o => {
        ...
        return (
          ...
          >{o.toLowerCase().replace(/^./, c => c.toUpperCase())}</button>
        );
      })}
    </div>
  );
