...
export default function TournamentMatchModal({
  ...
  const isParticipant =
    currentUserId && [match.entryA?.userId, match.entryB?.userId].includes(currentUserId);
  const canReport =
    (isParticipant || isStaff) && (match.status === "READY" || match.status === "LIVE");
  const canConfirm =
    match.status === "REPORTED" &&
    (isStaff || (currentUserId && currentUserId !== match.reportedById && isParticipant));
  const canDispute =
    match.status === "REPORTED" &&
    currentUserId &&
    currentUserId !== match.reportedById &&
    isParticipant;
  ...
  const winnerName =
    match.winnerEntryId === match.entryAId ? match.entryA?.displayName : match.entryB?.displayName;
    ...
    <Backdrop onClose={onClose}>
      <div
        ...
        style={{
          ...
          borderRadius: 6,
          boxShadow:
            "0 0 0 1px rgba(0,0,0,.5), 0 30px 80px rgba(0,0,0,.7), 0 0 30px rgba(245,130,32,.2)",
      ...
      >
        <div
          style={{
            padding: "14px 18px",
            borderBottom: "1px solid rgba(245,130,32,.18)",
            display: "flex",
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, marginTop: 2 }}>
              {match.entryA?.displayName || "TBD"} <span style={{ opacity: 0.4 }}>vs</span>{" "}
        ...
        {match.twitchLogin && isLive && (
          <div style={{ padding: 12, background: "#000" }}>
            ...
            >
              Streamed by{" "}
              <a
                href={`https://twitch.tv/${match.twitchLogin}`}
                target="_blank"
                rel="noreferrer"
        ...
        <div style={{ padding: 18 }}>
          ...
          {canReport && (
            ...
            >
              ...
              >
                <RadioBox
                  value={match.entryAId!}
                  picked={pickedWinner}
                  onPick={setPickedWinner}
                  label={match.entryA?.displayName || ""}
                />
                <input
                  type="number"
                  min={0}
              ...
              >
                <RadioBox
                  value={match.entryBId!}
                  picked={pickedWinner}
                  onPick={setPickedWinner}
                  label={match.entryB?.displayName || ""}
                />
              </div>
              <button
                onClick={() =>
                  call(`/tournaments/${tournamentId}/matches/${matchId}/report`, {
                    winnerEntryId: pickedWinner,
          ...
          {canConfirm && (
            ...
            >
                ...
                <div style={{ fontSize: 13 }}>
                  ...
                  <strong style={{ color: ACCENT }}>
                    {match.winnerEntryId === match.entryAId
                      ? match.entryA?.displayName
                      : match.entryB?.displayName}
                  </strong>
                </div>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                <button
                  onClick={() => call(`/tournaments/${tournamentId}/matches/${matchId}/confirm`)}
                  disabled={busy}
                ...
                {canDispute && (
                  <button
                    onClick={() => call(`/tournaments/${tournamentId}/matches/${matchId}/dispute`)}
                    disabled={busy}
                    style={dangerBtn}
                  >
                    Dispute
          ...
          {canMarkLive && !isLive && (
            <button
              onClick={() => call(`/tournaments/${tournamentId}/matches/${matchId}/live`)}
              disabled={busy}
              style={{ ...secondaryBtn, marginBottom: 12, width: "100%" }}
            >
              ● Mark Live
            </button>
          )}
                ...
                {!isFinal && match.entryA && match.entryB && (
                  <>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                      <button
                        onClick={() =>
                          call(`/tournaments/${tournamentId}/matches/${matchId}/admin-set`, {
                        ...
                        disabled={busy}
                        style={{ ...secondaryBtn, flex: 1, fontSize: 11 }}
                      >
