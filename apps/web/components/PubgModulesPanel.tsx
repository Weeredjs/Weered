...
const S = {
  ...
  btnPri: { padding: "6px 12px", borderRadius: 2, border: "1px solid rgba(255,170,0,.35)", background: "rgba(255,170,0,.12)", fontSize: 12, cursor: "pointer", color: "rgb(255,170,0)", fontWeight: 600 } as React.CSSProperties,
...
const MAP_NAMES: Record<string, string> = {
  ...
  DihorOtok_Main: "Vikendi", Tiger_Main: "Taego", Chimera_Main: "Deston",
  Kiki_Main: "Deston", Heaven_Main: "Haven", Summerland_Main: "Karakin",
  Range_Main: "Camp Jackal", Neon_Main: "Rondo",
};
function mapName(raw?: string): string { return MAP_NAMES[raw || ""] || raw || "Unknown"; }
...
const PUBG_MODES = ["Any", "Solo FPP", "Solo TPP", "Duo FPP", "Duo TPP", "Squad FPP", "Squad TPP", "Ranked"];
const PUBG_RANKS = ["Any", "Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master"];
const PUBG_REGIONS = ["Any", "NA", "EU", "AS", "OC", "SA", "SEA", "JP", "KR"];
const PUBG_TAGS = ["casual", "competitive", "chill", "sweaty", "mic-required", "no-mic", "18+", "ranked-grind"];
...
function TwitchStreams({ gameName, lobbyId, accentColor }: { gameName: string; lobbyId: string; accentColor: string }) {
  ...
  return (
    <>
      {activeStream && (
        <div style={{ ...S.card, padding: 0, marginBottom: 12, overflow: "hidden", border: `1px solid ${accentColor}55`, borderLeft: `2px solid ${accentColor}`, flexShrink: 0 }}>
          <iframe
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              ...
              <select style={{ ...S.select, width: "100%" }} value={platform} onChange={e => setPlatform(e.target.value)}>
                {["crossplay", "pc", "xbox", "psn"].map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div style={{ width: 80 }}>
...
function StatsLookup({ accent }: { accent: string }) {
  ...
  function StatBox({ label, value }: { label: string; value: string | number }) {
    return (
      <div style={{ flex: "1 1 80px", padding: "10px 12px", borderRadius: 2, border: `1px solid ${accent}20`, background: `${accent}06`, textAlign: "center" }}>
  ...
  function ModeRow({ label, data }: { label: string; data: any }) {
    ...
    return (
      <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: accent, width: 70, flexShrink: 0 }}>{label}</span>
  ...
  function totals(statsObj: any) {
    ...
    let wins = 0, kills = 0, rounds = 0, damage = 0, top10s = 0;
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <input style={{ ...S.input, flex: 1 }} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && lookup()} placeholder="PUBG username" />
      ...
      {stats && (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            <div style={{ width: 48, height: 48, borderRadius: 2, border: `2px solid ${accent}44`, background: `${accent}12`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 900, color: accent }}>
              {(stats.account?.name || "?")[0].toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "rgba(243,244,246,.95)" }}>{stats.account?.name}</div>
              <div style={{ fontSize: 11, color: "rgba(148,163,184,.5)", marginTop: 2 }}>{stats.account?.platform?.toUpperCase()} · {stats.season?.name || "Current Season"}</div>
            </div>
          </div>
          ...
          {lt && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
              ...
              <StatBox label="Win %" value={lt.winRate ? `${(lt.winRate * 100).toFixed(1)}%` : "—"} />
            </div>
          )}
...
function WeaponMastery({ accent }: { accent: string }) {
  ...
  async function lookup(name?: string) {
    const n = (name || query).trim(); if (!n) return;
    setLoading(true); setError(""); setWeapons([]);
  ...
  return (
    <div>
      ...
      {weapons.length > 0 && (
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: "rgba(243,244,246,.9)", marginBottom: 12 }}>{playerName}'s Arsenal</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {weapons.map((w, i) => (
              <div key={w.weapon} style={{
                ...S.card, display: "flex", alignItems: "center", gap: 10,
                border: i === 0 ? `1px solid ${accent}33` : undefined,
                background: i === 0 ? `${accent}08` : undefined,
              }}>
                <div style={{ width: 28, textAlign: "center", fontSize: 14, fontWeight: 900, color: i < 3 ? accent : "rgba(148,163,184,.4)" }}>
                  #{i + 1}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(243,244,246,.9)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {w.weapon.replace(/_/g, " ")}
                    {w.level > 0 && <span style={{ fontSize: 9, fontWeight: 700, marginLeft: 6, padding: "1px 5px", borderRadius: 3, background: `${accent}15`, color: accent }}>Lv.{w.level}</span>}
                  </div>
                  <div style={{ fontSize: 10, color: "rgba(148,163,184,.5)", marginTop: 2 }}>
                    {w.kills} kills · {w.damage.toLocaleString()} dmg · {w.headshots} HS · {w.longestKill}m longest
                  </div>
                </div>
                <div style={{ fontSize: 16, fontWeight: 900, color: accent, flexShrink: 0 }}>{w.kills}</div>
              </div>
            ))}
          </div>
        </div>
...
function RecentMatches({ accent }: { accent: string }) {
  ...
  async function lookup(name?: string) {
    const n = (name || query).trim(); if (!n) return;
    setLoading(true); setError(""); setMatchIds([]); setMatches([]);
    const j = await apiFetch(`/pubg/stats/${encodeURIComponent(n)}?platform=${platform}`);
  ...
  return (
    <div>
      ...
      {error && <div style={{ color: "#EF4444", fontSize: 12, marginBottom: 12 }}>{error}</div>}
      {loadingMatches && <div style={{ padding: 20, textAlign: "center", opacity: 0.4, fontSize: 13 }}>Loading matches...</div>}
      {matches.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {matches.map(m => {
            ...
            return (
              <div key={m.match?.id}>
                <div onClick={() => setExpanded(isExpanded ? null : m.match?.id)} style={{
                  ...
                  <div style={{
                    ...
                    background: isWin ? "rgba(34,197,94,.15)" : p?.rank && p.rank <= 10 ? `${accent}15` : "rgba(255,255,255,.04)",
                    color: isWin ? "#22C55E" : p?.rank && p.rank <= 10 ? accent : "rgba(148,163,184,.5)",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(243,244,246,.9)" }}>
                      ...
                      {isWin && <span style={{ marginLeft: 6, fontSize: 9, fontWeight: 800, padding: "1px 6px", borderRadius: 3, background: "rgba(34,197,94,.15)", color: "#22C55E" }}>WINNER</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "rgba(148,163,184,.5)", marginTop: 2 }}>
                      {p ? `${p.kills} kills · ${p.damage} dmg · ${p.assists} ast` : ""} · {m.match?.playerCount} players · {m.match?.duration}min
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div style={{ padding: "8px 12px", background: "rgba(0,0,0,.2)", borderRadius: "0 0 2px 2px", border: "1px solid rgba(255,255,255,.05)", borderTop: "none" }}>
                    <div style={{ ...S.label, marginBottom: 6 }}>TOP TEAMS</div>
                    {(m.teams || []).slice(0, 5).map((t: any, ti: number) => (
                      <div key={ti} style={{ padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,.03)", fontSize: 11 }}>
                        <span style={{ fontWeight: 700, color: t.rank === 1 ? "#22C55E" : accent, width: 28, display: "inline-block" }}>#{t.rank}</span>
                        {(t.members || []).map((mb: any, mi: number) => (
                          <span key={mi} style={{ color: mb.name?.toLowerCase() === playerName.toLowerCase() ? accent : "rgba(243,244,246,.7)", fontWeight: mb.name?.toLowerCase() === playerName.toLowerCase() ? 700 : 400 }}>
                            {mi > 0 ? ", " : ""}{mb.name} ({mb.kills}K/{mb.damage}dmg)
                          </span>
                        ))}
                      </div>
                    ))}
...
function Leaderboard({ accent }: { accent: string }) {
  ...
  async function load() {
    ...
    if (j.ok) { setPlayers(j.players || []); setSeason(j.season || ""); }
    else setError(j.error || "Failed to load");
    setLoading(false);
  }

  useEffect(() => { load(); }, [platform, mode]);
  ...
  return (
    <div>
      ...
      {error && <div style={{ color: "#EF4444", fontSize: 12, marginBottom: 12 }}>{error}</div>}
      {loading && players.length === 0 && <div style={{ padding: 20, textAlign: "center", opacity: 0.4, fontSize: 13 }}>Loading leaderboard...</div>}
      {players.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          <div style={{ display: "flex", gap: 4, padding: "6px 10px", fontSize: 9, fontWeight: 700, color: "rgba(148,163,184,.4)", letterSpacing: ".5px", textTransform: "uppercase" }}>
          ...
          {players.map((p, i) => (
            <div key={p.name + i} style={{
              ...S.card, display: "flex", gap: 4, alignItems: "center", padding: "8px 10px",
              border: i < 3 ? `1px solid ${accent}22` : undefined,
              background: i < 3 ? `${accent}06` : undefined,
            }}>
              <span style={{ width: 30, fontSize: 12, fontWeight: 900, color: i < 3 ? accent : "rgba(148,163,184,.4)" }}>
                {p.rank || i + 1}
              </span>
              ...
              <span style={{ width: 50, textAlign: "center", fontSize: 12, fontWeight: 600, color: "rgba(243,244,246,.6)" }}>{p.stats?.avgDmg}</span>
              <span style={{ width: 50, textAlign: "center", fontSize: 11, color: "rgba(148,163,184,.4)" }}>{p.stats?.games}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, ...style }}>
      ...
      <div style={{ padding: "6px 14px 8px", flexShrink: 0, borderTop: "1px solid rgba(255,255,255,.04)" }}>
        <p style={{ fontSize: 9, color: "rgba(100,116,139,.35)", lineHeight: 1.4, margin: 0, textAlign: "center" }}>
          PUBG, PLAYERUNKNOWN'S BATTLEGROUNDS and all related logos are trademarks of PUBG Corporation or its affiliates. Weered is not affiliated with, endorsed by, or sponsored by KRAFTON, Inc.
        </p>
      </div>
    </div>
  );
