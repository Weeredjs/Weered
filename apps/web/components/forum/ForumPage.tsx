...
import SectionSidebar, { SectionManageModal, type Section } from "./ForumSections";
...
export default function ForumPage({
  lobbyId,
  lobbyName,
}: { lobbyId?: string; lobbyName?: string } = {}) {
  ...
  const isStaff =
    !!me && (me.globalRole === "GOD" || me.globalRole === "ADMIN" || me.globalRole === "MOD");
  ...
  const loadSections = useCallback(async () => {
    ...
    const data = await apiFetch<any>(`/forum/sections?lobbyId=${encodeURIComponent(lobbyId)}`, {
      silent: true,
    });
  ...
  async function handleVote(postId: string, value: number) {
    ...
    if (data?.ok) {
      setPosts((prev) =>
        prev.map((p) => (p.id === postId ? { ...p, score: data.score, myVote: value } : p)),
      );
    }
  }
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              >
                {embedded
                  ? activeSection
                    ? `${activeSection.icon || ""} ${activeSection.name}`.trim()
                    : lobbyName || "Community Feed"
                  : "Forum"}
              </h1>
              {!embedded && (
          ...
          {me && (
            <button
              onClick={() => {
                setComposing(true);
                if (sectionId) setPostSection(sectionId);
              }}
              style={{
                padding: "8px 16px",
                borderRadius: 8,
          ...
          >
            ...
            <div style={{ marginBottom: 10 }}>
              <MarkdownComposer
                value={body}
                onChange={setBody}
                placeholder="Write your post... markdown, and drag or paste an image"
                maxLength={10000}
                rows={6}
              />
            </div>
                ...
                {embedded && sections.length > 0 ? (
                  ...
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {Object.entries(CATEGORY_CONFIG).map(([id, cfg]) => (
                      <button
                        key={id}
                        onClick={() => setPostCat(id)}
                        style={{
                          padding: "4px 10px",
                          borderRadius: 6,
                          border: `1px solid ${postCat === id ? cfg.color + "55" : "rgba(255,255,255,.06)"}`,
        ...
        {searchActive ? (
          ...
          <div style={{ textAlign: "center", padding: "56px 20px" }}>
            ...
            <div style={{ fontSize: 12, marginTop: 4, color: "rgba(148,163,184,.55)" }}>
              Post the first thread. Someone will pile in.
            </div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {posts.map((post) => {
              ...
              const tagList: string[] = Array.isArray(post.tags)
                ? post.tags.filter((t: any) => typeof t === "string")
                : [];
              return (
                <div
                  key={post.id}
                ...
                >
                  ...
                  >
                    <button
                      onClick={() => handleVote(post.id, post.myVote === 1 ? 0 : 1)}
                      style={{
                        background: "none",
                        border: "none",
                        cursor: "pointer",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {!embedded &&
                        (post.lobby ? (
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              setLobbyFilter({ id: post.lobby!.id, name: post.lobby!.name });
                            }}
                            onKeyDown={(e) => {
                            ...
                            role="button"
                            tabIndex={0}
                            title={`View ${post.lobby.name} forum`}
                            style={{
                              display: "inline-flex",
                              alignItems: "center",
                              gap: 4,
                              ...
                              whiteSpace: "nowrap",
                              cursor: "pointer",
                              background: post.lobby.accentColor
                    ...
                    <div
                      style={{ fontSize: 14, fontWeight: 700, lineHeight: 1.35, marginBottom: 6 }}
                    >
                      {post.title}
                    </div>
                    <div
...
function SearchResultsView({
  ...
  if (loading)
    return (
      <div style={{ padding: "20px 0", color: "rgba(255,255,255,.5)", fontSize: 12 }}>
        Searching...
      </div>
    );
