...
export default function BookmarksPage() {
  ...
  useEffect(() => {
    if (!me) { router.push("/forum"); return; }
    (async () => {
      setLoading(true);
  ...
  return (
    <div style={{
      maxWidth: 780, margin: "0 auto", padding: "20px 16px 60px", fontFamily: FONT,
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, margin: 0, letterSpacing: "-0.3px" }}>Saved posts</h1>
      ...
      {loading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[1,2,3].map(i => (
            <div key={i} style={{ height: 72, borderRadius: 10, background: "rgba(255,255,255,.03)" }} />
          ))}
        </div>
      ) : posts.length === 0 ? (
        <div style={{ textAlign: "center", padding: "56px 20px" }}>
          <div style={{ fontSize: 26, marginBottom: 10, opacity: 0.35 }}>★</div>
          <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(243,244,246,.75)" }}>Nothing saved yet.</div>
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); handleUnbookmark(post.id); }}
                      title="Unsave"
                      style={{
                        marginLeft: "auto", background: "none", border: "none", padding: 2,
                        color: "#fbbf24", fontSize: 14, cursor: "pointer", fontFamily: "inherit", lineHeight: 1,
                      }}>★</button>
                  </div>
