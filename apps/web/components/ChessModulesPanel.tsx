...
function ChessStreams() {
  ...
  useEffect(() => {
    fetch(`${API}/twitch/streams?game=Chess`)
      ...
      .then((j) => {
        if (j?.streams) setStreams(j.streams);
      })
      .catch(() => {})
  ...
  if (loading)
    return (
      <div style={{ color: "rgba(255,255,255,.4)", textAlign: "center", padding: 30 }}>
        Loading streams…
      </div>
    );
  if (streams.length === 0)
    return (
      <div style={{ color: "rgba(255,255,255,.4)", textAlign: "center", padding: 30 }}>
        No chess streams live right now. Check back later.
      </div>
    );
  ...
  return (
    ...
    >
      {streams.map((s: any) => (
        <a
          key={s.id || s.user_name}
          href={`https://twitch.tv/${s.user_login || s.user_name}`}
          target="_blank"
          rel="noreferrer"
          style={{
            textDecoration: "none",
        ...
        >
          {s.thumbnail_url && (
            <img
              src={String(s.thumbnail_url).replace("{width}", "320").replace("{height}", "180")}
              alt=""
              style={{ width: "100%", display: "block" }}
            />
          )}
          <div style={{ padding: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,.92)" }}>
            ...
            <div
              style={{ fontSize: 9, color: "rgba(124,58,237,.8)", marginTop: 4, fontWeight: 700 }}
            >
...
function ChessChallenges({ lobbyId }: { lobbyId: string }) {
  ...
  const refresh = useCallback(() => {
    ...
    fetch(`${API}/challenges?lobbyId=${encodeURIComponent(lobbyId)}`, { headers: tokenHeader() })
      .then((r) => r.json())
      .then((j) => {
        if (j?.ok && Array.isArray(j.challenges)) setItems(j.challenges);
      })
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [lobbyId]);
      ...
      {items.map((c: any) => {
        ...
        return (
          <div
            ...
            style={{
              padding: 12,
              background: done ? "rgba(34,197,94,.06)" : "rgba(255,255,255,.03)",
          ...
          >
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(255,255,255,.95)" }}>
                {def.title}
              </div>
              <div
                style={{
            ...
            >
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.5)" }}>
                {enr
                  ? done
                    ? `✓ Completed ${enr.completedAt ? new Date(enr.completedAt).toLocaleDateString() : ""}`
                    : `Progress: ${JSON.stringify(enr.progress || {}).slice(0, 80)}`
                  : "Not enrolled"}
              </div>
              {!enr && c.instanceId && (
                <button
                  onClick={() => enroll(c.instanceId)}
                  style={{
                    padding: "4px 10px",
                    fontSize: 10,
                    fontWeight: 800,
  ...
  if (noneLinked) {
    return (
      <div style={{ textAlign: "center", padding: 30, lineHeight: 1.6 }}>
        <div
          style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,.85)", marginBottom: 8 }}
        >
          No chess account linked yet
        </div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,.55)" }}>
          Go to Settings → External Integrations and link your Lichess or Chess.com username. We
          poll your recent games every 5 minutes and credit Weered chess challenges/tournaments
          automatically.
        </div>
      </div>
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {data.lichess && (
        <div
          style={{
            ...
            border: "1px solid rgba(124,58,237,.25)",
            borderLeft: `2px solid ${ACCENT_CHESS}`,
            borderRadius: 2,
          }}
        >
          <div
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 800, color: "rgba(196,181,253,.95)" }}>
              Lichess
            </div>
            <a
              href={data.lichess.url}
              target="_blank"
              rel="noreferrer"
              style={{ fontSize: 11, color: "rgba(124,58,237,.85)" }}
            >
              @{data.lichess.username}
            </a>
          </div>
          {data.lichess.perfs && (
            <div
            ...
            >
              {Object.entries(data.lichess.perfs).map(([perf, v]: any) => (
                <div
                  key={perf}
                  style={{ padding: 8, background: "rgba(255,255,255,.04)", borderRadius: 2 }}
                >
                  <div
                    style={{
                      fontSize: 9,
      ...
      {data.chessCom && (
        ...
        >
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 800, color: "rgba(134,239,172,.95)" }}>
              Chess.com
            </div>
            <a
              href={data.chessCom.url}
              target="_blank"
              rel="noreferrer"
              style={{ fontSize: 11, color: "rgba(34,197,94,.85)" }}
            >
              @{data.chessCom.username}
            </a>
          </div>
          <div
            style={{
          ...
          >
            {Object.entries(data.chessCom.ratings).map(
              ([k, v]: any) =>
                v != null && (
                  <div
                    key={k}
...
function ChessAudit() {
  ...
  if (games.length === 0)
    return (
      <div
        style={{ color: "rgba(255,255,255,.4)", textAlign: "center", padding: 30, lineHeight: 1.6 }}
      >
        No games ingested yet.
        <br />
        <span style={{ fontSize: 11, opacity: 0.7 }}>
          Link your Lichess / Chess.com in Settings. Worker polls every 5 minutes — first poll fires
          ~30s after server start.
        </span>
      </div>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ fontSize: 11, color: "rgba(255,255,255,.55)", marginBottom: 4 }}>
        Last {games.length} games — full metadata from the provider. Source of truth.
      </div>
      {games.map((g: any) => {
          ...
          >
            ...
            >
              <span
                style={{
                  color:
                    g.provider === "LICHESS" ? "rgba(196,181,253,.85)" : "rgba(134,239,172,.85)",
                  fontWeight: 700,
                }}
              >
                {g.provider}
              ...
              {g.rating && (
                <span>
                  my rating: {g.rating}
                  {typeof g.ratingDiff === "number" && g.ratingDiff !== 0
