...
export default function GroupsTab({
  ...
  const reloadMessages = useCallback(
    async (id: string) => {
      ...
      setLoadingMsgs(true);
      try {
        const r = await fetch(`${apiBase}/groups/${encodeURIComponent(id)}/messages`, {
          headers: auth,
        });
        const j = await r.json();
        if (j?.ok) {
  ...
  useEffect(() => {
    ...
    try {
      window.dispatchEvent(new CustomEvent("weered:groups:unread", { detail: { count: total } }));
    } catch {}
  }, [threads]);
  ...
  useEffect(() => {
    ...
    const onEdited = (e: Event) => {
      ...
      setMessages((cur) =>
        cur.map((x) => (x.id === d.msgId ? { ...x, body: d.body, editedAt: d.editedAt } : x)),
      );
    };
    const onDeleted = (e: Event) => {
      ...
      if (d.threadId !== activeId) return;
      setMessages((cur) =>
        cur.map((x) => (x.id === d.msgId ? { ...x, body: "", deletedAt: d.deletedAt } : x)),
      );
    };
    ...
    const onRenamed = (e: Event) => {
      ...
      setThreads((cur) =>
        cur.map((t) => (t.id === d.threadId ? { ...t, name: d.name || null } : t)),
      );
    };
    const onMembers = () => {
      void reloadThreads();
  ...
  const commitEdit = async () => {
    ...
    try {
      await fetch(
        `${apiBase}/groups/${encodeURIComponent(activeId)}/messages/${encodeURIComponent(id)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json", ...auth },
  ...
  const deleteMsg = async (msgId: string) => {
    ...
    try {
      await fetch(
        `${apiBase}/groups/${encodeURIComponent(activeId)}/messages/${encodeURIComponent(msgId)}`,
        {
          method: "DELETE",
          headers: auth,
        },
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, height: "100%" }}>
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: "var(--weered-muted)" }}>
          ...
          >
            ...
            >
              ...
              <path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 11-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 11-4 0v-.09A1.65 1.65 0 008.07 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 11-2.83-2.83l.06-.06A1.65 1.65 0 004.6 15a1.65 1.65 0 00-1.51-1H3a2 2 0 110-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06A1.65 1.65 0 009 4.6a1.65 1.65 0 001-1.51V3a2 2 0 114 0v.09c0 .67.4 1.27 1 1.51a1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06a1.65 1.65 0 00-.33 1.82c.24.6.84 1 1.51 1H21a2 2 0 110 4h-.09c-.67 0-1.27.4-1.51 1z" />
            </svg>
          </button>
        </div>
              ...
              return (
                ...
                >
                  ...
                  {m.replyToId && !isDeleted && (
                    ...
                    >
                      <span style={{ color: "var(--weered-accent-text)", fontWeight: 700 }}>
                        ↩ {m.replyToUserName || "?"}
                      </span>
                      <span
                        style={{
                  ...
                  >
                    ...
                    {isEdited && (
                      <span style={{ fontSize: 9, color: "var(--weered-muted)" }}>(edited)</span>
                    )}
                  </div>
        ...
        >
          {replyTo && (
            ...
            >
              <span style={{ color: "var(--weered-accent-text)", fontWeight: 700, flexShrink: 0 }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    {time && (
                      <span style={{ fontSize: 10, color: "var(--weered-muted)", flexShrink: 0 }}>
                        {time}
                      </span>
                    )}
                  </div>
          ...
          <div
            ...
            style={{
              ...
              background: m.avatar
                ...
                : "linear-gradient(135deg,#7c3aed,#5800e5)",
...
function CreateGroupView({
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/friends`, { headers: auth })
      .then((r) => r.json())
      .then((j) => {
        if (Array.isArray(j?.friends)) setFriends(j.friends);
      })
      .catch(() => {});
  }, [apiBase, token, auth]);
  ...
  const create = async () => {
    setErr("");
    if (picked.size === 0) {
      setErr("Pick at least one friend.");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, height: "100%" }}>
      ...
      >
        ...
        <div style={{ flex: 1, fontWeight: 700, fontSize: 14, color: "var(--weered-text)" }}>
          New group
        </div>
        <button
          ...
          style={{
            ...
            border: "1px solid var(--weered-accent-ring)",
            background:
              busy || picked.size === 0 ? "rgba(255,255,255,.05)" : "var(--weered-accent-bg)",
            color: "var(--weered-accent-text)",
            cursor: busy || picked.size === 0 ? "not-allowed" : "pointer",
            fontSize: 12,
            fontWeight: 800,
            fontFamily: "inherit",
            opacity: busy || picked.size === 0 ? 0.55 : 1,
          }}
        >
          Create
        </button>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  borderBottom: "1px solid rgba(255,255,255,.04)",
                  background: on ? "rgba(124,58,237,.10)" : "transparent",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  gap: 12,
                ...
                <div
                  style={{
                    ...
                    background: f.avatar
                      ...
                      : f.avatarColor || "linear-gradient(135deg,#7c3aed,#5800e5)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: 13,
...
function SettingsView({
  ...
  const kick = async (uid: string) => {
    ...
    try {
      await fetch(
        `${apiBase}/groups/${encodeURIComponent(thread.id)}/members/${encodeURIComponent(uid)}`,
        {
          method: "DELETE",
          headers: auth,
        },
  ...
  const leave = async () => {
    ...
    try {
      await fetch(
        `${apiBase}/groups/${encodeURIComponent(thread.id)}/members/${encodeURIComponent(meId)}`,
        {
          method: "DELETE",
          headers: auth,
        },
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, height: "100%" }}>
      ...
      >
        <div>
            ...
            {isOwner && (
              <button
                ...
                style={{
                  ...
                  fontWeight: 800,
                  fontFamily: "inherit",
                  opacity: name.trim() === (thread.name || "") ? 0.55 : 1,
                }}
              >
                Save
              </button>
          ...
          {err && (
            <div style={{ fontSize: 11, color: "rgba(239,68,68,.85)", marginTop: 6 }}>{err}</div>
          )}
        </div>
