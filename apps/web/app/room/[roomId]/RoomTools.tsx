...
export default function RoomTools({ roomId }: { roomId: string }) {
  ...
  const canChat = useMemo(
    () => roomId && joinedRoomId === roomId && joinStatus === "joined",
    [roomId, joinedRoomId, joinStatus],
  );
      ...
      >
        <div>
          <div style={{ fontWeight: 900, fontSize: 14 }}>
            {meta?.name || roomId}
            {meta?.locked ? (
              <span style={{ marginLeft: 8, ...pill, borderColor: "#f3c" }}>locked</span>
            ) : null}
          </div>
          <div style={{ fontSize: 12, color: "#666", marginTop: 4 }}>
            status: <b>{joinStatus}</b> • role: <b>{role}</b> • chat:{" "}
        ...
        {isMod ? (
          <div style={{ display: "flex", gap: 8 }}>
            {meta?.locked ? (
              <button style={btn} onClick={unlockRoom}>
                Unlock
              </button>
            ) : (
              <button style={btn} onClick={lockRoom}>
                Lock
              </button>
            )}
          </div>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
            <div style={{ ...card, padding: 10 }}>
              ...
              <div style={{ fontSize: 12, color: "#666", marginTop: 4 }}>
                {admin?.knocks?.length ?? 0}
              </div>
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                {(admin?.knocks || [])
                  .slice(-10)
                  .reverse()
                  .map((k: { userId: string; name: string; ts: number }) => (
                    <div
                      key={k.userId}
                      style={{
                        border: "1px solid var(--weered-border)",
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button style={btn} onClick={() => admit(k.userId)}>
                          Admit
                        </button>
                        <button style={btn} onClick={() => deny(k.userId)}>
                          Deny
                        </button>
                      </div>
                    </div>
                ...
                {(admin?.banned || [])
                  ...
                  .map((id) => (
                    ...
                    >
                      ...
                      <button style={btn} onClick={() => unban(id)}>
                        Unban
                      </button>
                    </div>
                  ))}
