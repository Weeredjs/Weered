...
const KINDS: { id: Kind; label: string; hint: string }[] = [
  ...
  {
    id: "KARMA_MIN",
    label: "Min lobby karma",
    hint: "Trigger when author karma is below threshold.",
  },
  {
    id: "ACCOUNT_AGE_MIN",
    label: "Min account age",
    hint: "Trigger when author account is too new.",
  },
];
const ACTIONS: { id: Action; label: string; hint: string }[] = [
  ...
  { id: "SHADOW_REMOVE", label: "Shadow remove", hint: "Author sees it; nobody else does." },
  {
    ...
    hint: "Flag for mod-queue review; content stays live.",
  },
];
...
export default function AutoModAdminPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setForbidden(false);
    const data = await apiFetch(`/forum/automod?lobbyId=${encodeURIComponent(lobbyId)}`, {
      silent: true,
    });
  ...
  async function deleteRule(rule: Rule) {
    const ok = await weeredConfirm({
      ...
      body: "Existing reports stay; future content won't be checked.",
      confirmLabel: "Delete",
      destructive: true,
    });
    if (!ok) return;
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 13, opacity: 0.6, marginTop: 6 }}>
          Only lobby owners or staff can configure auto-mod.
        </div>
        <button onClick={() => router.push(`/lobby/${lobbyId}`)} style={btnStyle()}>
          &larr; Back to lobby
        </button>
      </div>
    );
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 11, opacity: 0.4, marginTop: 2 }}>
            Automatic moderation for posts and comments in this lobby.
          </div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => router.push(`/lobby/${lobbyId}/admin`)} style={btnStyle()}>
            &larr; Admin
          </button>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 20px", color: "rgba(148,163,184,.55)" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(243,244,246,.7)" }}>
            No rules yet.
          </div>
          <div style={{ fontSize: 12, marginTop: 4 }}>
            Add one to start filtering content automatically.
          </div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
                  <button onClick={() => setEditing(rule)} style={btnStyle()}>
                    Edit
                  </button>
                  <button onClick={() => deleteRule(rule)} style={btnStyle("danger")}>
                    Delete
                  </button>
                </div>
              </div>
...
function summarizeConfig(kind: Kind, config: any): string {
  ...
  if (kind === "LINK_BLOCK")
    return `allowed: ${(config.allowedDomains || []).join(", ") || "(none)"}`;
...
function RuleEditor({
  ...
  async function save() {
    if (!name.trim()) {
      weeredToast.error("Name required");
      return;
    }
    setSubmitting(true);
    ...
    setSubmitting(false);
    if (data?.ok) {
      weeredToast.success(rule ? "Rule updated" : "Rule created");
      onSaved();
    }
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 18 }}>
          <button onClick={onClose} disabled={submitting} style={btnStyle()}>
            Cancel
          </button>
          <button onClick={save} disabled={submitting} style={btnStyle("primary")}>
...
function ConfigForm({
    ...
    return (
      <>
        <input
          ...
          placeholder="Pattern (e.g. (viagra|cialis))"
          style={inputStyle}
        />
        <input
          value={value?.flags || ""}
          onChange={(e) => onChange({ ...value, flags: e.target.value })}
          placeholder="Flags (default: i)"
  ...
  if (kind === "LINK_BLOCK") {
    const v = Array.isArray(value?.allowedDomains)
      ? value.allowedDomains.join(", ")
      : value?.allowedDomains || "";
    return (
      <textarea
        value={v}
        onChange={(e) => onChange({ ...value, allowedDomains: e.target.value })}
        placeholder="Allowed domains, comma-separated (e.g. youtube.com, twitch.tv)"
        rows={3}
        style={{ ...inputStyle, resize: "vertical" }}
      />
    );
  ...
  if (kind === "KARMA_MIN") {
    return (
      <input
        ...
        placeholder="Minimum lobby karma"
        style={inputStyle}
      />
    );
  }
  if (kind === "ACCOUNT_AGE_MIN") {
    return (
      <input
        type="number"
        value={value?.minDays ?? 0}
        onChange={(e) => onChange({ ...value, minDays: e.target.value })}
        placeholder="Minimum account age (days)"
        style={inputStyle}
      />
    );
  }
