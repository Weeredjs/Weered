...
const TOOLKIT_SECTIONS = [
  ...
  },
  {
    ...
    features: ["5-level role system with custom names", "Per-room lock & chat lock", "Kick / ban with reasons", "Full audit log of all actions", "Role-based admin panel access"],
  },
  {
  ...
  },
  {
    ...
    features: ["Event creation & scheduling", "Category tagging", "Global promotion pipeline", "Cover images & descriptions"],
  },
  {
  ...
  },
  {
    ...
    features: ["Custom paid tiers (Stripe)", "Auto role-grant on subscribe", "Billing portal for members", "Revenue share configuration", "Tier badge colors"],
  },
];
...
function AnnouncementCard({ a }: { a: typeof ANNOUNCEMENTS[0] }) {
  return (
    <div style={{
      flex: "1 1 220px", maxWidth: 340, padding: "14px 16px",
      borderRadius: 12,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(243,244,246,0.9)" }}>{a.title}</span>
      </div>
      <p style={{ fontSize: 11, lineHeight: 1.5, color: "rgba(148,163,184,0.7)", margin: 0 }}>{a.body}</p>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(243,244,246,0.9)", lineHeight: 1.3 }}>{ev.title}</div>
        {ev.lobby && (
          <div style={{ fontSize: 10, color: "rgba(148,163,184,0.5)", marginTop: 2 }}>{ev.lobby.name}</div>
        )}
      </div>
      <div style={{
        fontSize: 10, fontWeight: 700, color: ac,
        background: `${ac}18`, padding: "3px 8px", borderRadius: 6,
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {lobby.verified && (
              <span style={{ fontSize: 9, fontWeight: 800, color: "#22C55E", background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.25)", padding: "1px 5px", borderRadius: 4 }}>VERIFIED</span>
            )}
            {showRole && lobby.roleLevel && lobby.roleLevel >= 4 && (
              <span style={{ fontSize: 9, fontWeight: 700, color: ac, background: `${ac}18`, padding: "1px 5px", borderRadius: 4 }}>OWNER</span>
            )}
          </div>
          <div style={{
            fontSize: 11, color: "rgba(148,163,184,0.5)", marginTop: 2,
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ fontSize: 11, color: hasOnline ? "rgba(34,197,94,0.8)" : "rgba(100,116,139,0.4)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "rgba(243,244,246,0.95)" }}>{section.title}</div>
          <div style={{ fontSize: 11, color: "rgba(148,163,184,0.55)", marginTop: 2, lineHeight: 1.4 }}>{section.desc}</div>
        </div>
        <span style={{
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <div style={{
          fontSize: 22, fontWeight: 800, color: "rgba(243,244,246,0.95)", marginTop: 4,
        }}>
          {tier.price}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {tier.features.map((f, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 7, fontSize: 11, color: "rgba(226,232,240,0.75)", lineHeight: 1.4 }}>
      ...
      {!isFree && (
        <Link href="/settings" style={{ textDecoration: "none" }}>
          <button style={{
            ...
            color: "rgba(243,244,246,0.95)", fontSize: 12, fontWeight: 700,
...
export default function LobbyPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/lobbies`).then(r => r.json()).catch(() => ({ ok: false })),
      fetch(`${API}/me/lobbies`, { headers: authHeaders() }).then(r => r.json()).catch(() => ({ ok: false })),
      fetch(`${API}/events/upcoming?limit=5`).then(r => r.json()).catch(() => ({ ok: false })),
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12, height: "calc(100vh - 28px)", minHeight: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "20px 24px 40px" }}>
          ...
          {loading ? (
            <div style={{ padding: "60px 0", textAlign: "center", color: "rgba(100,116,139,0.4)", fontSize: 12, letterSpacing: "0.08em" }}>
              LOADING...
            </div>
          ) : (
            <>
              ...
              <div className="lobby-section">
                ...
                {events.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(148,163,184,0.45)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>
                      UPCOMING EVENTS
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {loggedIn && (
                <div className="lobby-section">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {myLobbies.map(l => <LobbyCardComp key={l.id} lobby={l} showRole />)}
                  </div>
                </div>
              )}
              ...
              <div className="lobby-section">
                ...
                {allLobbies.length === 0 ? (
                  <div style={{ padding: "20px 0", textAlign: "center", color: "rgba(100,116,139,0.4)", fontSize: 12 }}>
                    No lobbies yet. Be the first to create one.
                  </div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {allLobbies
                      .sort((a, b) => b.onlineCount - a.onlineCount || b._count.members - a._count.members)
                      .map(l => <LobbyCardComp key={l.id} lobby={l} />)
                    }
                  </div>
                )}
              </div>
              ...
              <div className="lobby-section">
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div className="tier-row" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    {PLATFORM_TIERS.map(t => <TierColumn key={t.name} tier={t} />)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {TOOLKIT_SECTIONS.map(s => <ToolkitCard key={s.title} section={s} />)}
                  </div>
                </div>
              </div>
            </>
