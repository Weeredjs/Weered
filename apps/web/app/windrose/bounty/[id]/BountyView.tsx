...
export default function BountyView({ id, initial }: { id: string; initial: Bounty | null }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/windrose/bounties/${encodeURIComponent(id)}`)
      ...
      .then(j => {
        if (cancelled) return;
        if (j?.ok && j.bounty) { setB(j.bounty); setErr(null); }
        else if (!initial) setErr("Bounty not found.");
        setLoading(false);
      })
      .catch(() => { if (!cancelled) { setErr("Couldn't load this bounty."); setLoading(false); } });
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        {loading ? (
          <Plaque><div style={{ fontSize: 14, color: PAL.parchDim, fontStyle: "italic" }}>Digging through the ledger…</div></Plaque>
        ) : err || !b ? (
          <Plaque>
            <h1 style={{ fontFamily: WR_DISPLAY, fontSize: 32, color: PAL.brassHi, margin: 0 }}>Bounty not found</h1>
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{
                fontFamily: WR_MONO, fontSize: 10, fontWeight: 800,
                letterSpacing: "6px", textTransform: "uppercase",
                color: PAL.blood, marginBottom: 10,
              }}>
                ☠ Wanted ☠
              </div>
              <h1 style={{
                fontFamily: WR_DISPLAY, fontSize: 56, lineHeight: 1,
                color: PAL.brassHi, margin: "6px 0 16px",
                letterSpacing: "1px",
              ...
              <div style={{
                ...
                <div style={{
                  fontFamily: WR_DISPLAY, fontSize: 54, lineHeight: 1,
                  color: PAL.brassHi, fontVariantNumeric: "tabular-nums",
                }}>
                  {b.amount.toLocaleString()}
                </div>
                <div style={{ fontFamily: WR_MONO, fontSize: 14, color: PAL.brass, letterSpacing: "3px" }}>
                  PAPER
                </div>
              </div>
              ...
              {statusConf && (
                <div style={{ marginTop: 6, marginBottom: 8 }}>
                  <span style={{
                    ...
                    padding: "5px 14px", fontFamily: WR_MONO,
                    fontSize: 11, letterSpacing: "2px", textTransform: "uppercase",
                    color: statusConf.color,
                    background: `${statusConf.color}12`,
                    border: `1px solid ${statusConf.color}50`,
                  }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: statusConf.color, boxShadow: `0 0 6px ${statusConf.color}` }} />
                    {statusConf.label}
                  </span>
                  <div style={{ fontSize: 11, color: PAL.parchDim, marginTop: 6, fontStyle: "italic" }}>{statusConf.sub}</div>
                </div>
              )}
            </div>
            ...
            {(b.status === "CLAIMED" || b.status === "SETTLED") && b.proofNote && (
              <div style={{
                ...
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: PAL.brass, marginBottom: 8, opacity: 0.75 }}>
                  Proof · {b.claimantId ? (
                    <Link href={`/windrose/hunter/${encodeURIComponent(b.claimantId)}`} style={{ color: PAL.brass, textDecoration: "underline dotted", textDecorationColor: `${PAL.brass}60`, textUnderlineOffset: 2 }}>
                      {b.claimantName || "hunter"}
                    </Link>
                  ) : (b.claimantName || "hunter")}
                </div>
                <div style={{ fontSize: 14, color: PAL.parchment, lineHeight: 1.6, fontStyle: "italic" }}>
                  {b.proofNote}
                </div>
                {b.proofImageUrl && (
                  <div style={{ marginTop: 10 }}>
                    <a href={b.proofImageUrl} target="_blank" rel="noopener noreferrer" style={{ color: PAL.verdigris, fontSize: 11, fontFamily: WR_MONO, letterSpacing: "0.5px" }}>
                      View evidence →
                    </a>
                  </div>
                )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
              <div style={{ ...plaqueStyle(), padding: "14px 16px" }}>
                ...
                <Link href={`/windrose/hunter/${encodeURIComponent(b.posterId)}`} style={{ fontFamily: WR_DISPLAY, fontSize: 20, color: PAL.brassHi, letterSpacing: "0.3px", textDecoration: "underline dotted", textDecorationColor: `${PAL.brass}60`, textUnderlineOffset: 3 }}>
                  {b.posterName}
                </Link>
              ...
              {b.claimantId ? (
                <div style={{ ...plaqueStyle(), padding: "14px 16px" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: b.status === "SETTLED" ? "#5db765" : PAL.brass, opacity: 0.75, marginBottom: 6 }}>
                    {b.status === "SETTLED" ? "Claimed by" : "Pending claim by"}
                  </div>
                  <Link href={`/windrose/hunter/${encodeURIComponent(b.claimantId)}`} style={{ fontFamily: WR_DISPLAY, fontSize: 20, color: PAL.brassHi, letterSpacing: "0.3px", textDecoration: "underline dotted", textDecorationColor: `${PAL.brass}60`, textUnderlineOffset: 3 }}>
                    {b.claimantName || "hunter"}
                  </Link>
                  <div style={{ fontSize: 11, color: PAL.parchDim, marginTop: 4, fontStyle: "italic" }}>
                    {timeAgo(b.claimedAt || "")}
                ...
                <div style={{ ...plaqueStyle(), padding: "14px 16px", opacity: 0.6 }}>
                  ...
                  <div style={{ fontFamily: WR_DISPLAY, fontSize: 20, color: PAL.parchDim, fontStyle: "italic" }}>
                    — no one yet —
                  </div>
                </div>
              )}
