...
export default function InvitePage() {
  ...
  const [state,  setState]  = useState<"loading" | "ready" | "accepting" | "done" | "error" | "expired" | "exhausted">("loading");
  ...
  useEffect(() => {
    ...
    fetch(`${API}/invites/${token}`)
      .then(r => r.json())
      .then(j => {
        if (j.ok) { setInvite(j.invite); setState("ready"); }
        else if (j.error === "expired")   setState("expired");
        else if (j.error === "exhausted") setState("exhausted");
        else { setState("error"); setErrMsg(j.error || "Invalid invite."); }
      })
      .catch(() => { setState("error"); setErrMsg("Could not load invite."); });
  }, [token]);
  ...
  async function accept() {
    ...
    if (j.ok) {
      ...
      setState("error");
      setErrMsg(j.error === "expired" ? "This invite has expired." : j.error === "exhausted" ? "This invite is full." : j.error || "Failed.");
    }
  }
  ...
  if (state === "expired") return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <div style={{ opacity: 0.5, fontSize: 14, textAlign: "center" }}>This invite link has passed its expiry date.</div>
        <a href="/lobby" style={{ padding: "10px 24px", borderRadius: 10, background: "rgba(124,58,237,.15)", border: "1px solid rgba(124,58,237,.30)", color: "rgb(216,180,254)", textDecoration: "none", fontWeight: 700, fontSize: 14 }}>Go to Lobby</a>
      </div>
    </div>
  );
  ...
  if (state === "exhausted") return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <div style={{ opacity: 0.5, fontSize: 14, textAlign: "center" }}>This invite has reached its maximum number of uses.</div>
        <a href="/lobby" style={{ padding: "10px 24px", borderRadius: 10, background: "rgba(124,58,237,.15)", border: "1px solid rgba(124,58,237,.30)", color: "rgb(216,180,254)", textDecoration: "none", fontWeight: 700, fontSize: 14 }}>Go to Lobby</a>
      </div>
    </div>
  );
    ...
    <div style={containerStyle}>
      <div style={cardStyle}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          {invite.note && (
            <div style={{ marginTop: 10, padding: "10px 14px", borderRadius: 10, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.09)", fontSize: 13, opacity: 0.8, fontStyle: "italic" }}>
              "{invite.note}"
            </div>
          )}
        </div>
        ...
        <button
          ...
          style={{ width: "100%", padding: "13px 0", borderRadius: 12, border: "1px solid rgba(124,58,237,.40)", background: "rgba(124,58,237,.18)", color: "rgb(216,180,254)", fontWeight: 800, fontSize: 16, cursor: "pointer", transition: "background .15s", letterSpacing: "-.2px" }}>
          {state === "accepting" ? "Joining…" : invite.type === "PLATFORM" ? "Join Weered" : `Join ${TYPE_LABEL[invite.type] || ""}`}
        </button>

        <div style={{ fontSize: 11, opacity: 0.3 }}>
          {invite.type === "PLATFORM" ? "You'll be asked to create an account if you don't have one." : "You'll need to be logged in to accept."}
        </div>
      </div>
    </div>
  );
