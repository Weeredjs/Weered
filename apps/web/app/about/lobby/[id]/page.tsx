...
async function loadLobbyBundle(id: string) {
  const [lobbyRes, roomsRes, postsRes, challengesRes] = await Promise.all([
    ...
    jget<{ posts?: Post[] }>(`${API}/forum/posts?lobbyId=${encodeURIComponent(id)}&limit=10&sort=hot`, { posts: [] }),
  ...
  return {
    ...
    challenges: (challengesRes.challenges ?? []).filter(c => c.status === "ACTIVE" && c.definition?.title).slice(0, 8),
  };
}
...
export default async function LobbyAboutPage(props: { params: Promise<{ id: string }> }) {
  ...
  const lfgGuideIds = new Set<string>(["destiny2","league-of-legends","mtg","helldivers2","poe","dnd"]);
  const hasLfgGuide = lfgGuideIds.has(id);

  if (!lobby?.name) {
    return (
      <main style={{ padding: 24, fontFamily: "system-ui, sans-serif", maxWidth: 720, margin: "40px auto" }}>
        <h1>Lobby not found</h1>
        <p>The lobby <code>{id}</code> doesn&apos;t exist. <Link href="/">Browse Weered →</Link></p>
      </main>
    );
  }

  const lobbyUrl = `${SITE}/lobby/${encodeURIComponent(id)}`;
  const keywords = Array.isArray(lobby.keywords) ? lobby.keywords : (typeof lobby.keywords === "string" ? lobby.keywords.split(",").map(s => s.trim()).filter(Boolean) : []);
  ...
  return (
    <main style={{
      ...
      <nav aria-label="Breadcrumb" style={{ fontSize: 13, opacity: 0.7, marginBottom: 16 }}>
        ...
        {" / "}
        <Link href="/lobby" style={{ color: "#7c9dff" }}>Lobbies</Link>
        {" / "}
        <Link href={lobbyUrl} style={{ color: "#7c9dff" }}>{lobby.name}</Link>
        {" / About"}
      </nav>

      <header style={{ marginBottom: 28 }}>
        <p style={{ fontSize: 12, opacity: 0.6, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>Weered lobby overview</p>
        <h1 style={{ fontSize: 32, margin: "0 0 12px", lineHeight: 1.15 }}>{lobby.name}</h1>
        {lobby.description && (
          <p style={{ fontSize: 17, opacity: 0.92 }}>{lobby.description}</p>
        )}
        <p style={{ marginTop: 16 }}>
          <Link href={lobbyUrl} style={{
            ...
            textDecoration: "none",
            fontWeight: 600,
          }}>Open the {lobby.name} lobby →</Link>
        </p>
      </header>
          ...
          <ul style={{ paddingLeft: 20 }}>
            {posts.map(p => (
              <li key={p.id} style={{ marginBottom: 8 }}>
                <Link href={`/forum/${encodeURIComponent(p.id)}`} style={{ color: "#7c9dff" }}>{p.title}</Link>
                {typeof p.commentCount === "number" && (
                  <span style={{ opacity: 0.65, fontSize: 13 }}> · {p.commentCount} {p.commentCount === 1 ? "reply" : "replies"}</span>
                )}
              </li>
            ))}
          </ul>
      ...
      <footer style={{
        ...
        <p>
          <Link href={lobbyUrl} style={{ color: "#7c9dff", fontWeight: 600 }}>→ Open the live {lobby.name} lobby</Link>
        </p>
        {hasLfgGuide && (
          <p style={{ marginBottom: 8 }}>
            <Link href={`/lfg/${id}`} style={{ color: "#7c9dff" }}>How to find a {lobby.name} group on Weered →</Link>
          </p>
        )}
        <p style={{ marginTop: 8 }}>
          Canonical: <Link href={lobbyUrl} style={{ color: "#7c9dff" }}>{lobbyUrl}</Link>
        </p>
      </footer>
    </main>
  );
...
export async function generateStaticParams() {
  try {
    ...
    return (Array.isArray(lobbies) ? lobbies : []).map((l: any) => ({ id: l.id })).filter((p: any) => p.id);
  } catch {
    return [];
  }
}
