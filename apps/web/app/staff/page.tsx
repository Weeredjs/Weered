...
function RoomsTab({ myRole }: { myRole: GlobalRole }) {
  ...
  async function renameRoom(id: string) {
    ...
    if (!name) return;
    const j = await apiFetch(`/staff/rooms/${encodeURIComponent(id)}/rename`, { method: "POST", body: JSON.stringify({ name }) });
    if (j.ok) { setMsg(`Renamed → ${name}`); setRooms(prev => prev.map(r => r.id === id ? { ...r, name } : r)); setEditId(null); setEditName(""); }
    else setMsg(j.error || "Failed.");
  }

  async function togglePin(id: string, currentlyPinned: boolean) {
    const j = await apiFetch(`/staff/rooms/${encodeURIComponent(id)}/pin`, { method: "POST", body: JSON.stringify({ pinned: !currentlyPinned }) });
    if (j.ok) { setMsg(`${!currentlyPinned ? "Pinned" : "Unpinned"} room`); setRooms(prev => prev.map(r => r.id === id ? { ...r, pinned: !currentlyPinned } : r)); }
    else setMsg(j.error || "Failed.");
  }

  async function closeRoom(id: string, name: string) {
    if (!confirm(`Close room "${name || id}"? This will kick all users and delete the room.`)) return;
    const j = await apiFetch(`/staff/rooms/${encodeURIComponent(id)}/close`, { method: "POST" });
    if (j.ok) { setMsg(`Closed ${name || id}`); setRooms(prev => prev.filter(r => r.id !== id)); }
    else setMsg(j.error || "Failed.");
  }
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {rooms.map(r => {
          ...
          return (
            <div key={r.id} style={{ ...S.card, borderColor: isPinned ? "rgba(245,158,11,.30)" : undefined, background: isPinned ? "rgba(245,158,11,.03)" : undefined }}>
                  ...
                  {isEditing ? (
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <input
                        ...
                        autoFocus
                      />
                      <button style={{ ...S.btnPri, fontSize: 11, padding: "4px 8px" }} onClick={() => renameRoom(r.id)}>Save</button>
                      <button style={{ ...S.btn, fontSize: 11, padding: "4px 8px" }} onClick={() => { setEditId(null); setEditName(""); }}>Cancel</button>
                    </div>
                  ) : (
                    <div style={{ fontWeight: 700, fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {isPinned && <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 999, border: "1px solid rgba(245,158,11,.40)", color: "rgb(253,230,138)", background: "rgba(245,158,11,.12)", fontWeight: 800 }}>📌 PINNED</span>}
                      {liveUsers > 0 && <span style={{ fontSize: 9, padding: "1px 5px", borderRadius: 999, border: "1px solid rgba(34,197,94,.30)", color: "rgba(134,239,172,.85)", background: "rgba(34,197,94,.08)" }}>● {liveUsers} live</span>}
                    </div>
                  )}
              ...
              {canEdit && !isEditing && (
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                  <button style={{ ...S.btn, fontSize: 11, padding: "4px 8px" }} onClick={() => { setEditId(r.id); setEditName(r.name); }}>Rename</button>
                  <button style={{ ...S.btn, fontSize: 11, padding: "4px 8px", borderColor: isPinned ? "rgba(245,158,11,.50)" : "rgba(245,158,11,.25)", color: "rgb(253,230,138)", fontWeight: isPinned ? 800 : 400 }} onClick={() => togglePin(r.id, isPinned)}>
                    {isPinned ? "📌 Unpin" : "Pin"}
                  </button>
                  <button style={{ ...S.warn, fontSize: 11, padding: "4px 8px" }} onClick={() => closeRoom(r.id, r.name)}>Close Room</button>
                  <button style={{ ...S.danger, fontSize: 11, padding: "4px 8px" }} onClick={() => deleteRoom(r.id, r.name)}>Delete</button>
                </div>
              )}
            </div>
          );
...
function ReportsTab() {
  ...
  const [loading, setLoading] = useState(false);
  const [filter, setFilter] = useState<"OPEN" | "REVIEWED" | "ACTIONED" | "DISMISSED" | "ALL">("OPEN");
  ...
  async function action(id: string, status: "REVIEWED" | "ACTIONED" | "DISMISSED") {
    setBusyId(id);
    try {
      const j = await apiFetch(`/staff/reports/${id}/action`, { method: "POST", body: JSON.stringify({ status }) });
      ...
      {!loading && reports.length === 0 && (
        <div style={{ padding: "32px 20px", textAlign: "center", fontSize: 13 }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)", marginTop: 3 }}>
            {filter === "OPEN" ? "When users flag things, they land here." : "Change the filter to see more."}
          </div>
        </div>
      )}
        ...
        {reports.map(r => (
          <div key={r.id} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <span style={{ padding: "3px 8px", borderRadius: 5, background: "rgba(239,68,68,.15)", border: "1px solid rgba(239,68,68,.3)", color: "rgb(252,165,165)", fontSize: 10, fontWeight: 800, letterSpacing: ".05em" }}>
                {REASON_LABELS[r.reason] || r.reason}
              </span>
              <span style={{ fontSize: 11, color: "rgba(255,255,255,.4)", fontFamily: "monospace" }}>
                {r.targetType}
              </span>
              {r.status !== "OPEN" && (
                <span style={{ padding: "3px 8px", borderRadius: 5, background: "rgba(255,255,255,.05)", fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,.5)" }}>
                  {r.status}
                </span>
              )}
              <span style={{ flex: 1 }} />
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,.75)", marginBottom: 6 }}>
              ...
              {r.targetType === "USER" ? <strong>{r.targetName || r.targetId}</strong> : <code style={{ fontSize: 11, color: "rgba(255,255,255,.55)" }}>{r.targetId}</code>}
              {r.context && <> in <code style={{ fontSize: 11, color: "rgba(255,255,255,.55)" }}>{r.context}</code></>}
            </div>
            {r.bodySnapshot && (
              <div style={{ padding: "8px 10px", marginTop: 4, marginBottom: 8, borderRadius: 7, background: "rgba(0,0,0,.3)", border: "1px solid rgba(255,255,255,.05)", fontSize: 12, color: "rgba(255,255,255,.82)", fontStyle: "italic", lineHeight: 1.45 }}>
                "{r.bodySnapshot}"
              </div>
            )}
            {r.note && (
            ...
            {r.reviewerName && r.reviewedAt && (
              <div style={{ fontSize: 10, color: "rgba(255,255,255,.35)", marginBottom: 8 }}>
                {r.status.toLowerCase()} by {r.reviewerName} on {new Date(r.reviewedAt).toLocaleString()}
              </div>
            )}
            {r.status === "OPEN" && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
...
type StaffEvent = {
  ...
  timezone: string; status: string; lobbyId: string | null;
  lobby?: { id: string; name: string } | null;
  createdById: string; createdByName: string;
  promotionStatus: string; promotionNote: string | null;
  promotionDenyReason: string | null;
  broadcastOnPublish: boolean; createdAt: string;
};
...
const PROMO_COLORS: Record<string, { bg: string; border: string; color: string }> = {
  ...
  PENDING:  { bg: "rgba(245,158,11,.10)", border: "rgba(245,158,11,.30)", color: "rgb(253,230,138)" },
  APPROVED: { bg: "rgba(16,185,129,.10)", border: "rgba(16,185,129,.30)", color: "rgb(167,243,208)" },
...
function EventsTab({ myRole }: { myRole: GlobalRole }) {
  ...
  useEffect(() => { loadEvents(); loadPromos(); }, [loadEvents, loadPromos]);
  ...
  async function createEvent() {
    ...
    if (j.ok) {
      ...
      setForm({ title: "", description: "", category: "", coverImageUrl: "", startsAt: "", endsAt: "", timezone: "UTC", status: "DRAFT", broadcastOnPublish: false });
      setView("all");
      loadEvents();
    } else setMsg(j.error || "Failed.");
  }
  ...
  async function updateEvent(id: string, data: any) {
    const j = await apiFetch(`/staff/events/${id}`, { method: "PATCH", body: JSON.stringify(data) });
    if (j.ok) { setMsg("Updated."); loadEvents(); }
    else setMsg(j.error || "Failed.");
  }

  async function deleteEvent(id: string) {
    const j = await apiFetch(`/staff/events/${id}`, { method: "DELETE" });
    if (j.ok) { setMsg("Deleted."); setSelected(null); loadEvents(); }
    else setMsg(j.error || "Failed.");
  }
  ...
  async function reviewPromo(id: string, decision: string, reason?: string) {
    ...
    if (j.ok) { setMsg(`${decision}.`); loadPromos(); loadEvents(); }
    else setMsg(j.error || "Failed.");
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button style={view === "promotions" ? S.btnPri : S.btn} onClick={() => setView("promotions")}>
          Promotions {promos.length > 0 && <span style={{ marginLeft: 4, fontSize: 10, padding: "1px 5px", borderRadius: 999, background: "rgba(245,158,11,.20)", color: "rgb(253,230,138)" }}>{promos.length}</span>}
        </button>
        ...
        <div style={{ ...S.card, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input style={S.input} placeholder="ban_court, raid_night, watch_party..." value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} />
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 18 }}>
              <input type="checkbox" checked={form.broadcastOnPublish} onChange={e => setForm(f => ({ ...f, broadcastOnPublish: e.target.checked }))} />
              <span style={{ fontSize: 12, opacity: 0.7 }}>Broadcast on publish</span>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button style={S.btnPri} onClick={createEvent} disabled={creating}>{creating ? "Creating..." : "Create Event"}</button>
            <button style={S.btn} onClick={() => setView("all")}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {view === "all" && (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <input style={{ ...S.input, flex: 1 }} placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} />
            <select style={{ ...S.input, width: 120, appearance: "auto" }} value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
            ...
            <select style={{ ...S.input, width: 100, appearance: "auto" }} value={scopeFilter} onChange={e => setScopeFilter(e.target.value)}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {events.map(ev => (
              <div key={ev.id} style={{ ...S.card, cursor: "pointer", transition: "border-color .1s", borderColor: selected === ev.id ? "rgba(124,58,237,.35)" : undefined }} onClick={() => setSelected(selected === ev.id ? null : ev.id)}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{ev.title}</div>
                    <div style={{ fontSize: 11, opacity: 0.5, marginTop: 2 }}>
                      {fmtDate(ev.startsAt)} {ev.lobby ? `· ${ev.lobby.name}` : "· GLOBAL"} {ev.category && `· ${ev.category}`}
                    </div>
                  </div>
                ...
                {selected === ev.id && (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,.07)", display: "flex", flexDirection: "column", gap: 8 }}>
                    {ev.description && <div style={{ fontSize: 12, opacity: 0.7 }}>{ev.description}</div>}
                    <div style={{ fontSize: 11, opacity: 0.5 }}>Created by {ev.createdByName} · {fmtDate(ev.createdAt)}</div>
                    {ev.promotionNote && <div style={{ fontSize: 11, opacity: 0.6 }}>Promo note: {ev.promotionNote}</div>}
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {ev.status === "DRAFT" && <button style={S.success} onClick={e => { e.stopPropagation(); updateEvent(ev.id, { status: "PUBLISHED" }); }}>Publish</button>}
                      {ev.status === "PUBLISHED" && <button style={S.btn} onClick={e => { e.stopPropagation(); updateEvent(ev.id, { status: "COMPLETED" }); }}>Complete</button>}
                      {ev.status !== "CANCELED" && <button style={S.warn} onClick={e => { e.stopPropagation(); updateEvent(ev.id, { status: "CANCELED" }); }}>Cancel</button>}
                      <button style={S.danger} onClick={e => { e.stopPropagation(); deleteEvent(ev.id); }}>Delete</button>
                    </div>
                  </div>
                )}
              </div>
            ))}
            {!loading && events.length === 0 && <div style={{ opacity: 0.4, fontSize: 13, padding: "20px 0", textAlign: "center" }}>No events found.</div>}
          </div>
        </div>
      )}
      ...
      {view === "promotions" && (
        <div>
          ...
          {promos.length === 0 && <div style={{ opacity: 0.4, fontSize: 13, padding: "20px 0", textAlign: "center" }}>No pending requests.</div>}
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {promos.map(ev => (
              <div key={ev.id} style={S.card}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{ev.title}</div>
                    <div style={{ fontSize: 11, opacity: 0.5 }}>{ev.lobby?.name || "Unknown lobby"} · {fmtDate(ev.startsAt)}</div>
                  </div>
                  <StatusBadge status={ev.status} />
                </div>
                {ev.description && <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 6 }}>{ev.description}</div>}
                {ev.promotionNote && <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 8, padding: "6px 10px", borderRadius: 6, background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)" }}>"{ev.promotionNote}"</div>}
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={S.success} onClick={() => reviewPromo(ev.id, "APPROVED")}>Approve</button>
                  <button style={S.danger} onClick={() => { const r = prompt("Deny reason (optional):"); reviewPromo(ev.id, "DENIED", r || undefined); }}>Deny</button>
                </div>
              </div>
            ))}
          </div>
    ...
    <div>
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <button
          ...
          style={showExcluded ? S.warn : S.btn}
        >
      ...
      <div style={{ ...S.card, padding: "10px 14px", marginBottom: 14, fontSize: 12, opacity: 0.75 }}>
        <strong style={{ color: "rgb(216,180,254)" }}>How this works:</strong>{" "}
        Hide a mod when the author emails support requesting removal from the catalog.
        Hidden mods stop appearing on /mods/windrose and the in-app Mods tab. The Nexus
        poller will re-upsert metadata, but the excluded flag and note persist.
      </div>
          ...
          {mods.map(m => (
            <div key={m.id} style={S.card}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                {m.thumbnailUrl ? (
                  <img src={m.thumbnailUrl} alt="" style={{ width: 56, height: 32, objectFit: "cover", borderRadius: 4, flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 56, height: 32, borderRadius: 4, background: "rgba(124,58,237,.12)", flexShrink: 0 }} />
                )}
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, opacity: 0.55 }}>
                    by {m.author || "?"} · 👍 {m.endorsements?.toLocaleString() || 0} · ⬇ {m.downloads?.toLocaleString() || 0}
                    {m.sourceUrl && (
                      <>
                        {" · "}
                        <a href={m.sourceUrl} target="_blank" rel="noopener noreferrer" style={{ color: "rgb(167,139,250)" }}>
                          source ↗
                        </a>
                      </>
                    )}
                  ...
                  {m.excluded && m.excludedNote && (
                    <div style={{ fontSize: 11, marginTop: 4, color: "rgb(252,165,165)", fontStyle: "italic" }}>
                      Note: {m.excludedNote}
                      {m.excludedAt && <span style={{ opacity: 0.6 }}> · {new Date(m.excludedAt).toLocaleDateString()}</span>}
                    </div>
                  )}
                </div>
                <button
