  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "LFG guides", item: `${SITE}/lfg` },
      {
        "@type": "ListItem",
        position: 3,
        name: `${g.game_name} ${g.noun_singular} finder`,
        item: pageUrl,
      },
    ],
  };
    ...
    step: [
      {
        "@type": "HowToStep",
        position: 1,
        name: `Open the ${g.game_name} lobby`,
        text: `Visit ${lobbyUrl} and join the lobby with any Weered account (Steam, Xbox, PSN, or email).`,
      },
      {
        "@type": "HowToStep",
        position: 2,
        name: `Pick a room matched to your activity`,
        text: `Each room targets a specific ${g.game_name} activity type (${g.activity_types
          .slice(0, 3)
          .map((a) => a.name)
          .join(", ")}, etc.).`,
      },
      {
        ...
        position: 3,
        name: `Post your LFG`,
        text: `Include the details that matter for ${g.game_name}: ${g.activity_types[0]?.typical_filters || "experience level, schedule, and platform"}.`,
      },
      {
        "@type": "HowToStep",
        position: 4,
        name: `Connect and play`,
        text: `Reply to ${g.noun_plural} that match your fit, jump into the room's voice chat, and play.`,
      },
    ],
  };
  ...
  return (
    ...
    >
      ...
      <nav aria-label="Breadcrumb" style={{ fontSize: 13, opacity: 0.7, marginBottom: 16 }}>
        ...
        {" / "}
        <Link href="/lfg" style={{ color: "#7c9dff" }}>
          LFG guides
        </Link>
        {" / "}
        {g.game_name}
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        <p style={{ opacity: 0.85, marginBottom: 12 }}>
          The {g.game_name} lobby on Weered has dedicated rooms or post categories for each common
          LFG type:
        </p>
          ...
          {g.activity_types.map((a, i) => (
            ...
            >
              ...
              <p style={{ fontSize: 13, opacity: 0.65, margin: 0 }}>
                Common filters: {a.typical_filters}
              </p>
            </div>
          ))}
        </div>
      ...
      >
        <p style={{ marginBottom: 8 }}>
          <Link href={`/lobby/${g.lobby_id}`} style={{ color: "#7c9dff", fontWeight: 600 }}>
            → Open the {g.game_name} lobby
          </Link>
        </p>
        <p style={{ marginBottom: 8 }}>
          <Link href={`/about/lobby/${g.lobby_id}`} style={{ color: "#7c9dff" }}>
            About the {g.game_name} lobby on Weered
          </Link>
        </p>
