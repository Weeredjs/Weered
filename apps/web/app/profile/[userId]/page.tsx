...
function StatBox({ label, value }: { label: string; value: string | number }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 22, fontWeight: 900, color: "var(--weered-text, #f5f5f4)", letterSpacing: "-0.5px" }}>
        {value}
      </span>
      <span style={{ fontSize: 11, opacity: 0.5, textTransform: "uppercase", letterSpacing: "0.8px", fontWeight: 700 }}>
        {label}
      </span>
    </div>
  );
...
export default function ProfilePage() {
  ...
  if (error || !profile) return (
    <div style={pageWrap}>
      <div style={{ opacity: 0.5, fontSize: 14, marginBottom: 16 }}>{error || "Profile not found."}</div>
      ...
      <div style={card}>
        ...
        <div style={{ padding: "0 24px", marginTop: -36, marginBottom: 16, display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8, paddingBottom: 4 }}>
            {isMe && (
              editing ? (
                <>
                  <button onClick={saveBio} disabled={saving} style={{ ...btnStyle, background: `${tier.color}22`, borderColor: `${tier.color}55`, color: tier.color }}>
                    {saving ? "Saving…" : "Save"}
                  </button>
                  <button onClick={() => { setEditing(false); setBio(profile.bio || ""); }} style={btnStyle}>
                    Cancel
                  </button>
                </>
              ) : (
        ...
        <div style={{ padding: "0 24px 20px" }}>
          ...
          {editing ? (
            ...
            <p style={{ fontSize: 13, lineHeight: 1.6, opacity: profile.bio ? 0.85 : 0.35, margin: 0, fontStyle: profile.bio ? "normal" : "italic" }}>
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
            <div style={{
              fontSize: 36, fontWeight: 950, letterSpacing: "-1px",
              color: tier.color,
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: "var(--weered-text, #f5f5f4)" }}>{rank}</div>
...
function NotorietyBar({ value, color }: { value: number; color: string }) {
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5, fontSize: 11, opacity: 0.35 }}>
