...
function HeroBanner({
  ...
  lobby: any;
  onJoin: (id: string, pinned: boolean) => void;
}) {
        ...
        <>
          <div
            style={{
              ...
              inset: 0,
              pointerEvents: "none",
              background: `radial-gradient(ellipse at 25% 50%, ${accent}20 0%, transparent 55%), radial-gradient(ellipse at 85% 20%, ${accent}10 0%, transparent 40%)`,
            }}
          />
          <div
            style={{
      ...
      >
        <div
          style={{
            ...
            fontSize: 26,
            lineHeight: 1.1,
            marginBottom: desc ? 6 : 14,
            letterSpacing: "-0.5px",
            textShadow: banner ? "0 2px 8px rgba(0,0,0,.6)" : "none",
          }}
        >
          {name}
        </div>
        {desc && (
          <div
            style={{
              fontSize: 12,
              color: "rgba(232,232,236,.6)",
              marginBottom: 14,
              maxWidth: 480,
              lineHeight: 1.4,
              textShadow: banner ? "0 1px 4px rgba(0,0,0,.5)" : "none",
              display: "-webkit-box",
              WebkitLineClamp: 3,
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            onJoin(roomId(lobby), Boolean(lobby?.pinned));
          }}
          style={{
            display: "inline-flex",
            alignItems: "center",
...
function LiveRoomCard({
  ...
  const accent =
    room?.lobbyAccentColor && /^#[0-9a-f]{6}$/i.test(room.lobbyAccentColor)
      ? room.lobbyAccentColor
      : "#7C3AED";
  ...
  return (
    <button
      ...
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = `${accent}24`;
...
function FeaturedStreamCard({ stream }: { stream: any }) {
  ...
  const sourceLabel =
    stream.source === "user"
      ? stream.weeredUser?.name
        ? `${stream.weeredUser.name} on Weered`
        : "Live on Weered"
      : stream.source === "game"
        ? `Top stream · ${stream.gameName || "Featured game"}`
        : "Trending on Twitch";
  const sourceColor =
    stream.source === "user" ? "#a78bfa" : stream.source === "game" ? "#f5b700" : "#9146ff";
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = `${sourceColor}66`;
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = `${sourceColor}24`;
      }}
    >
      <div
        style={{
    ...
    >
        ...
        <>
          ...
          <div
            style={{
              ...
              zIndex: 0,
              pointerEvents: "none",
              background: "linear-gradient(180deg, rgba(18,20,24,.36) 0%, rgba(18,20,24,.72) 100%)",
            }}
          />
        </>
      )}
      ...
      <div style={{ padding: "10px 12px 11px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {cnt > 0 && (
            <span
              style={{ fontSize: 9, fontFamily: "monospace", color: heatColor(cnt), opacity: 0.7 }}
            >
              {heat}
            </span>
          )}
    ...
    <div style={{ marginTop: 24 }}>
      ...
      >
        {friends.map((u, i) => {
          ...
          const loc = inRoom
            ? String(u?.room ?? u?.roomId ?? "a room").replace("room:", "")
            : "lobby";
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {room?.lobbyId && (
          <div style={{ fontSize: 10, color: "rgba(255,255,255,.25)", marginTop: 1 }}>
            {pickFirst(room?.lobbyName, room?.lobbyId, "")}
          </div>
        )}
      </div>
...
export default function HomePage() {
  ...
  const allUsers: any[] = useMemo(() => {
    ...
    for (const roomUsers of Object.values(usersByRoom) as any[][]) {
      for (const u of roomUsers) {
        ...
        seen.add(id);
        const rid = Object.keys(usersByRoom).find((rid) =>
  ...
  const lobbies = useMemo(() => {
    const sorted = filtered
      .filter((r) => isPinned(r) && isLobby(r))
      .sort((a, b) => {
        const oa = onlineCount(a),
          ob = onlineCount(b);
    ...
    if (d2idx > 0) {
      const [d2] = sorted.splice(d2idx, 1);
      sorted.unshift(d2);
    }
    return sorted;
  }, [filtered]);
  ...
  React.useEffect(() => {
    ...
    fetch("https://api.weered.ca/friends", { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d?.friends)) setFriendsList(d.friends);
      })
      .catch(() => {});
  }, []);

  const onlineFriends = useMemo(
    () => friendsList.filter((u) => u?.online).slice(0, 12),
    [friendsList],
  );
      ...
      window.dispatchEvent(
        new CustomEvent("weered:dock:open", {
          detail: {
            mode: "dm",
            peer: {
              id: pickFirst(user?.id, user?.userId, ""),
              name: pickFirst(user?.name, user?.username, ""),
            },
          },
        }),
      );
  ...
  return (
    ...
    >
          ...
          <Link
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLAnchorElement).style.background =
                "linear-gradient(135deg, rgba(245,183,0,0.22), rgba(245,183,0,0.08))";
              (e.currentTarget as HTMLAnchorElement).style.borderColor = "rgba(245,183,0,0.45)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLAnchorElement).style.background =
                "linear-gradient(135deg, rgba(245,183,0,0.12), rgba(245,183,0,0.04))";
              (e.currentTarget as HTMLAnchorElement).style.borderColor = "rgba(245,183,0,0.25)";
            }}
          >
            <svg
              width="12"
      ...
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "0 20px 8px" }}>
        ...
        {lobbies.length > 0 &&
          (() => {
            const visibleLobbies = lobbies.filter((r) =>
              featuredId ? roomId(r) !== featuredId : true,
            );
            ...
            const pri = (r: any) => {
              const i = FEATURED_PRIORITY.indexOf(roomId(r));
              return i === -1 ? 999 : i;
            };
            ...
            return (
              <div style={{ marginTop: 24 }}>
                <SectionHeader
                  icon="&#10022;"
                  label="Featured Lobbies"
                  sub="verified communities"
                />
                <div
                  className="weered-lobby-grid"
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(124,58,237,.10), rgba(255,255,255,.02))",
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: "rgba(148,163,184,.55)" }}>
                      Search · discover · find your community
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(245,183,0,.12), rgba(88,0,229,.10))",
                    ...
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLElement).style.borderColor = "rgba(245,183,0,.6)";
                      (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)";
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.borderColor = "rgba(245,183,0,.30)";
                      (e.currentTarget as HTMLElement).style.transform = "translateY(0)";
                    }}
                  >
                    <div
                    ...
                    <div style={{ fontSize: 10, color: "rgba(245,210,122,.55)" }}>
                      {canCreate
                        ? "Your game · your rules · your brand"
                        : "Get Indicted to host your own"}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {me && String(me?.tier || "INNOCENT").toUpperCase() === "INNOCENT" && (
          <div
            ...
            style={{
              ...
              cursor: "pointer",
              background:
                "linear-gradient(135deg, rgba(88,0,229,.08) 0%, rgba(167,139,250,.06) 50%, rgba(249,115,22,.05) 100%)",
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor = "rgba(88,0,229,.35)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.borderColor = "rgba(88,0,229,.18)";
            }}
          >
          ...
          <div style={{ marginTop: 24 }}>
            ...
            >
              {popularRooms.map((r, i) => (
                <LobbyCard key={roomId(r) || i} room={r} idx={i} onJoin={handleJoin} />
              ))}
            </div>
          </div>
        )}
        ...
        <FriendStrip
          ...
          onDm={handleDm}
          onJoin={(u) => {
            const rid = pickFirst(u?.room, u?.roomId, "");
            if (rid) handleJoin(rid);
          }}
        />
        ...
        {recentRooms.length > 0 && (
          <div style={{ marginTop: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {recentRooms.slice(0, 5).map((r, i) => (
                <RecentRow key={roomId(r) || i} room={r} onJoin={handleJoin} />
              ))}
            </div>
          </div>
        )}
          ...
          <EmptyState
            icon={
              <img
                ...
                alt=""
                style={{ width: 72, height: 72, opacity: 0.45 }}
              />
            }
