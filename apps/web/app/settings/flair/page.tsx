...
export default function FlairSettingsPage() {
  ...
  async function loadInventory() {
    if (!token) return;
    try {
      const r = await fetch(`${apiBase}/flair/inventory`, {
  ...
  if (!me) {
    return (
      <div style={{ padding: 24, color: "var(--weered-muted)" }}>Sign in to manage flair.</div>
    );
  }
    ...
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "24px 20px 80px" }}>
      ...
      {isEmpty ? (
        ...
        >
          ...
          <Link
            href="/lobby"
            style={{ fontSize: 13, fontWeight: 800, color: "#a78bfa", textDecoration: "none" }}
          >
            Browse tournaments →
          </Link>
        </div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: "var(--weered-muted, rgba(148,163,184,.6))" }}>
                    · {f.kind}
                  </span>
                </div>
                {entry.isEquipped && (
                  <span
                    style={{ fontSize: 10, fontWeight: 900, color: ring, letterSpacing: "1px" }}
                  >
                    EQUIPPED
                  </span>
                )}
...
function MintForm({ apiBase, token }: { apiBase: string; token: string }) {
  ...
  async function submit() {
    if (!slug || !name) {
      weeredToast("Slug and name required.");
      return;
    }
    setBusy(true);
    try {
