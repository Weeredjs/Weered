...
export function weeredReport(opts: { targetType: ReportTarget; targetId: string; context?: string }): Promise<{ ok: boolean; error?: string } | null> {
...
function getToken(): string { try { return localStorage.getItem("weered_token") || ""; } catch { return ""; } }

function ReportDialog({ targetType, targetId, context, onDone }: { targetType: ReportTarget; targetId: string; context?: string; onDone: (r: { ok: boolean; error?: string } | null) => void }) {
  ...
  async function submit() {
    ...
    try {
      const r = await fetch(`${API_BASE}/reports`, {
        ...
        body: JSON.stringify({ targetType, targetId, context, reason, note: note.trim() || undefined }),
      });
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 4 }}>Report this {targetType.toLowerCase()}.</div>
        <div style={{ fontSize: 12, color: "var(--weered-muted, rgba(148,163,184,.7))", marginBottom: 14 }}>
          ...
          {REASONS.map(r => (
            <button
              ...
              style={{
                ...
                border: reason === r.id ? "1px solid var(--weered-accent-ring, rgba(124,58,237,.5))" : "1px solid var(--weered-border, rgba(255,255,255,.06))",
                background: reason === r.id ? "var(--weered-accent-bg, rgba(124,58,237,.14))" : "transparent",
                color: reason === r.id ? "var(--weered-accent-text, #c4b5fd)" : "var(--weered-text, rgba(243,244,246,.9))",
                cursor: "pointer", fontFamily: "inherit", transition: "all 0.12s",
              }}
            >
              <span style={{ fontWeight: 700, fontSize: 13 }}>{r.label}</span>
              <span style={{ fontSize: 11, color: "var(--weered-muted, rgba(148,163,184,.65))" }}>{r.hint}</span>
            </button>
          ))}
        </div>
        <textarea
          ...
          style={{
            ...
            borderRadius: 8, border: "1px solid var(--weered-border, rgba(255,255,255,.1))",
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button type="button" onClick={() => onDone(null)} disabled={submitting} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid var(--weered-border, rgba(148,163,184,.2))", background: "transparent", color: "var(--weered-muted, rgba(148,163,184,.8))", fontSize: 12, fontWeight: 700, fontFamily: "inherit", cursor: submitting ? "default" : "pointer" }}>Cancel</button>
