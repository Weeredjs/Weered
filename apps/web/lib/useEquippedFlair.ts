...
function notify(userId: string, flair: EquippedFlair | null) {
  ...
  if (!set) return;
  for (const cb of set) try { cb(flair); } catch {}
}

async function fetchFlair(apiBase: string, userId: string): Promise<EquippedFlair | null> {
  try {
    const r = await fetch(`${apiBase}/flair/equipped/${encodeURIComponent(userId)}`, { cache: "no-store" });
    if (!r.ok) return null;
...
export function useEquippedFlair(userId: string | null | undefined, apiBase?: string): EquippedFlair | null {
  ...
  useEffect(() => {
    ...
    if (e && Date.now() - e.at < TTL_MS) {
      ...
      e.promise.then(f => { if (alive) setFlair(f); });
    } else {
      ...
      p.then(f => { if (alive) setFlair(f); });
    }

    return () => {
      alive = false;
      const s = subs.get(userId);
      if (s) { s.delete(cb); if (s.size === 0) subs.delete(userId); }
    };
  }, [userId, base]);
