...
async function fetchMoxfield(deckId: string): Promise<DeckLite | null> {
  try {
    ...
    const thumbnail = commanders[0] ? `https://api.scryfall.com/cards/named?format=image&version=art_crop&fuzzy=${encodeURIComponent(commanders[0])}` : null;
    return {
      source: "moxfield",
      id: deckId,
...
async function fetchArchidekt(deckId: string): Promise<DeckLite | null> {
  try {
    ...
    const colors = Array.isArray(j?.deckFormat?.colors) ? j.deckFormat.colors
      : (Array.isArray(j?.colors) ? j.colors : []);
...
export default async function mtgRoutes(app: FastifyInstance, _opts: Opts) {
  ...
  app.get("/mtg/deck", async (req, reply) => {
    const url = String((req.query as any)?.url || "").trim().slice(0, 500);
    ...
    if (cached && cached.expiresAt > Date.now()) {
      if (!cached.ok || !cached.data) return reply.code(404).send({ ok: false, error: "not_found" });
      return reply.send({ ok: true, deck: cached.data });
    }
    const data = parsed.source === "moxfield"
