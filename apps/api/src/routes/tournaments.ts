...
export default async function tournamentsRoutes(app: FastifyInstance, opts: Opts) {
    ...
    async (req, reply) => {
      ...
      const tournament = await prisma.tournament.create({
        data: {
          ...
          challengePoolIds:
            isRace && Array.isArray(body.challengePoolIds) ? body.challengePoolIds : [],
        } as any,
      });
      return reply.send({ ok: true, tournament });
    },
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const flairRewards = Array.isArray((tournament as any).rewards)
        ? ((tournament as any).rewards as any[])
        : [];
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      for (let r = 1; r <= numWinnerRounds; r++) {
        ...
        wb.push(
          Array.from({ length: matchesInRound }, (_, pos) => ({
            tournamentId: id,
            round: r,
            bracketPosition: pos,
            status: "PENDING",
  ...
  app.get("/tournaments/:id/standings", async (req, reply) => {
    ...
    const stats: Record<
      string,
      { wins: number; losses: number; pointsFor: number; pointsAgainst: number; alive: boolean }
    > = {};
    for (const e of entries)
      stats[e.id] = { wins: 0, losses: 0, pointsFor: 0, pointsAgainst: 0, alive: true };
    for (const m of matches) {
      if (!m.winnerEntryId) continue;
      const loserId = m.winnerEntryId === m.entryAId ? m.entryBId : m.entryAId;
      if (stats[m.winnerEntryId]) stats[m.winnerEntryId].wins++;
  ...
  app.post(
    "/tournaments/:id/matches/:matchId/report",
    {
      schema: {
        tags: ["tournaments"],
        params: z.object({ id: z.string().min(1), matchId: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      const u = authFromHeader((req as any).headers?.authorization);
  ...
  app.post(
    "/tournaments/:id/matches/:matchId/confirm",
    {
      schema: {
        tags: ["tournaments"],
        params: z.object({ id: z.string().min(1), matchId: z.string().min(1) }),
  ...
  app.post(
    "/tournaments/:id/matches/:matchId/dispute",
    {
      schema: {
        tags: ["tournaments"],
        params: z.object({ id: z.string().min(1), matchId: z.string().min(1) }),
  ...
  app.post(
    "/tournaments/:id/matches/:matchId/live",
    {
      schema: {
        tags: ["tournaments"],
        params: z.object({ id: z.string().min(1), matchId: z.string().min(1) }),
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      if (body.twitchLogin !== undefined)
        data.twitchLogin = body.twitchLogin ? String(body.twitchLogin).slice(0, 30) : null;
  ...
  app.post(
    "/tournaments/:id/matches/:matchId/admin-set",
    {
      schema: {
        tags: ["tournaments"],
        params: z.object({ id: z.string().min(1), matchId: z.string().min(1) }),
