...
async function getAutomodReporterId(): Promise<string> {
  ...
  if (fromEnv) {
    ...
    if (u) {
      _automodReporterId = u.id;
      return u.id;
    }
  }
  const staff = await prisma.user.findFirst({
    where: { globalRole: { in: ["FOUNDER", "ADMIN", "STAFF", "MOD"] as any } },
    select: { id: true },
  });
  if (staff) {
    _automodReporterId = staff.id;
    return staff.id;
  }
  const any = await prisma.user.findFirst({ select: { id: true } });
  if (any) {
    _automodReporterId = any.id;
    return any.id;
  }
  throw new Error("no_user_for_automod_reporter");
}

async function isLobbyModOrStaff(
  userId: string,
  lobbyId: string | null,
  opts: Opts,
): Promise<{ ok: boolean; isOwner: boolean; isStaff: boolean }> {
...
export async function runAutoMod(
  lobbyId: string | null,
  content: { kind: "POST" | "COMMENT"; title?: string; body: string; authorId: string },
): Promise<{
  blocked: boolean;
  ...
  for (const rule of rules) {
      ...
      if (rule.kind === ForumAutoModKind.REGEX_FILTER) {
        ...
        for (const w of words) {
          const lw = String(w || "")
            .toLowerCase()
            .trim();
          if (lw && lowerText.includes(lw)) {
            trigger = true;
            reason = `blocked word: ${lw}`;
            break;
          }
        }
      } else if (rule.kind === ForumAutoModKind.LINK_BLOCK) {
        const allowed: string[] = Array.isArray(cfg.allowedDomains)
          ? cfg.allowedDomains.map((d: any) => String(d).toLowerCase())
          : [];
        ...
        while ((m = urlRe.exec(text)) !== null) {
          ...
          if (!ok) {
            trigger = true;
            reason = `disallowed link host: ${host}`;
            break;
          }
        }
      } else if (rule.kind === ForumAutoModKind.KARMA_MIN) {
        const min = Number(cfg.min || 0);
        const agg = await prisma.forumPost.aggregate({
          where: { lobbyId, authorId: content.authorId },
          _sum: { score: true },
        });
        const cAgg = await prisma.forumComment.aggregate({
          where: { authorId: content.authorId, post: { lobbyId } },
          _sum: { score: true },
        });
        const total = (agg._sum.score || 0) + (cAgg._sum.score || 0);
        if (total < min) {
          trigger = true;
          reason = `lobby karma ${total} < ${min}`;
        }
        ...
        if (u) {
          ...
          if (ageDays < minDays) {
            trigger = true;
            reason = `account age ${ageDays.toFixed(1)}d < ${minDays}d`;
          }
        }
      }
    } catch (e) {
    ...
    if (trigger) {
      return {
        blocked: rule.action === ForumAutoModAction.REMOVE,
        action: rule.action,
        ruleId: rule.id,
        ruleName: rule.name,
        reason,
...
export async function autoModTick() {
  try {
    const now = new Date();
    const locked = await prisma.forumPost.findMany({
      where: { locked: true },
      select: { id: true, lobbyId: true, tags: true },
    });
...
export default async function forumModRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.post("/forum/reports", async (req, reply) => {
    ...
    if ((!pid && !cid) || (pid && cid))
      return reply.code(400).send({ error: "must_specify_one_target" });
    ...
    if (pid) {
      ...
      const exists = await prisma.forumComment.findUnique({
        where: { id: cid },
        select: { id: true },
      });
  ...
  app.get("/forum/reports", async (req, reply) => {
    ...
    if (lobbyId) {
      where.OR = [{ post: { lobbyId } }, { comment: { post: { lobbyId } } }];
    }
  ...
  app.patch("/forum/reports/:id", async (req, reply) => {
    ...
    if (!REPORT_STATUSES.includes(String(status)))
      return reply.code(400).send({ error: "invalid_status" });

    const existing = await prisma.forumReport.findUnique({
      where: { id },
      include: {
        post: { select: { lobbyId: true } },
        comment: { select: { post: { select: { lobbyId: true } } } },
      },
    });
  ...
  app.post("/forum/posts/:id/remove", async (req, reply) => {
    ...
    await prisma.forumPost.update({
      where: { id },
      data: { removedAt: new Date(), removedById: u.id, removeReason: reason.slice(0, 500) },
    });
  ...
  app.post("/forum/posts/:id/lock", async (req, reply) => {
    ...
    await recordModAction(post.lobbyId, u.id, "POST_LOCK", "POST", id, reason, {
      unlockAt: newTags.unlockAt,
    });
    return reply.send({ ok: true });
  });
  ...
  app.post("/forum/posts/:id/unlock", async (req, reply) => {
    ...
    await prisma.forumPost.update({
      where: { id },
      data: { locked: false, tags: { ...tags, unlockAt: null } },
    });
  ...
  app.get("/forum/mod-actions", async (req, reply) => {
    ...
    const rows = await prisma.forumModAction.findMany({
      where,
      orderBy: { createdAt: "desc" },
      take: limit,
    });
    ...
    return reply.send({
      ...
      actions: rows.map((r) => ({
        ...r,
        modName: mmap[r.modId] || (r.modId === "system" ? "system" : "Unknown"),
      })),
    });
  });
  ...
  app.get("/forum/automod", async (req, reply) => {
    ...
    const perm = await isLobbyModOrStaff(u.id, lobbyId, opts);
    if (!perm.ok) return reply.code(403).send({ error: "forbidden" });
    const rules = await prisma.forumAutoModRule.findMany({
      where: { lobbyId },
      orderBy: { createdAt: "desc" },
    });
  ...
  app.post("/forum/automod", async (req, reply) => {
    ...
    if (!lobbyId || typeof lobbyId !== "string")
      return reply.code(400).send({ error: "lobbyId_required" });
    if (!name || typeof name !== "string") return reply.code(400).send({ error: "name_required" });
    if (!AUTOMOD_KINDS.includes(String(kind)))
      return reply.code(400).send({ error: "invalid_kind" });
    if (!AUTOMOD_ACTIONS.includes(String(action)))
...
function validateAutoModConfig(kind: string, raw: any): { value?: any; error?: string } {
  ...
  if (kind === "WORD_BLOCK") {
    const words = Array.isArray(cfg.words)
      ? cfg.words.map((w: any) => String(w || "").trim()).filter(Boolean)
      : [];
...
export async function applyAutoModSideEffects(
  lobbyId: string | null,
  content: { kind: "POST" | "COMMENT"; title?: string; body: string; authorId: string },
): Promise<{
  blocked: boolean;
  ...
  reason?: string;
  action?: ForumAutoModAction;
  sideEffect?: { removedAt?: Date; removedById?: string; removeReason?: string; tagsPatch?: any };
}> {
  ...
  if (result.action === ForumAutoModAction.SHADOW_REMOVE) {
    ...
    return {
      ...
      sideEffect: {
        removedAt: new Date(),
        removedById: reporterId || content.authorId,
        removeReason: `Auto-mod: ${result.ruleName || ""}`,
      },
    };
  }
  if (result.action === ForumAutoModAction.REQUIRE_REVIEW) {
    return {
      ...
      sideEffect: {
        tagsPatch: {
          requiresReview: true,
          automodRuleId: result.ruleId,
          automodReason: result.reason,
        },
      },
    };
  }
...
export async function fileAutoModReport(
