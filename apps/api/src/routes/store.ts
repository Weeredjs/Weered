...
export default async function storeRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.get("/store", async (req, reply) => {
    ...
    const filtered = items.filter(
      (i: any) => !i.weeklyRotation || i.rotationWeek === currentWeek || i.rotationWeek === null,
    );
  ...
  app.post(
    "/store/buy/:itemId",
    {
      schema: {
        tags: ["store"],
        summary: "Buy a store item with Paper",
      ...
      try {
        ...
        const result: any = await prisma.$transaction(async (tx) => {
          ...
          if (item.maxSupply != null) {
            ...
            const si = await tx.storeItem.findUnique({
              where: { id: itemId },
        ...
        if ((e as any)?.message === "__SOLD_OUT__")
          return reply.code(400).send({ ok: false, error: "sold_out" });
        log.error("[store] purchase error:", e);
  ...
  app.post(
    "/inventory/equip/:userItemId",
    {
      schema: {
        tags: ["store"],
        summary: "Equip an owned item",
        params: z.object({ userItemId: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      const u = authFromHeader((req as any).headers?.authorization);
  ...
  app.post(
    "/inventory/consume/:userItemId",
    {
      schema: {
        tags: ["store"],
        summary: "Consume a consumable item",
        params: z.object({ userItemId: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      ...
      if (!ui || ui.userId !== u.id) return reply.code(404).send({ ok: false, error: "not_found" });
      if (ui.item.category !== "CONSUMABLE")
        return reply.code(400).send({ ok: false, error: "not_consumable" });
  ...
  app.get("/market", async (req, reply) => {
    ...
    const items = itemIds.length
      ? await prisma.storeItem.findMany({
          where: { id: { in: itemIds } },
          select: { id: true, imageUrl: true, category: true, description: true },
        })
      : [];
  ...
  app.post(
    "/market/list",
    {
      schema: {
        tags: ["market"],
        summary: "List an owned item on the market",
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (listing.sellerId === u.id)
        return reply.code(400).send({ ok: false, error: "cant_buy_own" });
      ...
      try {
        ...
        const result: any = await prisma.$transaction(async (tx) => {
          ...
          await tx.user.update({
            ...
            data: { paper: { increment: listing.price } },
          });
          await tx.userItem.update({
            where: { id: listing.userItemId },
            data: {
              userId: u.id,
              acquiredFrom: listing.sellerId,
          ...
          await tx.paperTransaction.create({
            data: {
              ...
              description: `Bought: ${listing.itemName}`,
              refId: listingId,
            },
          });
          await tx.paperTransaction.create({
            data: {
              ...
              type: "EARN_TRADE_SOLD",
              amount: listing.price,
              balance: sellerNewBalance,
              description: `Sold: ${listing.itemName}`,
              refId: listingId,
            },
          });
        ...
        if ((e as any)?.message === "__INSUFFICIENT__")
          return reply.code(400).send({ ok: false, error: "insufficient_paper" });
        log.error("[market] purchase error:", e);
        return reply.code(500).send({ ok: false, error: "purchase_failed" });
      }
    },
  );
  ...
  app.post(
    "/market/cancel/:listingId",
    {
      schema: {
        tags: ["market"],
        summary: "Cancel your market listing",
        params: z.object({ listingId: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      ...
      const listing = await prisma.marketListing.findUnique({ where: { id: listingId } });
      if (!listing || listing.sellerId !== u.id)
        return reply.code(404).send({ ok: false, error: "not_found" });
      if (listing.status !== "ACTIVE")
        return reply.code(400).send({ ok: false, error: "not_active" });
