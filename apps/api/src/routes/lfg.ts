...
export default async function lfgRoutes(app: FastifyInstance, opts: Opts) {
    ...
    async (req, reply) => {
      ...
      const post = await prisma.lfgPost.create({
        data: {
          ...
          maxPlayers: hasRoles
            ? roleSlots.length
            : Math.min(Math.max(Number(body.maxPlayers) || 4, 2), 12),
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (post.status !== "OPEN")
        return reply
          .code(400)
          .send({ ok: false, error: "post_not_open", message: "This fireteam is no longer open." });
      if (post.players.includes(u.id))
        return reply.code(400).send({
          ok: false,
          error: "already_joined",
          message: "You're already in this fireteam.",
        });
      if (post.players.length >= post.maxPlayers)
        return reply
          .code(400)
          .send({ ok: false, error: "full", message: "This fireteam is full." });
  ...
  app.post(
    "/lfg/:postId/roles/:index/claim",
    {
      schema: {
        tags: ["lfg"],
        params: z.object({ postId: z.string().min(1), index: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      const u = authFromHeader((req as any).headers?.authorization);
      ...
      const post = await prisma.lfgPost.findUnique({ where: { id: postId } });
      if (!post) return reply.code(404).send({ ok: false, error: "not_found" });
      if (!Array.isArray(post.roleSlots) || post.roleSlots.length === 0)
        return reply.code(400).send({ ok: false, error: "not_a_role_post" });
      if (post.status === "CLOSED" || post.status === "COMPLETED")
        return reply.code(400).send({ ok: false, error: "post_closed" });
      if (index < 0 || index >= post.roleSlots.length)
        return reply.code(400).send({ ok: false, error: "bad_slot" });
      const claims: string[] = [...post.roleClaims];
      const names: string[] = [...post.roleClaimNames];
      if (claims.includes(u.id))
        return reply
          .code(400)
          .send({ ok: false, error: "already_in", message: "You already hold a slot." });
      if (claims[index])
        return reply
          .code(400)
          .send({ ok: false, error: "slot_taken", message: "That slot is taken." });
      claims[index] = u.id;
      names[index] = u.name;
      ...
      await prisma.lfgPost.update({
        ...
        data: {
          roleClaims: claims,
          roleClaimNames: names,
          players,
          playerNames: names.filter(Boolean),
          status,
        },
      });
      if (post.userId !== u.id) {
        createNotification?.({
          userId: post.userId,
          type: "LOBBY_EVENT",
          title: `${u.name} claimed ${post.roleSlots[index]}`,
          body: post.activity,
          actorId: u.id,
          actorName: u.name,
