...
export default async function dmRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.get("/dm/conversations", async (req, reply) => {
    ...
    for (const m of msgs) {
      ...
      if (!peers.has(peerId))
        peers.set(peerId, { peerId, lastMessage: m.body, lastAt: m.createdAt, unread: 0 });
      if (m.toId === viewer.id && !m.readAt) peers.get(peerId).unread++;
    }
    const peerIds = Array.from(peers.keys());
    const users = await prisma.user.findMany({
      where: { id: { in: peerIds } },
      select: { id: true, name: true, usernameKey: true, avatar: true, avatarColor: true } as any,
    });
  ...
  app.post(
    "/dm/:peerId",
    {
      schema: {
        tags: ["dm"],
        params: z.object({ peerId: z.string().min(1) }),
