...
export default async function mlbRoutes(app: FastifyInstance) {
  ...
  app.get("/mlb/scoreboard", async (req, reply) => {
    try {
      ...
      const res = await fetchWithTimeout(
        `${MLB_API}/schedule?sportId=1&date=${date}&hydrate=linescore,team,probablePitcher`,
      );
  ...
  app.get("/mlb/standings", async (req, reply) => {
    try {
      ...
      const res = await fetchWithTimeout(
        `${MLB_API}/standings?leagueId=103,104&season=${season}&standingsTypes=regularSeason&hydrate=team`,
      );
      const data = await res.json();
  ...
  app.get("/mlb/leaders", async (req, reply) => {
    try {
      ...
      for (const cat of categories) {
        const res = await fetchWithTimeout(
          `${MLB_API}/stats/leaders?leaderCategories=${cat.stat}&season=${season}&sportId=1&limit=10`,
        );
        const data = await res.json();
  ...
  app.get("/mlb/player/search", async (req, reply) => {
    try {
      ...
      const res = await fetchWithTimeout(
        `${MLB_API}/people/search?names=${encodeURIComponent(q)}&sportId=1&active=true&hydrate=currentTeam`,
      );
      const data = await res.json();
  ...
  app.get("/mlb/player/:id/stats", async (req, reply) => {
    try {
      ...
      const [bioRes, statsRes] = await Promise.all([
        ...
        fetchWithTimeout(
          `${MLB_API}/people/${id}/stats?stats=season,career&group=hitting,pitching,fielding&season=${season}`,
        ),
      ]);
        ...
        subset.map(async (gid: number) => {
          try {
            ...
            for (const item of items) {
              const mp4 =
                item.playbacks?.find((p: any) => p.name === "mp4Avc") ||
                item.playbacks?.find((p: any) => p.url?.includes(".mp4"));
              if (!mp4) continue;
              allHighlights.push({
                id: item.id,
                ...
                thumbnailUrl:
                  item.image?.cuts?.find((c: any) => c.width >= 320 && c.width <= 640)?.src ||
  ...
  app.get("/mlb/matchups", async (req, reply) => {
    try {
      ...
      const schedRes = await fetchWithTimeout(
        `${MLB_API}/schedule?sportId=1&date=${date}&hydrate=probablePitcher(stats(type=season)),team,linescore,weather`,
      );
      const schedData = await schedRes.json();
      ...
      const matchups = await Promise.all(
        games.map(async (g: any) => {
          ...
          const mapPitcher = (team: any) => {
            ...
            if (!pp) return null;
            const seasonStats =
              pp.stats?.find((s: any) => s.type?.displayName === "statsSingleSeason")?.stats || {};
            return {
              id: pp.id,
              name: pp.fullName,
