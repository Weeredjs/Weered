...
app.get("/forum/posts", async (req, reply) => {
  ...
  let myBookmarks: Record<string, true> = {};
  if (u) {
...
app.post("/forum/posts", {
  ...
  if (sectionId) {
    ...
    if (lobbyId && sec.lobbyId !== String(lobbyId).trim()) return reply.code(400).send({ error: "Section does not belong to lobby" });
    if (sec.postsOnly) {
  ...
  const data: any = { title: title.trim(), body: body.trim(), category: cat as any, authorId: u.id, authorName: user?.name || "Unknown", lobbyId: validLobbyId, sectionId: validSectionId, tags: cleanTags as any };
...
app.post("/forum/posts/:postId/comments", {
  ...
  if (parentId) {
    ...
    if (parent.postId !== postId) return reply.code(400).send({ error: "Parent belongs to different post" });
    if ((parent.depth || 0) >= COMMENT_MAX_DEPTH - 1) return reply.code(400).send({ error: "max_depth_reached" });
  ...
  const cdata: any = {
    postId, authorId: u.id, authorName: user?.name || "Unknown", body: body.trim(),
    parentId: parentId ? String(parentId) : null, depth, path,
  };
  if (automod.sideEffect?.removedAt) {
...
app.get("/forum/me/bookmarks", async (req, reply) => {
  ...
  const posts = await prisma.forumPost.findMany({
    where: { id: { in: postIds } },
    include: { lobby: { select: { id: true, name: true } }, section: { select: { id: true, name: true } } },
  });
...
app.post("/forum/uploads", async (req, reply) => {
  ...
  try {
    ...
    pipeline = pipeline.resize({ width: FORUM_MAX_DIM, height: FORUM_MAX_DIM, fit: "inside", withoutEnlargement: true });
    let out = await pipeline.webp({ quality: 80 }).toBuffer();
    if (out.length > 1_000_000) {
      out = await sharp(raw, { failOn: "none" })
        .rotate()
        .resize({ width: FORUM_MAX_DIM, height: FORUM_MAX_DIM, fit: "inside", withoutEnlargement: true })
        .webp({ quality: 65 })
        .toBuffer();
    }
...
app.post("/forum/sections", {
  ...
  if (!lobbyId || !slug || !name) return reply.code(400).send({ error: "lobbyId, slug, name required" });
  const lid = String(lobbyId).trim();
  const s = String(slug).trim().toLowerCase();
  if (!SLUG_RE.test(s)) return reply.code(400).send({ error: "Invalid slug (a-z0-9- only, 1-30 chars)" });
  if (!(await canModSectionLobby(u.id, lid))) return reply.code(403).send({ error: "Forbidden" });

  const dup = await prisma.forumSection.findUnique({ where: { lobbyId_slug: { lobbyId: lid, slug: s } } });
...
app.patch("/forum/sections/:id", {
  ...
  if (!(await canModSectionLobby(u.id, existing.lobbyId))) return reply.code(403).send({ error: "Forbidden" });
  ...
  if (typeof body.slug === "string") {
    ...
    if (s !== existing.slug) {
      const dup = await prisma.forumSection.findUnique({ where: { lobbyId_slug: { lobbyId: existing.lobbyId, slug: s } } });
      if (dup) return reply.code(409).send({ error: "Slug already in use for this lobby" });
    }
    data.slug = s;
  }
  ...
  if (typeof body.color === "string" || body.color === null) data.color = body.color ? String(body.color).slice(0, 16) : null;
  if (typeof body.icon === "string" || body.icon === null) data.icon = body.icon ? String(body.icon).slice(0, 32) : null;
...
app.delete("/forum/sections/:id", {
  ...
  if (postCount > 0 && !force) {
    return reply.code(409).send({ error: "Section has posts. Pass ?force=true to confirm.", postCount });
  }
...
app.post("/forum/sections/reorder", {
  ...
  if (!lobbyId || !Array.isArray(order)) return reply.code(400).send({ error: "lobbyId + order[] required" });
  const lid = String(lobbyId).trim();
  if (!(await canModSectionLobby(u.id, lid))) return reply.code(403).send({ error: "Forbidden" });

  const ids = order.map((o: any) => String(o?.id || "")).filter(Boolean);
  const owned = await prisma.forumSection.findMany({ where: { id: { in: ids }, lobbyId: lid }, select: { id: true } });
  const ownedSet = new Set(owned.map(s => s.id));

  await prisma.$transaction(
    order
      .filter((o: any) => ownedSet.has(String(o?.id || "")) && Number.isFinite(Number(o?.order)))
      .map((o: any) => prisma.forumSection.update({ where: { id: String(o.id) }, data: { order: Number(o.order) } }))
  );
...
app.get("/forum/search", async (req, reply) => {
  ...
  return reply.send({
    ...
    query: q,
    results: {
      posts: posts.map(p => ({ ...p, rank: Number(p.rank), author: authorMap[p.authorId] || null })),
      comments: comments.map(c => ({ ...c, rank: Number(c.rank), author: authorMap[c.authorId] || null })),
    },
  });
});
