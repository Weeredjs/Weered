...
export default async function profileRoutes(app: FastifyInstance, opts: Opts) {
      ...
      return reply.send({
        ...
        ...(await (async () => {
          if (!viewer || viewer.id === u.id)
            return { friendStatus: viewer && viewer.id === u.id ? "self" : "none" };
          ...
          try {
            const fr: any = await prisma.friendRequest.findFirst({
              where: {
                OR: [
                  { fromId: viewer.id, toId: u.id },
                  { fromId: u.id, toId: viewer.id },
                ],
          ...
          if (policy !== "OFF" && (policy === "EVERYONE" || friendStatus === "friends")) {
            ...
            if (currentRoomId) {
              joinable = true;
              try {
                currentRoomIsLobby = !!(await prisma.lobby.findUnique({
          ...
          return {
            ...
            joinable,
            currentRoomId: joinable ? currentRoomId : null,
            currentRoomIsLobby,
          };
        })()),
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const avatarColor =
        typeof body.avatarColor === "string" ? body.avatarColor.slice(0, 20) : undefined;
      const avatar = typeof body.avatar === "string" ? body.avatar.slice(0, 500) : undefined;
      const bannerUrl =
        typeof body.bannerUrl === "string" ? body.bannerUrl.slice(0, 500) : undefined;
      const statusText =
        typeof body.statusText === "string" ? body.statusText.trim().slice(0, 80) : undefined;
      const statusEmoji =
        typeof body.statusEmoji === "string" ? body.statusEmoji.trim().slice(0, 24) : undefined;
      const FRAME_KEYS = ["none", "gold", "flames", "crystal", "neon", "circuit"];
      const avatarFrame =
        typeof body.avatarFrame === "string" && FRAME_KEYS.includes(body.avatarFrame)
          ? body.avatarFrame
          : undefined;
      const NAME_KEYS = ["none", "gold", "fire", "ice", "toxic", "royal", "rainbow"];
      const nameEffect =
        typeof body.nameEffect === "string" && NAME_KEYS.includes(body.nameEffect)
          ? body.nameEffect
          : undefined;
      const JOIN_KEYS = ["EVERYONE", "FRIENDS", "OFF"];
      const joinPolicy =
        typeof body.joinPolicy === "string" && JOIN_KEYS.includes(body.joinPolicy)
          ? body.joinPolicy
          : undefined;
      const invitePolicy =
        typeof body.invitePolicy === "string" && JOIN_KEYS.includes(body.invitePolicy)
          ? body.invitePolicy
          : undefined;
      ...
      if (
        ...
        avatar === undefined &&
        bannerUrl === undefined &&
        panelBgColor === undefined &&
        panelAccentColor === undefined &&
        pillBgColor === undefined &&
      ...
      try {
        const u = await prisma.user.update({
          ...
          data: {
            ...
            ...(avatarFrame !== undefined && {
              avatarFrame: avatarFrame === "none" ? null : avatarFrame,
            }),
            ...(nameEffect !== undefined && {
              nameEffect: nameEffect === "none" ? null : nameEffect,
            }),
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!u) return reply.code(401).send({ ok: false, error: "unauthorized" });
      const body: any = (req as any).body || {};
      const raw = String(body.twitchLogin || "")
        .trim()
        .toLowerCase();
      ...
      await prisma.user.update({
        ...
        data: {
          twitchLogin,
          livePresence: twitchLogin ? undefined : Prisma.DbNull,
    ...
    async (req, reply) => {
      ...
      if (!/^[A-Za-z][A-Za-z0-9_-]{2,15}$/.test(raw)) {
        return reply.code(400).send({
          ...
          message:
            "PSN ID must be 3-16 characters, start with a letter, and contain only letters, numbers, dashes, or underscores.",
        });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const primary =
        twitchData && twitchData.source === "TWITCH"
          ? twitchData
          : xboxData && xboxData.source === "XBOX"
            ? xboxData
            : (steamData ?? null);
