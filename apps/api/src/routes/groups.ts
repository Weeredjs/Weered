...
export default async function groupRoutes(app: FastifyInstance, opts: Opts) {
  const { authFromHeader, dmDeliver, isUserOnline, sendPush, resolveMentions, createNotification } =
    opts;
  ...
  app.post(
    "/groups",
    {
      schema: {
        tags: ["groups"],
        body: z
          .object({ name: z.string().optional(), memberIds: z.array(z.string()).optional() })
          .passthrough(),
      },
    },
    async (req, reply) => {
      ...
      const memberIds = Array.from(
        new Set(rawMembers.filter((x) => typeof x === "string" && x && x !== viewer.id)),
      );
      if (memberIds.length === 0)
        return reply.code(400).send({ error: "At least one other member required" });
  ...
  app.get("/groups/:id/messages", async (req, reply) => {
    ...
    try {
      ...
      log.error("[groups/messages GET]", e);
      return reply.code(500).send({ error: "Server error" });
    }
  });
  ...
  app.post(
    "/groups/:id/messages",
    {
      schema: {
        ...
        body: z.object({ body: z.string().min(1) }).passthrough(),
      },
    },
    async (req, reply) => {
      const viewer = authFromHeader((req.headers as any).authorization);
      ...
      try {
        ...
        log.error("[groups/messages POST]", e);
        return reply.code(500).send({ error: "Server error" });
      }
    },
  );
  ...
  app.patch(
    "/groups/:id/messages/:msgId",
    {
      schema: {
        tags: ["groups"],
        params: z.object({ id: z.string().min(1), msgId: z.string().min(1) }),
      },
    },
    async (req, reply) => {
      const viewer = authFromHeader((req.headers as any).authorization);
      ...
      try {
        ...
        log.error("[groups/messages PATCH]", e);
        return reply.code(500).send({ error: "Server error" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        log.error("[groups/messages DELETE]", e);
        return reply.code(500).send({ error: "Server error" });
      }
    },
  );
    ...
    async (req, reply) => {
      ...
      try {
        ...
        deliverToMembers(memberIds, {
          ...
          threadId: id,
          added: [...adds, ...reactivations],
        });
        return reply.send({ ok: true, added: [...adds, ...reactivations] });
      } catch (e) {
        log.error("[groups/members POST]", e);
        return reply.code(500).send({ error: "Server error" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const memberIds = await activeMemberIds(id);
        deliverToMembers([...memberIds, userId], {
        ...
        log.error("[groups/members DELETE]", e);
        return reply.code(500).send({ error: "Server error" });
      }
    },
  );
