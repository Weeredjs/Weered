...
export default async function chatMediaRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.post(
    "/chat/upload",
    { bodyLimit: 16 * 1024 * 1024, schema: { tags: ["chatMedia"] } },
    async (req, reply) => {
      const u = authFromHeader((req as any).headers?.authorization);
      ...
      if (user.mediaBannedUntil && new Date(user.mediaBannedUntil).getTime() > Date.now()) {
        return reply
          .code(403)
          .send({ ok: false, error: "media_banned", until: user.mediaBannedUntil });
      }
      const paid = PAID_TIERS.has(String(user.tier));
      if (!paid && user.notoriety < MIN_NOTORIETY) {
        return reply
          .code(403)
          .send({ ok: false, error: "locked", notoriety: user.notoriety, required: MIN_NOTORIETY });
      }
      ...
      const att = await prisma.chatAttachment.create({
        data: {
          uploaderId: u.id,
          roomId,
          url: `/chat/media/${fname}`,
          thumbUrl: `/chat/media/${tname}`,
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const updated = await prisma.chatAttachment.update({
        ...
        data: {
          reports: { increment: 1 },
          ...(att.reports + 1 >= AUTO_HIDE_REPORTS && att.status === "ACTIVE"
            ? { status: "HIDDEN" }
            : {}),
        },
      });
  ...
  const sweep = async () => {
    try {
      ...
      log.warn("[media] sweep error:", e?.message || e);
    }
  };
