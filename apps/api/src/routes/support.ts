...
export default async function supportRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.post<{
    ...
    if (reason.length < 20) return reply.code(400).send({ ok: false, error: "reason_too_short", message: "Tell us at least 20 characters of context." });
    ...
    const user = await prisma.user.findUnique({ where: { id: la.userId }, select: { id: true, banned: true } });
    if (!user) return reply.code(401).send({ ok: false, error: "invalid_credentials" });
    if (!user.banned) return reply.code(400).send({ ok: false, error: "not_banned", message: "Your account is not banned." });

    const open = await prisma.banAppeal.findFirst({
      where: { userId: user.id, status: "PENDING" },
      select: { id: true, createdAt: true },
    });
    if (open) return reply.code(409).send({ ok: false, error: "already_pending", appealId: open.id, submittedAt: open.createdAt });
  ...
  app.get<{ Querystring: { status?: string; take?: string } }>("/staff/ban-appeals", async (req, reply) => {
    ...
    const rows = await prisma.banAppeal.findMany({
      ...
      take,
      include: {
        user: { select: { id: true, name: true, banReason: true, bannedAt: true, bannedBy: true } },
      },
    });
  ...
  app.post<{
    ...
    if (decision !== "APPROVED" && decision !== "DENIED") {
      return reply.code(400).send({ ok: false, error: "decision_required", message: "decision must be APPROVED or DENIED" });
    }
    ...
    if (appeal.status !== "PENDING") return reply.code(400).send({ ok: false, error: "already_reviewed" });

    await prisma.$transaction(async (tx) => {
      await tx.banAppeal.update({
        where: { id: appealId },
        data: { status: decision, reviewedBy: u.id, reviewerNote: note || null, reviewedAt: new Date() },
      });
  ...
  app.post<{
    ...
    if (body.length < 10) return reply.code(400).send({ ok: false, error: "body_too_short", message: "Tell us at least 10 characters of context." });
