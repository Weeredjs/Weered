...
export default async function helldiversMoRoutes(app: FastifyInstance, opts: Opts) {
  ...
  app.post(
    "/helldivers/major-orders/:moId/claim",
    {
    ...
    async (req, reply) => {
      ...
      const existing = await prisma.challengeEnrollment
        .findUnique({
      ...
      try {
        if (def.paperReward && awardPaper) {
          await awardPaper(
            u.id,
            "CHALLENGE_REWARD",
            Number(def.paperReward),
            `Helldivers 2 Major Order: ${moId}`,
            def.id,
          );
        }
        ...
        log.warn("[helldivers-mo] payout failed", (e as any)?.message);
      }
