...
describe("paperRoutes — atomic daily claim + tip overdraft guard", () => {
  ...
  beforeEach(async () => {
    app = await buildTestApp((a, { authFromHeader }) =>
      paperRoutes(a, { authFromHeader, awardPaper } as any),
    );
  });
  afterEach(async () => {
    try {
    ...
    if (created.length) {
      try {
        await prisma.paperTransaction.deleteMany({ where: { userId: { in: created } } });
      } catch {}
      try {
  ...
  it("grants the daily bonus exactly once under concurrent claims", async () => {
    ...
    const user = await prisma.user.create({
      data: { usernameKey: "itest_daily_" + stamp, name: "daily", paper: 0, lastDailyAt: null },
      select: { id: true },
    });
    ...
    const paper = (
      await prisma.user.findUnique({ where: { id: user.id }, select: { paper: true } })
    )?.paper;
    const grants = await prisma.paperTransaction.count({
      where: { userId: user.id, type: "EARN_DAILY" },
    });
  ...
  it("processes a tip exactly once under concurrent sends (no overdraft, recipient credited once)", async () => {
    ...
    const sender = await prisma.user.create({
      data: { usernameKey: "itest_tipper_" + stamp, name: "tipper_" + stamp, paper: 10 },
      select: { id: true },
    });
    const recip = await prisma.user.create({
      data: { usernameKey: "itest_recip_" + stamp, name: "recip_" + stamp, paper: 0 },
    ...
    const senderPaper = (
      await prisma.user.findUnique({ where: { id: sender.id }, select: { paper: true } })
    )?.paper;
    const recipPaper = (
      await prisma.user.findUnique({ where: { id: recip.id }, select: { paper: true } })
    )?.paper;
    const spend = await prisma.paperTransaction.count({
      where: { userId: sender.id, type: "SPEND_GIFT" },
    });
    const earn = await prisma.paperTransaction.count({
      where: { userId: recip.id, type: "EARN_GIFT" },
    });
  ...
  it("rejects a tip below the minimum (400 amount_too_low)", async () => {
    const stamp = Date.now() + "_" + Math.floor(performance.now());
    const sender = await prisma.user.create({
      data: { usernameKey: "itest_tiplo_" + stamp, name: "tiplo_" + stamp, paper: 100 },
      select: { id: true },
    });
    const recip = await prisma.user.create({
      data: { usernameKey: "itest_reclo_" + stamp, name: "reclo_" + stamp, paper: 0 },
    ...
    const res = await app.inject({
      ...
      url: "/paper/tip",
      headers: {
        authorization: `Bearer ${testToken(sender.id)}`,
