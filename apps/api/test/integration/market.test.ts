...
describe("POST /market/buy — concurrency / double-sell + lost-update guard", () => {
  ...
  beforeEach(async () => {
    ...
    const seller = await prisma.user.create({
      data: { usernameKey: "itest_seller_" + stamp, name: "seller", paper: 0 },
      select: { id: true },
    });
    const buyer = await prisma.user.create({
      data: { usernameKey: "itest_mbuyer_" + stamp, name: "buyer", paper: PRICE },
      select: { id: true },
    });
    sellerId = seller.id;
    buyerId = buyer.id;
    ...
    itemId = item.id;
    const ui = await prisma.userItem.create({
      data: { userId: sellerId, itemId, acquiredFrom: "store", acquiredPrice: PRICE },
      select: { id: true },
    });
    userItemId = ui.id;
  ...
  afterEach(async () => {
    ...
    try {
      await prisma.paperTransaction.deleteMany({ where: { refId: listingId } });
    } catch {}
    try {
      await prisma.marketListing.deleteMany({ where: { id: listingId } });
    } catch {}
    try {
      await prisma.userItem.deleteMany({ where: { id: userItemId } });
    } catch {}
    try {
  ...
  it("sells the listing exactly once under concurrent buys (no double-sell, no overdraft, no lost-update)", async () => {
    ...
    const spend = await prisma.paperTransaction.count({
      where: { userId: buyerId, refId: listingId, type: "SPEND_MARKET" },
    });
    const earn = await prisma.paperTransaction.count({
      where: { userId: sellerId, refId: listingId, type: "EARN_TRADE_SOLD" },
    });
  ...
  it("rejects /market/list with a missing required field (Zod 400)", async () => {
    const res = await app.inject({
      ...
      url: "/market/list",
      headers: {
        authorization: `Bearer ${testToken(sellerId)}`,
