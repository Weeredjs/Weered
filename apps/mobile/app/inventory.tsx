  ...
  return (
    <SafeAreaView edges={["bottom"]} className="flex-1 bg-weered-bg">
      ...
      {q.isLoading ? (
        ...
        >
          {categories.length === 0 && (
            <Text className="text-weered-muted text-sm text-center py-12">No items yet. Visit the Store.</Text>
          )}
          {categories.map((cat) => (
            <View key={cat} className="mt-3">
              <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pb-2">{cat}</Text>
              {byCategory[cat].map((item) => (
                <View key={item.id} className="px-4 py-3 border-b border-border/30 flex-row items-start">
                  {item.imageUrl ? (
                    <Image
                  ...
                  <View className="flex-1 ml-3">
                    ...
                    {item.mintNumber != null && item.maxSupply != null && (
                      <Text className="text-amber-400 text-xs mt-0.5">#{item.mintNumber} of {item.maxSupply}</Text>
                    )}
                    <View className="flex-row mt-2">
                      {item.category === "CONSUMABLE" ? (
                        <Pressable
                          onPress={() => Alert.alert("Use item?", item.name, [
                        ...
                        >
                          <Text className={`text-xs font-bold ${item.equipped ? "text-white" : "text-weered-muted"}`}>
