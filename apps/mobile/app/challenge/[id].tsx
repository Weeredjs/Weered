...
export default function ChallengeDetail() {
  ...
  const enroll = useMutation({
    mutationFn: () =>
      api<{ ok: boolean; error?: string }>(`/challenges/${instanceId}/enroll`, { method: "POST" }),
    onSuccess: (r) => {
      ...
      if (r.error === "bungie_not_linked") {
        Alert.alert(
          "Link Bungie first",
          "This challenge tracks Destiny 2 activity. Link your Bungie account on the web to enroll.",
        );
      }
    },
      ...
      >
        ...
        {me && (
          <View className="px-4 py-3 border-b border-border/20">
            {completed ? (
              ...
              <Pressable
                onPress={() =>
                  Alert.alert("Abandon challenge?", "Progress will be wiped.", [
        ...
        <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-4 pb-2">
          ...
          {d.requireAll
            ? "all"
            : d.requireCount
              ? `${d.requireCount} of ${objectives.length}`
              : "any"}
        </Text>
        {objectives.map((obj) => {
          ...
          return (
            <View key={obj.id} className="px-4 py-2.5 border-b border-border/20">
              <View className="flex-row items-center mb-1">
                ...
                <Text className="text-weered-text text-sm flex-1" numberOfLines={2}>
              ...
              <View className="h-1.5 bg-panel rounded-full overflow-hidden">
                <View
                  style={{ width: `${pct}%` }}
                  className={`h-1.5 rounded-full ${done ? "bg-green-500" : "bg-weered"}`}
                />
              </View>
            </View>
          );
