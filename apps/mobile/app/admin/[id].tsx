...
import { View, Text, Pressable, ScrollView, TextInput, Alert, ActivityIndicator, RefreshControl } from "react-native";
...
type AdminLobby = {
  ...
  accentColor: string | null; logoUrl: string | null; bannerUrl: string | null;
  websiteUrl: string | null; keywords: string[];
  verified: boolean; pinned: boolean;
  moduleType: string | null; enabledModules: string[];
...
type Room = { id: string; name: string; locked: boolean; ownerId: string; onlineCount: number; memberCount: number };
type Audit = { id: string; type: string; actorName: string; targetId?: string; note?: string; ts: string; detail?: string };
...
export default function LobbyAdmin() {
  ...
  const requestsQ = useQuery({
    ...
    queryFn: () => api<{ ok: boolean; requests: { id: string; userId: string; userName: string; createdAt: string; note?: string }[] }>(`/lobbies/${lobbyId}/admin/join-requests`),
  ...
  return (
    <SafeAreaView edges={["bottom"]} className="flex-1 bg-weered-bg">
      ...
      >
        ...
        <TabBtn label={`Members (${data.members.length})`} active={tab === "members"} onPress={() => setTab("members")} />
        <TabBtn label="Requests" active={tab === "requests"} onPress={() => setTab("requests")} />
        <TabBtn label={`Rooms (${data.rooms.length})`} active={tab === "rooms"} onPress={() => setTab("rooms")} />
        <TabBtn label="Audit" active={tab === "audit"} onPress={() => setTab("audit")} />
      </ScrollView>

      <ScrollView
        className="flex-1"
        refreshControl={<RefreshControl refreshing={q.isRefetching} onRefresh={refresh} tintColor="#5800E5" />}
      >
        {tab === "settings" && (
          <SettingsTab lobby={data.lobby} canEdit={perms.has("edit_branding")} onSaved={() => qc.invalidateQueries({ queryKey: ["lobby-admin", lobbyId] })} lobbyId={lobbyId} />
        )}
        {tab === "members" && (
          <MembersTab lobbyId={lobbyId} members={data.members} myLevel={myLevel} roleNames={data.lobby.roleNames || {}} canKick={perms.has("kick")} canBan={perms.has("ban")} canManageRoles={perms.has("manage_roles")} />
        )}
        {tab === "requests" && (
          <RequestsTab lobbyId={lobbyId} requests={requestsQ.data?.requests ?? []} isLoading={requestsQ.isLoading} />
        )}
        {tab === "rooms" && (
          <RoomsTab lobbyId={lobbyId} rooms={data.rooms} canPin={perms.has("pin_rooms")} canDelete={perms.has("manage_rooms")} />
        )}
        {tab === "audit" && (
          <AuditTab audit={data.audit} bans={data.bans} lobbyId={lobbyId} canUnban={perms.has("ban")} />
        )}
      </ScrollView>
    </SafeAreaView>
  );
...
function TabBtn({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} className="px-3 py-3 active:opacity-70">
      <Text className={`text-xs uppercase tracking-wide font-bold ${active ? "text-weered" : "text-weered-muted"}`}>
        {label}
      </Text>
    </Pressable>
  );
...
function SettingsTab({
  lobby, canEdit, onSaved, lobbyId,
}: {
  lobby: AdminLobby; canEdit: boolean; onSaved: () => void; lobbyId: string;
}) {
  ...
  const branding = useMutation({
    mutationFn: () => api(`/lobbies/${lobbyId}/admin/branding`, {
      method: "PATCH",
      body: {
        name, description, accentColor, logoUrl, bannerUrl, websiteUrl,
        keywords: keywords.split(",").map((k) => k.trim()).filter(Boolean),
      },
    }),
    onSuccess: () => { Alert.alert("Saved", "Branding updated."); onSaved(); },
    onError: (e: any) => Alert.alert("Couldn't save", e?.message || "Unknown error"),
  });

  const joinModeMut = useMutation({
    mutationFn: () => api(`/lobbies/${lobbyId}/admin/join-mode`, {
      method: "PATCH",
      body: { joinMode, password: joinPassword },
    }),
    onSuccess: () => { Alert.alert("Saved", "Join mode updated."); onSaved(); },
    onError: (e: any) => Alert.alert("Couldn't save", e?.message || "Unknown error"),
  });
  ...
  return (
    <View className="px-4 py-5">
      <View className="flex-row items-center mb-5">
        ...
        <View className="flex-1 ml-3">
          ...
          <Text className="text-weered-muted text-xs mt-0.5">
            {lobby.verified ? "✓ Verified · " : ""}{lobby.moduleType && lobby.moduleType !== "NONE" ? lobby.moduleType : "No module"}
          </Text>
        </View>
      </View>
      ...
      <Field label="Accent color (hex)">
        <Input value={accentColor} onChangeText={setAccentColor} editable={canEdit} placeholder="#5800E5" />
      </Field>
      <Field label="Logo URL">
        <Input value={logoUrl} onChangeText={setLogoUrl} editable={canEdit} autoCapitalize="none" />
      </Field>
      <Field label="Banner URL">
        <Input value={bannerUrl} onChangeText={setBannerUrl} editable={canEdit} autoCapitalize="none" />
      </Field>
      <Field label="Website URL">
        <Input value={websiteUrl} onChangeText={setWebsiteUrl} editable={canEdit} autoCapitalize="none" />
      </Field>
      <Field label="Keywords (comma separated)">
        <Input value={keywords} onChangeText={setKeywords} editable={canEdit} autoCapitalize="none" />
      </Field>
      ...
      <View className="flex-row flex-wrap mb-3">
        {["OPEN", "APPROVAL", "PASSWORD"].map((m) => (
          ...
          >
            <Text className={`text-xs font-bold ${joinMode === m ? "text-white" : "text-weered-muted"}`}>{m}</Text>
          </Pressable>
        ))}
      </View>
      {joinMode === "PASSWORD" && (
        <Field label="Password">
          <Input value={joinPassword} onChangeText={setJoinPassword} editable={canEdit} autoCapitalize="none" />
        </Field>
      )}
      {canEdit && (
        <Pressable
...
function Input(props: React.ComponentProps<typeof TextInput>) {
  return (
    <TextInput
      ...
      style={[{ fontSize: 14, minHeight: 42 }, props.multiline ? { minHeight: 70, textAlignVertical: "top" } : null]}
    />
  );
}

function MembersTab({
  lobbyId, members, myLevel, roleNames, canKick, canBan, canManageRoles,
}: {
  lobbyId: string; members: Member[]; myLevel: number; roleNames: Record<string, string>;
  ...
  const kick = useMutation({
    mutationFn: (userId: string) => api(`/lobbies/${lobbyId}/admin/members/${userId}/kick`, { method: "POST" }),
    onSuccess: invalidate,
    onError: (e: any) => Alert.alert("Kick failed", e?.message || "Unknown error"),
  });
  const ban = useMutation({
    mutationFn: ({ userId, reason }: { userId: string; reason?: string }) =>
      api(`/lobbies/${lobbyId}/admin/members/${userId}/ban`, { method: "POST", body: { reason: reason || "" } }),
    onSuccess: invalidate,
  ...
  const setRole = useMutation({
    mutationFn: ({ userId, roleLevel }: { userId: string; roleLevel: number }) =>
      api(`/lobbies/${lobbyId}/admin/members/${userId}/role`, { method: "POST", body: { roleLevel } }),
    onSuccess: invalidate,
      ...
      {members.map((m) => {
        ...
        return (
          <View key={m.id} className="px-4 py-3 border-b border-border/30">
            <Pressable onPress={() => router.push(`/user/${m.userId}`)} className="flex-row items-center active:opacity-70">
            ...
            {canActOn && (canKick || canBan || canManageRoles) && (
              <View className="flex-row flex-wrap mt-2">
                ...
                {canKick && (
                  <Pressable
                    onPress={() => Alert.alert("Kick?", `Remove ${m.name}?`, [
                      { text: "Cancel", style: "cancel" },
                      { text: "Kick", style: "destructive", onPress: () => kick.mutate(m.userId) },
                    ])}
                ...
                {canBan && (
                  <Pressable
                    onPress={() => Alert.alert("Ban?", `Ban ${m.name}?`, [
                      { text: "Cancel", style: "cancel" },
                      { text: "Ban", style: "destructive", onPress: () => ban.mutate({ userId: m.userId }) },
                    ])}
...
function RequestsTab({
  ...
  const approve = useMutation({
    mutationFn: (reqId: string) => api(`/lobbies/${lobbyId}/admin/join-requests/${reqId}/approve`, { method: "POST" }),
    onSuccess: invalidate,
    onError: (e: any) => Alert.alert("Failed", e?.message || "Unknown error"),
  });
  const deny = useMutation({
    mutationFn: ({ reqId, reason }: { reqId: string; reason?: string }) =>
      api(`/lobbies/${lobbyId}/admin/join-requests/${reqId}/deny`, { method: "POST", body: { reason: reason || "" } }),
    onSuccess: invalidate,
    onError: (e: any) => Alert.alert("Failed", e?.message || "Unknown error"),
  });

  if (isLoading) return <View className="py-12 items-center"><ActivityIndicator color="#5800E5" /></View>;
  if (requests.length === 0) {
    return <Text className="text-weered-muted text-sm text-center py-12">No pending requests.</Text>;
  }
  return (
    <View className="py-2">
      {requests.map((r) => (
        <View key={r.id} className="px-4 py-3 border-b border-border/30">
          ...
          {!!r.note && <Text className="text-weered-muted text-xs mt-0.5" numberOfLines={2}>{r.note}</Text>}
          <Text className="text-weered-muted text-xs mt-0.5">{new Date(r.createdAt).toLocaleString()}</Text>
...
function RoomsTab({
  lobbyId, rooms, canPin, canDelete,
}: {
  lobbyId: string; rooms: Room[]; canPin: boolean; canDelete: boolean;
}) {
  const qc = useQueryClient();
  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["lobby-admin", lobbyId] });
  ...
  const del = useMutation({
    mutationFn: (roomId: string) => api(`/lobbies/${lobbyId}/admin/rooms/${roomId}`, { method: "DELETE" }),
    onSuccess: invalidate,
    onError: (e: any) => Alert.alert("Failed", e?.message || "Unknown error"),
  });
        ...
        <View key={r.id} className="px-4 py-3 border-b border-border/30">
          ...
          {(canPin || canDelete) && (
            <View className="flex-row mt-2">
              ...
              {canDelete && (
                <Pressable
                  onPress={() => Alert.alert("Delete room?", `Delete "${r.name}"? This can't be undone.`, [
                    { text: "Cancel", style: "cancel" },
...
function AuditTab({
  audit, bans, lobbyId, canUnban,
}: {
  audit: Audit[]; bans: Ban[]; lobbyId: string; canUnban: boolean;
}) {
  const qc = useQueryClient();
  const unban = useMutation({
    mutationFn: (userId: string) => api(`/lobbies/${lobbyId}/admin/members/${userId}/ban`, { method: "DELETE" }),
  ...
  return (
    <View>
      {bans.length > 0 && (
        <View className="py-2">
          <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-2 pb-1">Banned ({bans.length})</Text>
          {bans.map((b) => (
      ...
      <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-3 pb-1">Recent audit log</Text>
      {audit.length === 0 && (
      ...
      {audit.map((a) => (
        <View key={a.id} className="px-4 py-2.5 border-b border-border/30">
          ...
          <Text className="text-weered-muted text-xs mt-0.5">{new Date(a.ts).toLocaleString()}</Text>
        </View>
      ))}
    </View>
  );
