...
export default function Settings() {
  ...
  const wipe = useMutation({
    mutationFn: () => api("/profile/me/delete", {
      method: "POST",
      body: { confirm: "DELETE" },
    }),
    onSuccess: () => {
      ...
      >
        ...
        {blocks.map((b, i) => (
          <View key={b.id}>
            ...
            <View className="flex-row items-center px-4 py-3">
              <Pressable onPress={() => router.push(`/user/${b.userId}`)} className="flex-1 flex-row items-center active:opacity-70">
                <View className="mr-3"><Avatar name={b.name} url={b.avatar} size={36} /></View>
                <View className="flex-1">
                  <Text className="text-weered-text font-semibold" numberOfLines={1}>{b.name}</Text>
                  {!!b.reason && <Text className="text-weered-muted text-xs" numberOfLines={1}>{b.reason}</Text>}
                </View>
              </Pressable>
              <Pressable
        ...
        <View className="mx-4 mt-2 mb-4 p-4 bg-red-500/5 border border-red-500/30 rounded-xl">
          ...
          <Text className="text-weered-muted text-xs mb-3">
            Wipes your profile, bio, email, linked game accounts, push tokens. Messages and activity are anonymized, not deleted.
            This can't be undone. Type <Text className="text-red-400 font-bold">DELETE</Text> to confirm.
          </Text>
          <TextInput
            value={deleteConfirm}
