...
type Tournament = {
  ...
  format: string; entryType: string;
  ...
  return (
    <SafeAreaView edges={["bottom"]} className="flex-1 bg-weered-bg">
      ...
      <View className="flex-row px-2 py-2 border-b border-border/40">
        {STATUSES.map((s) => (
          <Pressable key={s.id || "active"} onPress={() => setStatus(s.id)} className="px-3 py-1 active:opacity-70">
            <Text className={`text-xs font-bold uppercase ${status === s.id ? "text-weered" : "text-weered-muted"}`}>{s.label}</Text>
          </Pressable>
        ))}
      </View>
      ...
      {q.isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <TournamentRow t={item} onPress={() => router.push(`/tournament/${item.id}`)} />}
          ListEmptyComponent={<Text className="text-weered-muted text-sm text-center py-12">No tournaments right now.</Text>}
        />
      )}
    </SafeAreaView>
  );
...
function TournamentRow({ t, onPress }: { t: Tournament; onPress: () => void }) {
  ...
  if (t.status === "REGISTRATION") { badge = "REG OPEN"; badgeColor = "#5800E5"; }
  else if (t.status === "ACTIVE") { badge = "LIVE"; badgeColor = "#22c55e"; }
  else if (t.status === "COMPLETED") { badge = "DONE"; badgeColor = "#94a3b8"; }
  else { badge = t.status; }

  return (
    <Pressable onPress={onPress} className="px-4 py-3 flex-row active:bg-panel">
      {t.iconUrl ? (
        <Image source={{ uri: t.iconUrl }} style={{ width: 56, height: 56, borderRadius: 10, backgroundColor: "#1a1a1a" }} />
      ) : (
        <View style={{ width: 56, height: 56, borderRadius: 10, backgroundColor: "#5800E533", alignItems: "center", justifyContent: "center" }}>
          <Text className="text-weered font-black">🏆</Text>
        </View>
      )}
      <View className="flex-1 ml-3">
        <View className="flex-row items-center mb-0.5">
          <Text style={{ color: badgeColor }} className="text-[10px] font-bold uppercase mr-2">{badge}</Text>
          <Text className="text-weered-muted text-[10px] uppercase">{t.format} · {t.entryType}</Text>
        </View>
        <Text className="text-weered-text font-bold flex-1" numberOfLines={1}>{t.title}</Text>
        {!!t.description && (
          <Text className="text-weered-muted text-xs mt-0.5" numberOfLines={2}>{t.description}</Text>
        )}
        <Text className="text-weered-muted text-xs mt-1">
          {t._count.entries}/{t.maxEntries} entries · starts {starts.toLocaleDateString()}
          {t.status === "ACTIVE" && endsAt.getTime() > now ? ` · ends ${endsAt.toLocaleDateString()}` : ""}
        </Text>
      </View>
    </Pressable>
  );
