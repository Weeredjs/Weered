        ...
        meQ.isLoading ? (
          ...
          >
            <View className="items-center py-8 border-b border-border/30">
              ...
              <Text className="text-weered text-4xl font-black mt-3">
                {meQ.data.score.toLocaleString()}
              </Text>
              ...
              {meQ.data.nextRank && (
                <View className="mt-5 w-64 items-center">
                  <Text className="text-weered-muted text-xs">
                    {meQ.data.nextRank.pointsNeeded} points to{" "}
                    <Text className="text-weered-text font-bold">{meQ.data.nextRank.title}</Text>
                  </Text>
            ...
            <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-5 pb-2">
              Recent events
            </Text>
            ...
            {meQ.data.recentEvents.map((e, i) => (
              <View key={i} className="px-4 py-2.5 border-b border-border/20 flex-row items-center">
                <Text
                  className={`font-bold text-sm w-14 ${e.points >= 0 ? "text-green-400" : "text-red-400"}`}
                >
                  {e.points >= 0 ? "+" : ""}
                  {e.points}
                </Text>
            ...
            {meQ.data.ranks.map((r) => {
              ...
              return (
                ...
                >
                  <Text
                    className={`flex-1 font-semibold text-sm ${isCurrent ? "text-weered" : "text-weered-text"}`}
                  >
                    {r.title}
                  </Text>
              ...
              {l.avatar ? (
                ...
                <View
                  style={{
                    ...
                    borderRadius: 18,
                    marginRight: 10,
                    backgroundColor: l.avatarColor || "#5800E5",
                    alignItems: "center",
