...
export default function PokerTable() {
  ...
  useEffect(() => {
    ...
    return () => {
      off?.();
      wsClient.send({ type: "poker:leave", tableId: tid });
    };
  }, [tid]);

  const cashout = useMutation({
    mutationFn: () =>
      api<{ ok: boolean; cashed?: number; balance?: number; error?: string }>(
        `/poker/${tid}/cashout`,
        { method: "POST" },
      ),
    onSuccess: (r) => {
    ...
    <SafeAreaView edges={["bottom"]} className="flex-1 bg-weered-bg">
        ...
        options={{
          ...
          headerRight: () =>
            mySeat ? (
              <Pressable
                onPress={() =>
                  Alert.alert("Cash out?", "You'll leave the table with your remaining chips.", [
      ...
      {!state ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
          <View className="px-4 py-3 border-b border-border/30">
            ...
            <View className="flex-row items-center mt-1">
              <Text className="text-weered-muted text-xs mr-3">
                Blinds {state.blinds.small}/{state.blinds.big}
              </Text>
          ...
          <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-3 pb-2">
            Seats
          </Text>
          {state.seats.map((s, i) => (
            <SeatRow key={i} seat={s} seatIndex={i} state={state} me={me?.id} />
          ))}
          ...
          {!mySeat && (
            <View className="px-4 mt-5">
              <Pressable
                onPress={() => {
                  setBuyin(String(state.minBuyin));
                  setJoinOpen(true);
                }}
          ...
          {state.lastShowdownResult && (
            <View className="mx-4 mt-4 p-3 bg-panel border border-border rounded-xl">
              ...
              <Text className="text-weered-text text-sm">
                {JSON.stringify(state.lastShowdownResult).slice(0, 200)}
              </Text>
            </View>
          )}
        </ScrollView>
          ...
          {isMyTurn ? (
            <View className="flex-row">
              ...
              {toCall === 0 ? (
                <Pressable
                  onPress={() => act("check")}
                  className="bg-panel border border-border px-3 py-2 rounded-lg mx-1 active:opacity-70"
                >
                ...
                <Pressable
                  onPress={() => act("call")}
                  className="bg-weered px-3 py-2 rounded-lg mx-1 active:opacity-80"
                >
            ...
            <Text className="text-weered-muted text-xs">
              {state.phase === "waiting" ? "Waiting for more players" : "Waiting for turn"}
            </Text>
          )}
        </View>
      )}
          ...
          <View className="flex-1 bg-black/70 justify-center px-5">
            <View className="bg-weered-bg border border-border rounded-2xl p-5">
              ...
              <Text className="text-weered-muted text-xs mb-3">
                Min ${state?.minBuyin} · Max ${state?.maxBuyin} · Blinds {state?.blinds.small}/
                {state?.blinds.big}
              </Text>
              <TextInput
                value={buyin}
        ...
        <Modal transparent animationType="fade" onRequestClose={() => setRaiseOpen(false)}>
          <View className="flex-1 bg-black/70 justify-center px-5">
            <View className="bg-weered-bg border border-border rounded-2xl p-5">
              ...
              <Text className="text-weered-muted text-xs mb-3">
                Current bet ${state.currentBet} · Min $
                {Math.max(state.currentBet * 2, state.blinds.big * 2)} · Your stack ${mySeat?.chips}
              </Text>
              <TextInput
                value={raiseAmount}
...
function CardChip({ card }: { card: Card }) {
  ...
  const isRed =
    card.suit === "h" || card.suit === "d" || card.suit === "hearts" || card.suit === "diamonds";
  ...
  return (
    ...
    >
      {isHidden ? (
        ...
        <>
          <Text style={{ color: isRed ? "#dc2626" : "#111", fontWeight: "900", fontSize: 18 }}>
            {card.rank}
          </Text>
    ...
    >
      ...
      <View className="flex-1">
        <View className="flex-row items-center">
          <Text className={`font-bold text-sm ${isMe ? "text-weered" : "text-weered-text"}`}>
            {seat.name}
          </Text>
        ...
        <Text className="text-weered-muted text-xs">
          ${seat.chips.toLocaleString()} stack · bet ${seat.bet}
        </Text>
      </View>
      <View className="flex-row">
        {seat.cards.map((c, i) => (
