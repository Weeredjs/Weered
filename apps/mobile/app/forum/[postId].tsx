...
type Comment = {
  ...
  score: number; createdAt: string; deletedAt?: string | null;
  author: Author | null; myVote: number;
};
type Post = {
...
type DetailResp = { ok: boolean; post: Post; comments: Comment[]; isMod: boolean; modPerms: { canDelete: boolean; canLock: boolean; canPin: boolean } };
...
export default function ForumPost() {
  ...
  const votePost = useMutation({
    mutationFn: (value: 1 | -1 | 0) => api(`/forum/posts/${id}/vote`, { method: "POST", body: { value } }),
  ...
  const addComment = useMutation({
    mutationFn: () => api(`/forum/posts/${id}/comments`, { method: "POST", body: { body: comment.trim() } }),
    onSuccess: () => {
      setComment("");
  ...
  const deletePost = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["forum-list"] }); router.back(); },
      ...
      <Stack.Screen
        options={{
          ...
          headerRight: () => canDelete ? (
            <Pressable
              onPress={() => Alert.alert("Delete post?", post?.title || "", [
        ...
        >
          <View className="px-4 py-4 border-b border-border/40">
            <View className="flex-row items-center mb-1">
              ...
              <Text className="text-weered-muted text-[10px] font-bold uppercase mr-2">{post.category}</Text>
            </View>
            <Text className="text-weered-text font-black text-xl mb-2">{post.title}</Text>

            <Pressable onPress={() => router.push(`/user/${post.authorId}`)} className="flex-row items-center mb-3 active:opacity-70">
              <Avatar name={post.authorName} url={post.author?.avatar} size={28} />
              <Text className="text-weered-muted text-xs ml-2">
                <Text className="text-weered-text font-semibold">{post.authorName}</Text> · {formatRel(post.createdAt)}
              </Text>
            </Pressable>

            <RichText body={post.body} style={{ color: "rgba(243,244,246,.92)", fontSize: 15, lineHeight: 22 }} />

            <View className="flex-row items-center mt-3">
              <VoteButtons score={post.score} myVote={post.myVote} onVote={(v) => votePost.mutate(v)} />
          ...
          {!post.locked && me && (
            <View className="px-4 mb-4">
              ...
              >
                <Text className="text-white font-bold text-sm">{addComment.isPending ? "Posting…" : "Comment"}</Text>
              </Pressable>
            </View>
          )}
          ...
          {comments.map((c) => {
            ...
            return (
              <View key={c.id} className="px-4 py-3 border-b border-border/20">
                <View className="flex-row items-center mb-1">
                  <Pressable onPress={() => router.push(`/user/${c.authorId}`)} className="flex-row items-center active:opacity-70 flex-1">
                    <Avatar name={c.authorName} url={c.author?.avatar} size={20} />
                    <Text className="text-weered-text font-semibold text-xs ml-2">{c.authorName}</Text>
                    <Text className="text-weered-muted text-xs ml-2">· {formatRel(c.createdAt)}</Text>
                  </Pressable>
                  {!c.deletedAt && canDeleteComment && (
                    <Pressable
                      onPress={() => Alert.alert("Delete comment?", "This can't be undone.", [
                        { text: "Cancel", style: "cancel" },
                ...
                {c.deletedAt ? (
                  ...
                  <RichText body={c.body} style={{ color: "rgba(243,244,246,.88)", fontSize: 14 }} />
                )}
                {!c.deletedAt && (
                  <View className="mt-1.5">
                    <VoteButtons compact score={c.score} myVote={c.myVote} onVote={(v) => voteComment.mutate({ commentId: c.id, value: v })} />
                  </View>
                )}
              </View>
            );
...
function VoteButtons({ score, myVote, onVote, compact = false }: { score: number; myVote: number; onVote: (v: 1 | -1 | 0) => void; compact?: boolean }) {
  ...
  return (
    <View className="flex-row items-center">
      ...
      >
        <Text className={`${sz} font-bold ${myVote === 1 ? "text-weered" : "text-weered-muted"}`}>▲</Text>
      </Pressable>
      ...
      >
        <Text className={`${sz} font-bold ${myVote === -1 ? "text-red-400" : "text-weered-muted"}`}>▼</Text>
      </Pressable>
    </View>
  );
}
