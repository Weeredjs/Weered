...
type StandingsResp = {
  ...
  divisions?: {
    name: string;
    teams: { name: string; wins: number; losses: number; gamesBack: string }[];
  }[];
};
type LeadersResp = {
  ok: boolean;
  leaders?: {
    category: string;
    stat: string;
    players: { name: string; team: string; value: string }[];
  }[];
};
...
function ScoresTab() {
  ...
  if (games.length === 0)
    return <Text className="text-weered-muted text-sm text-center py-6">No games today.</Text>;
  return (
    <View className="py-2">
      {games.map((g) => {
        ...
        return (
          <View key={g.gameId} className="px-4 py-2.5 border-b border-border/20">
            <View className="flex-row items-center mb-0.5">
              <Text
                className={`text-[10px] font-bold uppercase mr-2 ${live ? "text-red-400" : "text-weered-muted"}`}
              >
                {g.status}
              </Text>
              {g.linescore?.currentInning && (
                <Text className="text-weered-muted text-[10px]">
                  {g.linescore.inningHalf} {g.linescore.currentInning}
                </Text>
              )}
            </View>
            <View className="flex-row items-center">
              <Text className="text-weered-text font-bold flex-1">{g.away.abbr}</Text>
              <Text className="text-weered-text font-black text-lg w-8 text-right">
                {g.away.score ?? "-"}
              </Text>
            </View>
            <View className="flex-row items-center">
    ...
    <View className="py-2">
      {divisions.map((div) => (
        <View key={div.name} className="mb-3">
          <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pt-1 pb-1">
            {div.name}
          </Text>
          {div.teams.map((t) => (
            <View
            ...
            >
              ...
              <Text className="text-weered-text text-xs mr-3">
                {t.wins}-{t.losses}
              </Text>
              <Text className="text-weered-muted text-xs">
                {t.gamesBack === "-" ? "—" : `${t.gamesBack} GB`}
              </Text>
            </View>
          ))}
        </View>
    ...
    <View className="py-2">
      {leaders.map((l) => (
        <View key={l.category} className="mb-3 px-4">
          <Text className="text-weered-muted text-xs uppercase tracking-wide pb-1">
            {l.category} · {l.stat}
          </Text>
