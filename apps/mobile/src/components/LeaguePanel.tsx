...
type SummonerResp = {
  ...
  ranked?: {
    solo: { tier: string; rank: string; lp: number; wins: number; losses: number; winRate: number } | null;
    flex: { tier: string; rank: string; lp: number; wins: number; losses: number; winRate: number } | null;
  };
  recentMatches?: {
    matchId: string;
    championName: string;
    kills: number; deaths: number; assists: number;
    cs: number; win: boolean; gameDuration: number; queueId: number;
  }[];
};

type RotationResp = { ok: boolean; freeChampionIds?: number[]; error?: string };
type ChampionsResp = { ok: boolean; champions?: { id: string; key: number; name: string; image: string }[] };
type LeaderboardResp = {
  ok: boolean;
  entries?: { rank: number; gameName: string; tagLine: string; lp: number; wins: number; losses: number; winRate: number }[];
  error?: string;
};
type StreamsResp = {
  ok: boolean;
  streams?: { userLogin: string; userName: string; title: string; viewerCount: number; thumbnailUrl: string }[];
};
...
export function LeaguePanel({ lobbyId }: { lobbyId: string }) {
  ...
  return (
    <View className="border-t border-border/40 pt-3">
      <Text className="text-weered-muted text-xs uppercase tracking-wide px-4 pb-2">League of Legends</Text>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 12 }}>
        ...
        <TabBtn label="🔄 Rotation" active={tab === "rotation"} onPress={() => setTab("rotation")} />
        <TabBtn label="🔍 Summoner" active={tab === "summoner"} onPress={() => setTab("summoner")} />
      </ScrollView>
...
function StreamsTab() {
  const q = useQuery({
    ...
    queryFn: () => api<StreamsResp>(`/twitch/streams?game=${encodeURIComponent("League of Legends")}&first=20`),
    staleTime: 5 * 60 * 1000,
  });
...
function LeaderboardTab() {
  ...
  if (!q.data?.ok || !q.data.entries?.length) {
    return <Text className="text-weered-muted text-sm text-center py-6">Leaderboard unavailable.</Text>;
  }
  return (
    <View className="py-2">
...
function RotationTab() {
  ...
  if (rotationQ.isLoading || champsQ.isLoading) return <View className="py-8 items-center"><ActivityIndicator color="#5800E5" /></View>;
  if (!rotationQ.data?.ok || !rotationQ.data.freeChampionIds?.length) {
    return (
      <Text className="text-weered-muted text-sm text-center py-6">
        Rotation unavailable{rotationQ.data?.error === "riot_not_configured" ? " (Riot API key expired)" : ""}.
      </Text>
    );
  }
  ...
  return (
    <View className="flex-row flex-wrap px-3 py-3">
      {rotationQ.data.freeChampionIds.map((key) => {
        ...
        if (!c) return null;
        return (
          <View key={key} className="items-center w-[22%] mb-3" style={{ marginHorizontal: "1.5%" }}>
            <Image source={{ uri: c.image }} style={{ width: 52, height: 52, borderRadius: 6 }} />
            <Text className="text-weered-text text-xs mt-1" numberOfLines={1}>{c.name}</Text>
          </View>
        );
      })}
    </View>
...
function SummonerTab() {
  ...
  const summonerQ = useQuery({
    ...
    queryFn: () => api<SummonerResp>(`/league/summoner/${encodeURIComponent(submitted!.gameName)}/${encodeURIComponent(submitted!.tagLine)}`),
    enabled: !!submitted,
  });
    ...
    <View className="py-3">
      <View className="px-4 mb-3 flex-row">
        <TextInput
          ...
          style={{ fontSize: 14 }}
        />
        <Pressable onPress={onSearch} className="bg-weered px-4 py-2 rounded-lg ml-2 active:opacity-80 justify-center">
      ...
      {summonerQ.data && !summonerQ.data.ok && (
        <Text className="text-red-400 text-sm px-4 pb-3">
          {summonerQ.data.error === "player_not_found" ? "Player not found."
            : summonerQ.data.error === "riot_not_configured" ? "Riot API key not configured (dev key may have expired)."
      ...
      {summonerQ.data?.ok && summonerQ.data.summoner && (
        <View className="px-4 pb-4">
          <View className="flex-row items-center mb-3">
            <Image source={{ uri: summonerQ.data.summoner.profileIconUrl }} style={{ width: 48, height: 48, borderRadius: 8 }} />
            <View className="flex-1 ml-3">
              <Text className="text-weered-text font-bold">
                {summonerQ.data.summoner.gameName}
                <Text className="text-weered-muted font-normal">#{summonerQ.data.summoner.tagLine}</Text>
              </Text>
              <Text className="text-weered-muted text-xs mt-0.5">Level {summonerQ.data.summoner.summonerLevel}</Text>
            </View>
          </View>

          {summonerQ.data.ranked?.solo && <RankRow label="Solo/Duo" r={summonerQ.data.ranked.solo} />}
          ...
          {(summonerQ.data.recentMatches?.length ?? 0) > 0 && (
            <>
              <Text className="text-weered-muted text-xs uppercase tracking-wide mt-3 mb-1.5">Recent matches</Text>
              {summonerQ.data.recentMatches!.map((m) => (
                <View key={m.matchId} className={`flex-row items-center px-3 py-2 mb-1.5 rounded-lg ${m.win ? "bg-green-500/10 border border-green-500/30" : "bg-red-500/10 border border-red-500/30"}`}>
                  <Text className="text-weered-text font-semibold text-sm flex-1" numberOfLines={1}>{m.championName}</Text>
                  <Text className="text-weered-text text-sm mr-3">{m.kills}/{m.deaths}/{m.assists}</Text>
                  <Text className="text-weered-muted text-xs">{Math.floor(m.gameDuration / 60)}m</Text>
                </View>
              ))}
            </>
          )}
...
function RankRow({ label, r }: { label: string; r: { tier: string; rank: string; lp: number; wins: number; losses: number; winRate: number } }) {
  return (
