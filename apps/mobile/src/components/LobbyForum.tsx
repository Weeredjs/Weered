...
type Post = {
  ...
  authorName: string; score: number; commentCount: number;
  pinned: boolean; locked: boolean; createdAt: string;
};
...
export function LobbyForum({ lobbyId }: { lobbyId: string }) {
  const q = useQuery({
    ...
    queryFn: () => api<ListResp>(`/forum/posts?lobbyId=${encodeURIComponent(lobbyId)}&sort=hot&limit=5`),
    enabled: !!lobbyId,
  });
  ...
  return (
    <View className="border-t border-border/40 pt-3 pb-1">
      ...
      {q.isLoading && <View className="py-4 items-center"><ActivityIndicator color="#5800E5" /></View>}
      ...
      {posts.map((p) => (
        ...
        >
          ...
          <Text className="text-weered-text font-semibold text-sm" numberOfLines={2}>{p.title}</Text>
